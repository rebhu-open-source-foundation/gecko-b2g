...
add_task(
  async function test_storage_local_should_not_cache_idb_open_rejections() {
    ...
    async function test_storage_local_on_idb_disk_full_rejection() {
        ...
        <html>
          ...
          "tab.js"() {
            browser.test.onMessage.addListener(
              async ({ msg, expectErrorOnSet, errorShouldInclude }) => {
                ...
                try {
                  ...
                  if (
                    expectErrorOnSet &&
                    err.message.includes(errorShouldInclude)
                  ) {
                  ...
                  browser.test.fail(
                    `Got an unexpected exception on storage.local.get: ${err}`
                  );
                  throw err;
                }
      ...
      info(
        `Wait the storage.local.set API call to reject while the disk is full`
      );
      await extension.awaitMessage("storage-local-set-rejected");
      info(
        "Got the a rejection on storage.local.set while the disk is full as expected"
      );

      setLowDiskMode(false);
      extension.sendMessage({
        msg: "call-storage-local",
        expectErrorOnSet: false,
      });
      info(
        `Wait the storage.local API calls to resolve successfully once the disk is free again`
      );
      await extension.awaitMessage("storage-local-get-resolved");
      info(
        "storage.local.set and storage.local.get resolve successfully once the disk is free again"
      );
      ...
      info(
        `Wait the storage.local.set transaction to be aborted while the disk is full`
      );
      await extension.awaitMessage("storage-local-set-rejected");
      info(
        "Got the a rejection on storage.local.set while the disk is full as expected"
    ...
    return runWithPrefs(
      [
        ...
        [
          `${ExtensionStorageIDB.IDB_MIGRATED_PREF_BRANCH}.${EXTENSION_ID}`,
          true,
        ],
      ],
