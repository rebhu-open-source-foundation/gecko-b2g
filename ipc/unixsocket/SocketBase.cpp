...
UnixSocketBuffer::UnixSocketBuffer()
    : mSize(0), mOffset(0), mAvailableSpace(0), mData(nullptr) {
...
const uint8_t* UnixSocketBuffer::Consume(size_t aLen) {
...
nsresult UnixSocketBuffer::Read(void* aValue, size_t aLen) {
...
uint8_t* UnixSocketBuffer::Append(size_t aLen) {
...
nsresult UnixSocketBuffer::Write(const void* aValue, size_t aLen) {
...
void UnixSocketBuffer::CleanupLeadingSpace() {
  if (GetLeadingSpace()) {
...
ssize_t UnixSocketRawData::Receive(int aFd) {
  if (!GetTrailingSpace()) {
    if (!GetLeadingSpace()) {
...
ssize_t UnixSocketRawData::Send(int aFd) {
  if (!GetSize()) {
    return 0;
  }
...
SocketConnectionStatus SocketBase::GetConnectionStatus() const {
  return mConnectionStatus;
}

int SocketBase::GetSuggestedConnectDelayMs() const { return mConnectDelayMs; }

void SocketBase::NotifySuccess() {
  mConnectionStatus = SOCKET_CONNECTED;
  mConnectTimestamp = PR_IntervalNow();
  OnConnectSuccess();
}

void SocketBase::NotifyError() {
...
void SocketBase::NotifyDisconnect() {
  mConnectionStatus = SOCKET_DISCONNECTED;
  mConnectDelayMs = CalculateConnectDelayMs();
  mConnectTimestamp = 0;
  OnDisconnect();
...
uint32_t SocketBase::CalculateConnectDelayMs() const {
  uint32_t connectDelayMs = mConnectDelayMs;

  if (mConnectTimestamp &&
      (PR_IntervalNow() - mConnectTimestamp) > connectDelayMs) {
...
void SocketBase::SetConnectionStatus(SocketConnectionStatus aConnectionStatus) {
...
SocketIOBase::~SocketIOBase() { MOZ_COUNT_DTOR(SocketIOBase); }

MessageLoop* SocketIOBase::GetConsumerThread() const { return mConsumerLoop; }

bool SocketIOBase::IsConsumerThread() const {
