...
class UnixSocketBuffer {
 public:
  ...
  const uint8_t* GetData() const { return mData + mOffset; }

  size_t GetSize() const { return mSize - mOffset; }

  const uint8_t* Consume(size_t aLen);

  nsresult Read(void* aValue, size_t aLen);

  nsresult Read(int8_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(uint8_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(int16_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(uint16_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(int32_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(uint32_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(int64_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(uint64_t& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(float& aValue) { return Read(&aValue, sizeof(aValue)); }

  nsresult Read(double& aValue) { return Read(&aValue, sizeof(aValue)); }

  uint8_t* Append(size_t aLen);

  nsresult Write(const void* aValue, size_t aLen);

  nsresult Write(int8_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(uint8_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(int16_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(uint16_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(int32_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(uint32_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(int64_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(uint64_t aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(float aValue) { return Write(&aValue, sizeof(aValue)); }

  nsresult Write(double aValue) { return Write(&aValue, sizeof(aValue)); }
 ...
 protected:
   ...
   */
  void ResetBuffer(uint8_t* aData, size_t aOffset, size_t aSize,
                   size_t aAvailableSpace) {
  ...
  size_t GetLeadingSpace() const { return mOffset; }

  size_t GetTrailingSpace() const { return mAvailableSpace - mSize; }

  size_t GetAvailableSpace() const { return mAvailableSpace; }

  void* GetTrailingBytes() { return mData + mSize; }
