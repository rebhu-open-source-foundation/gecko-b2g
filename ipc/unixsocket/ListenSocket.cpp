...
class ListenSocketIO final : public UnixSocketWatcher, public SocketIOBase {
 public:
  class ListenTask;

  ListenSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop,
                 ListenSocket* aListenSocket, UnixSocketConnector* aConnector);
  ~ListenSocketIO();
...
ListenSocketIO::ListenSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop,
...
UnixSocketConnector* ListenSocketIO::GetConnector() const {
  return mConnector.get();
}

void ListenSocketIO::Listen(ConnectionOrientedSocketIO* aCOSocketIO) {
  ...
  if (!IsOpen()) {
    int fd;
    nsresult rv = mConnector->CreateListenSocket(address, &mAddressLength, fd);
    if (NS_FAILED(rv)) {
      FireSocketError();
      return;
    }
...
void ListenSocketIO::OnError(const char* aFunction, int aErrno) {
  MOZ_ASSERT(MessageLoopForIO::current() == GetIOLoop());
...
void ListenSocketIO::FireSocketError() {
  MOZ_ASSERT(MessageLoopForIO::current() == GetIOLoop());
...
void ListenSocketIO::OnSocketCanAcceptWithoutBlocking() {
  ...
  nsresult rv = mConnector->AcceptStreamSocket(
      GetFd(), reinterpret_cast<struct sockaddr*>(&storage), &addressLength,
      fd);
  if (NS_FAILED(rv)) {
    FireSocketError();
    return;
  ...
  mCOSocketIO->Accept(fd, reinterpret_cast<struct sockaddr*>(&storage),
                      addressLength);
}

// |SocketIOBase|

SocketBase* ListenSocketIO::GetSocketBase() { return mListenSocket; }

bool ListenSocketIO::IsShutdownOnConsumerThread() const {
  MOZ_ASSERT(IsConsumerThread());

  return mListenSocket == nullptr;
}

bool ListenSocketIO::IsShutdownOnIOThread() const {
  return mShuttingDownOnIOThread;
}

void ListenSocketIO::ShutdownOnConsumerThread() {
  MOZ_ASSERT(IsConsumerThread());
  MOZ_ASSERT(!IsShutdownOnConsumerThread());
...
void ListenSocketIO::ShutdownOnIOThread() {
  MOZ_ASSERT(!IsConsumerThread());
...
nsresult ListenSocket::Listen(UnixSocketConnector* aConnector,
...
nsresult ListenSocket::Listen(ConnectionOrientedSocket* aCOSocket) {
  ...
  rv = aCOSocket->PrepareAccept(connector.get(), mIO->GetConsumerThread(),
                                mIO->GetIOLoop(), io);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
void ListenSocket::OnConnectSuccess() { mConsumer->OnConnectSuccess(mIndex); }

void ListenSocket::OnConnectError() { mConsumer->OnConnectError(mIndex); }

void ListenSocket::OnDisconnect() { mConsumer->OnDisconnect(mIndex); }
