...
class StreamSocketIO final : public ConnectionOrientedSocketIO {
 public:
  ...
  StreamSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop,
                 StreamSocket* aStreamSocket, UnixSocketConnector* aConnector);
  StreamSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop, int aFd,
...
StreamSocketIO::StreamSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop,
...
StreamSocket* StreamSocketIO::GetStreamSocket() { return mStreamSocket; }
...
void StreamSocketIO::ClearDelayedConnectTask() {
  MOZ_ASSERT(IsConsumerThread());

  mDelayedConnectTask = nullptr;
}

void StreamSocketIO::CancelDelayedConnectTask() {
  MOZ_ASSERT(IsConsumerThread());
...
nsresult StreamSocketIO::QueryReceiveBuffer(UnixSocketIOBuffer** aBuffer) {
  MOZ_ASSERT(aBuffer);
...
void StreamSocketIO::DiscardBuffer() {
  // Nothing to do.
}

// |SocketIOBase|

SocketBase* StreamSocketIO::GetSocketBase() { return GetDataSocket(); }

bool StreamSocketIO::IsShutdownOnConsumerThread() const {
  MOZ_ASSERT(IsConsumerThread());

  return mStreamSocket == nullptr;
}

bool StreamSocketIO::IsShutdownOnIOThread() const {
  return mShuttingDownOnIOThread;
}

void StreamSocketIO::ShutdownOnConsumerThread() {
  MOZ_ASSERT(IsConsumerThread());
  MOZ_ASSERT(!IsShutdownOnConsumerThread());
...
void StreamSocketIO::ShutdownOnIOThread() {
  MOZ_ASSERT(!IsConsumerThread());
...
StreamSocket::StreamSocket(StreamSocketConsumer* aConsumer, int aIndex)
    : mIO(nullptr), mConsumer(aConsumer), mIndex(aIndex) {
...
void StreamSocket::ReceiveSocketData(UniquePtr<UnixSocketBuffer>& aBuffer) {
  mConsumer->ReceiveSocketData(mIndex, aBuffer);
}

nsresult StreamSocket::Connect(UnixSocketConnector* aConnector, int aDelayMs,
...
nsresult StreamSocket::PrepareAccept(UnixSocketConnector* aConnector,
                                     MessageLoop* aConsumerLoop,
                                     MessageLoop* aIOLoop,
  ...
  mIO = new StreamSocketIO(aConsumerLoop, aIOLoop, -1,
                           UnixSocketWatcher::SOCKET_IS_CONNECTING, this,
                           aConnector);
  aIO = mIO;
...
void StreamSocket::SendSocketData(UnixSocketIOBuffer* aBuffer) {
  MOZ_ASSERT(mIO);
...
void StreamSocket::OnConnectSuccess() { mConsumer->OnConnectSuccess(mIndex); }

void StreamSocket::OnConnectError() { mConsumer->OnConnectError(mIndex); }

void StreamSocket::OnDisconnect() { mConsumer->OnDisconnect(mIndex); }
