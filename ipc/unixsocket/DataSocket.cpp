...
void DataSocketIO::EnqueueData(UnixSocketIOBuffer* aBuffer) {
  if (!aBuffer->GetSize()) {
...
bool DataSocketIO::HasPendingData() const { return !mOutgoingQ.IsEmpty(); }

ssize_t DataSocketIO::ReceiveData(int aFd) {
  MOZ_ASSERT(aFd >= 0);
...
nsresult DataSocketIO::SendPendingData(int aFd) {
  MOZ_ASSERT(aFd >= 0);
...
DataSocket::~DataSocket() { MOZ_COUNT_DTOR_INHERITED(DataSocket, SocketBase); }
