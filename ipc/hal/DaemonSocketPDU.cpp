...
void DaemonSocketPDU::GetHeader(uint8_t& aService, uint8_t& aOpcode,
                                uint16_t& aPayloadSize) {
...
ssize_t DaemonSocketPDU::Send(int aFd) {
  struct iovec iv;
...
ssize_t DaemonSocketPDU::Receive(int aFd) {
  struct iovec iv;
  memset(&iv, 0, sizeof(iv));
...
nsTArray<int> DaemonSocketPDU::AcquireFds() {
...
nsresult DaemonSocketPDU::UpdateHeader() {
...
size_t DaemonSocketPDU::GetPayloadSize() const {
  MOZ_ASSERT(GetSize() >= PDU_HEADER_SIZE);

  return GetSize() - PDU_HEADER_SIZE;
}

void DaemonSocketPDU::OnError(const char* aFunction, int aErrno) {
