...
void LogProtocolError(const char* aFmt, ...) {
  va_list ap;
...
nsresult Convert(int aIn, uint8_t& aOut) {
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn < std::numeric_limits<uint8_t>::min(),
                                  int, uint8_t) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn > std::numeric_limits<uint8_t>::max(),
                                  int, uint8_t)) {
...
nsresult Convert(int aIn, int16_t& aOut) {
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn < std::numeric_limits<int16_t>::min(),
                                  int, int16_t) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn > std::numeric_limits<int16_t>::max(),
                                  int, int16_t)) {
    aOut = 0;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
...
nsresult Convert(int aIn, int32_t& aOut) {
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn < std::numeric_limits<int32_t>::min(),
                                  int, int32_t) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn > std::numeric_limits<int32_t>::max(),
                                  int, int32_t)) {
    aOut = 0;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
...
void PDUInitOp::WarnAboutTrailingData() const {
  ...
  detail::LogProtocolError(
      "Unpacked PDU of type (%x,%x) still contains %zu Bytes of data.", service,
      opcode, size);
}
