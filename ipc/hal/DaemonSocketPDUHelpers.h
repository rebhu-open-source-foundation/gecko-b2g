...
struct DaemonSocketPDUHeader {
  DaemonSocketPDUHeader() : mService(0x00), mOpcode(0x00), mLength(0x00) {}

  DaemonSocketPDUHeader(uint8_t aService, uint8_t aOpcode, uint16_t aLength)
      : mService(aService), mOpcode(aOpcode), mLength(aLength) {}
...
void LogProtocolError(const char*, ...);

inline bool ConvertWarnIfImpl(const char* aFile, unsigned long aLine,
                              bool aCondition, const char* aExpr,
                              const char* aIn, const char* aOut) {
  if (MOZ_UNLIKELY(aCondition)) {
    LogProtocolError("%s:%d: Convert('%s' to '%s') failed: %s", aFile, aLine,
                     aIn, aOut, aExpr);
  }
  return aCondition;
}

inline bool PackWarnIfImpl(const char* aFile, unsigned long aLine,
                           bool aCondition, const char* aExpr,
                           const char* aIn) {
  if (MOZ_UNLIKELY(aCondition)) {
    LogProtocolError("%s:%d: Pack('%s') failed: %s", aFile, aLine, aIn, aExpr);
  }
  return aCondition;
}

inline bool UnpackWarnIfImpl(const char* aFile, unsigned long aLine,
                             bool aCondition, const char* aExpr,
                             const char* aOut) {
  if (MOZ_UNLIKELY(aCondition)) {
    LogProtocolError("%s:%d: Unpack('%s') failed: %s", aFile, aLine, aOut,
                     aExpr);
  }
  return aCondition;
}
...
#define MOZ_HAL_IPC_CONVERT_WARN_IF(condition, in, out)              \
  ::mozilla::ipc::DaemonSocketPDUHelpers::detail::ConvertWarnIfImpl( \
      __FILE__, __LINE__, condition, #condition, #in, #out)

#define MOZ_HAL_IPC_PACK_WARN_IF(condition, in)                   \
  ::mozilla::ipc::DaemonSocketPDUHelpers::detail::PackWarnIfImpl( \
      __FILE__, __LINE__, condition, #condition, #in)

#define MOZ_HAL_IPC_UNPACK_WARN_IF(condition, out)                  \
  ::mozilla::ipc::DaemonSocketPDUHelpers::detail::UnpackWarnIfImpl( \
      __FILE__, __LINE__, condition, #condition, #out)
...
nsresult Convert(bool aIn, int32_t& aOut);

nsresult Convert(int aIn, uint8_t& aOut);

nsresult Convert(int aIn, int16_t& aOut);

nsresult Convert(int aIn, int32_t& aOut);

nsresult Convert(uint8_t aIn, bool& aOut);

nsresult Convert(uint8_t aIn, char& aOut);

nsresult Convert(uint8_t aIn, int& aOut);

nsresult Convert(uint8_t aIn, unsigned long& aOut);

nsresult Convert(uint32_t aIn, int& aOut);

nsresult Convert(uint32_t aIn, uint8_t& aOut);

nsresult Convert(size_t aIn, uint16_t& aOut);
...
template <typename T>
nsresult PackPDU(T aIn, DaemonSocketPDU& aPDU);

nsresult PackPDU(bool aIn, DaemonSocketPDU& aPDU);

inline nsresult PackPDU(uint8_t aIn, DaemonSocketPDU& aPDU) {
...
inline nsresult PackPDU(int32_t aIn, DaemonSocketPDU& aPDU) {
  return aPDU.Write(aIn);
}

inline nsresult PackPDU(uint32_t aIn, DaemonSocketPDU& aPDU) {
  return aPDU.Write(aIn);
}

inline nsresult PackPDU(int64_t aIn, DaemonSocketPDU& aPDU) {
  return aPDU.Write(aIn);
}

inline nsresult PackPDU(uint64_t aIn, DaemonSocketPDU& aPDU) {
  return aPDU.Write(aIn);
}

inline nsresult PackPDU(float aIn, DaemonSocketPDU& aPDU) {
  return aPDU.Write(aIn);
}

inline nsresult PackPDU(double aIn, DaemonSocketPDU& aPDU) {
  return aPDU.Write(aIn);
}

nsresult PackPDU(const DaemonSocketPDUHeader& aIn, DaemonSocketPDU& aPDU);
...
template <typename Tin, typename Tout>
inline nsresult PackPDU(const PackConversion<Tin, Tout>& aIn,
...
template <typename T>
struct PackArray {
  PackArray(const T* aData, size_t aLength) : mData(aData), mLength(aLength) {}
 ...
 */
template <typename T>
inline nsresult PackPDU(const PackArray<T>& aIn, DaemonSocketPDU& aPDU) {
...
template <>
inline nsresult PackPDU<uint8_t>(const PackArray<uint8_t>& aIn,
                                 DaemonSocketPDU& aPDU) {
...
template <>
inline nsresult PackPDU<char>(const PackArray<char>& aIn,
                              DaemonSocketPDU& aPDU) {
  /* Write raw bytes in one pass */
  return aPDU.Write(aIn.mData, aIn.mLength);
}
 ...
 */
inline nsresult PackPDU(const PackCString0& aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(
      PackArray<uint8_t>(reinterpret_cast<const uint8_t*>(aIn.mString.get()),
                         aIn.mString.Length() + 1),
      aPDU);
}
 ...
 */
template <typename T>
nsresult PackPDU(const PackReversed<T>& aIn, DaemonSocketPDU& aPDU);

/* This implementation of |PackPDU| packs elements in |PackArray| in reversed
 * order. (ex. reversed GATT UUID, see bug 1171866)
 */
template <typename U>
inline nsresult PackPDU(const PackReversed<PackArray<U>>& aIn,
                        DaemonSocketPDU& aPDU) {
...
struct PackArray<PackReversed<U>> {
  PackArray(const U* aData, size_t aLength) : mData(aData), mLength(aLength) {}
 ...
 */
template <typename U>
inline nsresult PackPDU(const PackArray<PackReversed<U>>& aIn,
                        DaemonSocketPDU& aPDU) {
  for (size_t i = 0; i < aIn.mLength; ++i) {
...
inline nsresult PackPDU(const T1& aIn1, const T2& aIn2, DaemonSocketPDU& aPDU) {
...
inline nsresult PackPDU(const T1& aIn1, const T2& aIn2, const T3& aIn3,
                        const T4& aIn4, DaemonSocketPDU& aPDU) {
  nsresult rv = PackPDU(aIn1, aPDU);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
inline nsresult PackPDU(const T1& aIn1, const T2& aIn2, const T3& aIn3,
                        const T4& aIn4, const T5& aIn5, DaemonSocketPDU& aPDU) {
  nsresult rv = PackPDU(aIn1, aPDU);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
// introduce link errors on non-handled data types
template <typename T>
nsresult UnpackPDU(DaemonSocketPDU& aPDU, T& aOut);

nsresult UnpackPDU(DaemonSocketPDU& aPDU, bool& aOut);

nsresult UnpackPDU(DaemonSocketPDU& aPDU, char& aOut);

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, int8_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, uint8_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, uint16_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, int32_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, uint32_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, int64_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, uint64_t& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, float& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, double& aOut) {
  return aPDU.Read(aOut);
}

inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, DaemonSocketPDUHeader& aOut) {
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, nsDependentCString& aOut);
 ...
 */
nsresult UnpackPDU(DaemonSocketPDU& aPDU, const UnpackCString0& aOut);
 ...
 */
nsresult UnpackPDU(DaemonSocketPDU& aPDU, const UnpackString0& aOut);
...
template <typename Tin, typename Tout>
inline nsresult UnpackPDU(DaemonSocketPDU& aPDU,
                          const UnpackConversion<Tin, Tout>& aOut) {
  Tin in;
...
template <typename T>
struct UnpackArray {
  UnpackArray(T* aData, size_t aLength) : mData(aData), mLength(aLength) {}

  UnpackArray(UniquePtr<T[]>& aData, size_t aLength)
      : mData(nullptr), mLength(aLength) {
    aData.reset(new T[mLength]);
    mData = aData.get();
  }

  UnpackArray(UniquePtr<T>& aData, size_t aSize, size_t aElemSize)
      : mData(nullptr), mLength(aSize / aElemSize) {
    aData.reset(new T[mLength]);
    mData = aData.get();
  }
...
template <typename T>
inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, const UnpackArray<T>& aOut) {
...
template <typename T>
inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, UnpackArray<T>& aOut) {
  for (size_t i = 0; i < aOut.mLength; ++i) {
    nsresult rv = UnpackPDU(aPDU, aOut.mData[i]);
    if (NS_FAILED(rv)) {
      return rv;
...
template <>
inline nsresult UnpackPDU<uint8_t>(DaemonSocketPDU& aPDU,
                                   const UnpackArray<uint8_t>& aOut) {
  /* Read raw bytes in one pass */
  return aPDU.Read(aOut.mData, aOut.mLength);
}

template <typename T>
inline nsresult UnpackPDU(DaemonSocketPDU& aPDU, nsTArray<T>& aOut) {
 ...
 */
template <typename T>
nsresult UnpackPDU(DaemonSocketPDU& aPDU, const UnpackReversed<T>& aOut);

template <typename U>
inline nsresult UnpackPDU(DaemonSocketPDU& aPDU,
                          const UnpackReversed<UnpackArray<U>>& aOut) {
  for (size_t i = 0; i < aOut.mValue->mLength; ++i) {
    nsresult rv =
        UnpackPDU(aPDU, aOut.mValue->mData[aOut.mValue->mLength - i - 1]);
    if (NS_FAILED(rv)) {
      return rv;
    }
  }
...
class ConstantInitOp3 final {
 public:
  ConstantInitOp3(const T1& aArg1, const T2& aArg2, const T3& aArg3)
      : mArg1(aArg1), mArg2(aArg2), mArg3(aArg3) {}
...
class UnpackPDUInitOp final : private PDUInitOp {
 public:
  ...
  template <typename T1, typename T2, typename T3, typename T4, typename T5,
            typename T6>
  nsresult operator()(T1& aArg1, T2& aArg2, T3& aArg3, T4& aArg4, T5& aArg5,
                      T6& aArg6) const {
    DaemonSocketPDU& pdu = GetPDU();
  ...
  template <typename T1, typename T2, typename T3, typename T4, typename T5,
            typename T6, typename T7, typename T8>
  nsresult operator()(T1& aArg1, T2& aArg2, T3& aArg3, T4& aArg4, T5& aArg5,
                      T6& aArg6, T7& aArg7, T8& aArg8) const {
    DaemonSocketPDU& pdu = GetPDU();
  ...
  nsresult operator()(T1& aArg1, T2& aArg2, T3& aArg3, T4& aArg4, T5& aArg5,
                      T6& aArg6, T7& aArg7, T8& aArg8, T9& aArg9) const {
    DaemonSocketPDU& pdu = GetPDU();
