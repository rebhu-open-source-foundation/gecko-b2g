...
class DaemonSocketIO final : public ConnectionOrientedSocketIO {
 public:
  DaemonSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop, int aFd,
                 ConnectionStatus aConnectionStatus,
                 UnixSocketConnector* aConnector, DaemonSocket* aConnection,
...
DaemonSocketIO::DaemonSocketIO(MessageLoop* aConsumerLoop, MessageLoop* aIOLoop,
...
nsresult DaemonSocketIO::QueryReceiveBuffer(UnixSocketIOBuffer** aBuffer) {
  MOZ_ASSERT(aBuffer);
...
void DaemonSocketIO::ConsumeBuffer() {
  MOZ_ASSERT(mConsumer);

  mConsumer->Handle(*mPDU);
}

void DaemonSocketIO::DiscardBuffer() {
  // Nothing to do.
}

// |SocketIOBase|

SocketBase* DaemonSocketIO::GetSocketBase() { return mConnection; }

bool DaemonSocketIO::IsShutdownOnConsumerThread() const {
  MOZ_ASSERT(IsConsumerThread());

  return mConnection == nullptr;
}

bool DaemonSocketIO::IsShutdownOnIOThread() const {
  return mShuttingDownOnIOThread;
}

void DaemonSocketIO::ShutdownOnConsumerThread() {
  MOZ_ASSERT(IsConsumerThread());
...
void DaemonSocketIO::ShutdownOnIOThread() {
  MOZ_ASSERT(!IsConsumerThread());
...
nsresult DaemonSocket::PrepareAccept(UnixSocketConnector* aConnector,
  ...
  mIO = new DaemonSocketIO(aConsumerLoop, aIOLoop, -1,
                           UnixSocketWatcher::SOCKET_IS_CONNECTING, aConnector,
                           this, mIOConsumer);
  aIO = mIO;
...
void DaemonSocket::SendSocketData(UnixSocketIOBuffer* aBuffer) {
  MOZ_ASSERT(mIO);
...
void DaemonSocket::OnConnectSuccess() { mConsumer->OnConnectSuccess(mIndex); }

void DaemonSocket::OnConnectError() { mConsumer->OnConnectError(mIndex); }

void DaemonSocket::OnDisconnect() { mConsumer->OnDisconnect(mIndex); }
