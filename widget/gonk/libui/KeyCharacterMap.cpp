...
KeyCharacterMap::KeyCharacterMap() : mType(KEYBOARD_TYPE_UNKNOWN) {}
...
status_t KeyCharacterMap::load(const String8& filename, Format format,
                               sp<KeyCharacterMap>* outMap) {
  outMap->clear();

  Tokenizer* tokenizer;
  status_t status = Tokenizer::open(filename, &tokenizer);
  if (status) {
    ALOGE("Error %d opening key character map file %s.", status,
          filename.string());
  } else {
...
status_t KeyCharacterMap::loadContents(const String8& filename,
                                       const char* contents, Format format,
                                       sp<KeyCharacterMap>* outMap) {
  outMap->clear();
...
sp<KeyCharacterMap> KeyCharacterMap::combine(
    const sp<KeyCharacterMap>& base, const sp<KeyCharacterMap>& overlay) {
  if (overlay == NULL) {
    return base;
  }
  if (base == NULL) {
...
sp<KeyCharacterMap> KeyCharacterMap::empty() { return sEmpty; }

int32_t KeyCharacterMap::getKeyboardType() const { return mType; }
...
char16_t KeyCharacterMap::getCharacter(int32_t keyCode,
...
#if DEBUG_MAPPING
  ALOGD("getCharacter: keyCode=%d, metaState=0x%08x ~ Result %d.", keyCode,
        metaState, result);
#endif
  return result;
}

bool KeyCharacterMap::getFallbackAction(
...
#if DEBUG_MAPPING
  ALOGD(
      "getFallbackKeyCode: keyCode=%d, metaState=0x%08x ~ Result %s, "
      "fallback keyCode=%d, fallback metaState=0x%08x.",
      keyCode, metaState, result ? "true" : "false", outFallbackAction->keyCode,
      outFallbackAction->metaState);
#endif
  return result;
}

char16_t KeyCharacterMap::getMatch(int32_t keyCode, const char16_t* chars,
  ...
  if (getKey(keyCode, &key)) {
    ...
    // However, if we find a perfect meta state match for one behavior then use
    // that one.
    for (const Behavior* behavior = key->firstBehavior; behavior;
...
bool KeyCharacterMap::getEvents(int32_t deviceId, const char16_t* chars,
                                size_t numChars,
...
#if DEBUG_MAPPING
      ALOGD(
          "getEvents: deviceId=%d, chars=[%s] ~ Failed to find mapping for "
          "character %d.",
  ...
  ALOGD("getEvents: deviceId=%d, chars=[%s] ~ Generated %d events.", deviceId,
        toString(chars, numChars).string(), int32_t(outEvents.size()));
  for (size_t i = 0; i < outEvents.size(); i++) {
    ALOGD("  Key: keyCode=%d, metaState=0x%08x, %s.", outEvents[i].getKeyCode(),
          outEvents[i].getMetaState(),
...
status_t KeyCharacterMap::mapKey(int32_t scanCode, int32_t usageCode,
                                 int32_t* outKeyCode) const {
  if (usageCode) {
...
bool KeyCharacterMap::matchesMetaState(int32_t eventMetaState,
  ...
  // match those, taking into account that a behavior can specify that it
  // handles one, both or either of a left/right modifier pair.
  if ((eventMetaState & behaviorMetaState) == behaviorMetaState) {
    const int32_t EXACT_META_STATES =
        AMETA_CTRL_ON | AMETA_CTRL_LEFT_ON | AMETA_CTRL_RIGHT_ON |
        AMETA_ALT_ON | AMETA_ALT_LEFT_ON | AMETA_ALT_RIGHT_ON | AMETA_META_ON |
...
bool KeyCharacterMap::findKey(char16_t ch, int32_t* outKeyCode,
                              int32_t* outMetaState) const {
  if (!ch) {
    return false;
  }
...
void KeyCharacterMap::addKey(Vector<KeyEvent>& outEvents, int32_t deviceId,
                             int32_t keyCode, int32_t metaState, bool down,
                             nsecs_t time) {
  outEvents.push();
  KeyEvent& event = outEvents.editTop();
  event.initialize(deviceId, AINPUT_SOURCE_KEYBOARD,
                   down ? AKEY_EVENT_ACTION_DOWN : AKEY_EVENT_ACTION_UP, 0,
                   keyCode, 0, metaState, 0, time, time);
}

void KeyCharacterMap::addMetaKeys(Vector<KeyEvent>& outEvents, int32_t deviceId,
                                  int32_t metaState, bool down, nsecs_t time,
                                  int32_t* currentMetaState) {
  // Add and remove meta keys symmetrically.
  if (down) {
    addLockedMetaKey(outEvents, deviceId, metaState, time, AKEYCODE_CAPS_LOCK,
                     AMETA_CAPS_LOCK_ON, currentMetaState);
    addLockedMetaKey(outEvents, deviceId, metaState, time, AKEYCODE_NUM_LOCK,
                     AMETA_NUM_LOCK_ON, currentMetaState);
    addLockedMetaKey(outEvents, deviceId, metaState, time, AKEYCODE_SCROLL_LOCK,
                     AMETA_SCROLL_LOCK_ON, currentMetaState);
...
bool KeyCharacterMap::addSingleEphemeralMetaKey(Vector<KeyEvent>& outEvents,
                                                int32_t deviceId,
                                                int32_t metaState, bool down,
                                                nsecs_t time, int32_t keyCode,
...
void KeyCharacterMap::addDoubleEphemeralMetaKey(
    Vector<KeyEvent>& outEvents, int32_t deviceId, int32_t metaState, bool down,
    nsecs_t time, int32_t leftKeyCode, int32_t leftKeyMetaState,
    int32_t rightKeyCode, int32_t rightKeyMetaState, int32_t eitherKeyMetaState,
    int32_t* currentMetaState) {
  bool specific = false;
  specific |= addSingleEphemeralMetaKey(outEvents, deviceId, metaState, down,
                                        time, leftKeyCode, leftKeyMetaState,
                                        currentMetaState);
  specific |= addSingleEphemeralMetaKey(outEvents, deviceId, metaState, down,
                                        time, rightKeyCode, rightKeyMetaState,
                                        currentMetaState);

  if (!specific) {
    addSingleEphemeralMetaKey(outEvents, deviceId, metaState, down, time,
                              leftKeyCode, eitherKeyMetaState,
                              currentMetaState);
  }
}

void KeyCharacterMap::addLockedMetaKey(Vector<KeyEvent>& outEvents,
                                       int32_t deviceId, int32_t metaState,
                                       nsecs_t time, int32_t keyCode,
                                       int32_t keyMetaState,
                                       int32_t* currentMetaState) {
  if ((metaState & keyMetaState) == keyMetaState) {
...
KeyCharacterMap::Key::Key() : label(0), number(0), firstBehavior(NULL) {}

KeyCharacterMap::Key::Key(const Key& other)
    : label(other.label),
      number(other.number),
      firstBehavior(other.firstBehavior ? new Behavior(*other.firstBehavior)
                                        : NULL) {}
...
status_t KeyCharacterMap::Parser::parse() {
  ...
  if (mFormat == FORMAT_BASE) {
    if (mMap->mType == KEYBOARD_TYPE_OVERLAY) {
      ALOGE(
          "%s: Base keyboard layout must specify a keyboard 'type' other than "
          "'OVERLAY'.",
          mTokenizer->getLocation().string());
      return BAD_VALUE;
    }
    ...
    if (mMap->mType != KEYBOARD_TYPE_OVERLAY) {
      ALOGE(
          "%s: Overlay keyboard layout missing required keyboard "
...
status_t KeyCharacterMap::Parser::parseType() {
  ...
  if (typeToken == "NUMERIC") {
    ...
    ALOGE("%s: Expected keyboard type label, got '%s'.",
          mTokenizer->getLocation().string(), typeToken.string());
    return BAD_VALUE;
  }
...
status_t KeyCharacterMap::Parser::parseMap() {
  ...
  ALOGE("%s: Expected keyword after 'map', got '%s'.",
        mTokenizer->getLocation().string(), keywordToken.string());
  return BAD_VALUE;
}
...
status_t KeyCharacterMap::Parser::parseMapKey() {
  ...
  if (*end) {
    ALOGE("%s: Expected key %s number, got '%s'.",
          mTokenizer->getLocation().string(), mapUsage ? "usage" : "scan code",
          codeToken.string());
    return BAD_VALUE;
  }
  ...
  if (map.indexOfKey(code) >= 0) {
    ALOGE("%s: Duplicate entry for key %s '%s'.",
          mTokenizer->getLocation().string(), mapUsage ? "usage" : "scan code",
          codeToken.string());
    return BAD_VALUE;
  }
  ...
  if (!keyCode) {
    ALOGE("%s: Expected key code label, got '%s'.",
          mTokenizer->getLocation().string(), keyCodeToken.string());
    return BAD_VALUE;
  }
...
status_t KeyCharacterMap::Parser::parseKey() {
  ...
  if (mMap->mKeys.indexOfKey(keyCode) >= 0) {
    ALOGE("%s: Duplicate entry for key code '%s'.",
          mTokenizer->getLocation().string(), keyCodeToken.string());
    return BAD_VALUE;
  }
...
#if DEBUG_PARSER
        ALOGD("Parsed key number: keyCode=%d, number=%d.", mKeyCode,
              key->number);
#endif
        break;
      case PROPERTY_META: {
...
#if DEBUG_PARSER
        ALOGD("Parsed key meta: keyCode=%d, meta=0x%x, char=%d, fallback=%d.",
              mKeyCode, newBehavior->metaState, newBehavior->character,
              newBehavior->fallbackKeyCode);
#endif
        break;
      }
    }
...
status_t KeyCharacterMap::Parser::parseModifier(const String8& token,
                                                int32_t* outMetaState) {
  if (token == "base") {
    *outMetaState = 0;
    return NO_ERROR;
  }
...
status_t KeyCharacterMap::Parser::parseCharacterLiteral(
    char16_t* outCharacter) {
  char ch = mTokenizer->nextChar();
  if (ch != '\'') {
    goto Error;
  }
