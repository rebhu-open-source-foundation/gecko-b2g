...
static String8 matrixToString(const float* a, uint32_t m, uint32_t n,
                              bool rowMajor) {
  String8 str;
  str.append("[");
...
VelocityTracker::VelocityTracker(const char* strategy)
  ...
  // Allow the default strategy to be overridden using a system property for
  // debugging.
  if (!strategy) {
  ...
  if (!configureStrategy(strategy)) {
    ...
    if (!configureStrategy(DEFAULT_STRATEGY)) {
      LOG_ALWAYS_FATAL(
          "Could not create the default velocity tracker strategy '%s'!",
          strategy);
    }
  }
}

VelocityTracker::~VelocityTracker() { delete mStrategy; }
...
VelocityTrackerStrategy* VelocityTracker::createStrategy(const char* strategy) {
  ...
  if (!strcmp("int1", strategy)) {
    ...
    // underestimate the velocity of a fling but this strategy tends to respond
    // to changes in direction more quickly and accurately.
  ...
  if (!strcmp("legacy", strategy)) {
    ...
    // old data points, consistently underestimates velocity and takes a very
    // long time to adjust to changes in direction.
...
void VelocityTracker::clearPointers(BitSet32 idBits) {
  ...
  if (mActivePointerId >= 0 && idBits.hasBit(mActivePointerId)) {
    mActivePointerId =
        !remainingIdBits.isEmpty() ? remainingIdBits.firstMarkedBit() : -1;
  }
...
#if DEBUG_VELOCITY
  ALOGD(
      "VelocityTracker: addMovement eventTime=%lld, idBits=0x%08x, "
      "activePointerId=%d",
...
void VelocityTracker::addMovement(const MotionEvent* event) {
  ...
  switch (actionMasked) {
    ...
    default:
      ...
      // velocity of the pointers. Note that ACTION_UP and ACTION_POINTER_UP
      // always report the last known position of the pointers that went up.
      // ACTION_POINTER_UP does include the new position of pointers that
      // remained down but we will also receive an ACTION_MOVE with this
      // information if any of them actually moved.  Since we don't know how
      // many pointers will be going up at once it makes sense to just wait for
      // the following ACTION_MOVE before adding the movement.
      return;
  }
...
bool VelocityTracker::getVelocity(uint32_t id, float* outVx,
                                  float* outVy) const {
  Estimator estimator;
...
static bool solveLeastSquares(const float* x, const float* y, const float* w,
                              uint32_t m, uint32_t n, float* outB,
                              float* outDet) {
#if DEBUG_STRATEGY
...
#if DEBUG_STRATEGY
  ALOGD("  - a=%s",
        matrixToString(&a[0][0], m, n, false /*rowMajor*/).string());
#endif
...
#if DEBUG_STRATEGY
  ALOGD("  - q=%s",
        matrixToString(&q[0][0], m, n, false /*rowMajor*/).string());
  ...
  ALOGD("  - qr=%s",
        matrixToString(&qr[0][0], m, n, false /*rowMajor*/).string());
#endif
...
float LeastSquaresVelocityTrackerStrategy::chooseWeight(uint32_t index) const {
  switch (mWeighting) {
    case WEIGHTING_DELTA: {
      ...
      float deltaMillis =
          (mMovements[nextIndex].eventTime - mMovements[index].eventTime) *
          0.000001f;
      if (deltaMillis < 0) {
        return 0.5f;
      }
...
void IntegratingVelocityTrackerStrategy::clear() { mPointerIdBits.clear(); }
...
void IntegratingVelocityTrackerStrategy::populateEstimator(
    const State& state, VelocityTracker::Estimator* outEstimator) const {
...
LegacyVelocityTrackerStrategy::LegacyVelocityTrackerStrategy() { clear(); }
...
bool LegacyVelocityTrackerStrategy::getEstimator(
    uint32_t id, VelocityTracker::Estimator* outEstimator) const {
  outEstimator->clear();
  ...
  // Find the oldest sample that contains the pointer and that is not older than
  // HORIZON.
  nsecs_t minTime = newestMovement.eventTime - HORIZON;
  uint32_t oldestIndex = mIndex;
  uint32_t numTouches = 1;
  do {
    uint32_t nextOldestIndex =
        (oldestIndex == 0 ? HISTORY_SIZE : oldestIndex) - 1;
  ...
  // overestimate the velocity at that time point.  Most samples might be
  // measured 16ms apart but some consecutive samples could be only 0.5sm apart
  // because the hardware or driver reports them irregularly or in bursts.
  float accumVx = 0;
  float accumVy = 0;
  ...
  while (numTouches-- > 1) {
    ...
    // If the duration between samples is small, we may significantly
    // overestimate the velocity.  Consequently, we impose a minimum duration
    // constraint on the samples that we include in the calculation.
    if (duration >= MIN_DURATION) {
      ...
      accumVx =
          (accumVx * lastDuration + vx * duration) / (duration + lastDuration);
      accumVy =
          (accumVy * lastDuration + vy * duration) / (duration + lastDuration);
  ...
  // Report velocity.
  const VelocityTracker::Position& newestPosition =
      newestMovement.getPosition(id);
  outEstimator->time = newestMovement.eventTime;
  outEstimator->confidence = 1;
