...
struct SpriteTransformationMatrix {
  inline SpriteTransformationMatrix()
      : dsdx(1.0f), dtdx(0.0f), dsdy(0.0f), dtdy(1.0f) {}
  inline SpriteTransformationMatrix(float dsdx, float dtdx, float dsdy,
                                    float dtdy)
  ...
  inline bool operator==(const SpriteTransformationMatrix& other) {
    return dsdx == other.dsdx && dtdx == other.dtdx && dsdy == other.dsdy &&
           dtdy == other.dtdy;
  }
...
/*
 * A sprite is a simple graphical object that is displayed on-screen above other
 * layers. The basic sprite class is an interface. The implementation is
 * provided by the sprite controller.
 */
 ...
 *
 * This interface is used by PointerController and SpotController to draw
 * pointers or spot representations of fingers.  It is not intended for general
 * purpose use by other components.
 *
 * All sprite position updates and rendering is performed asynchronously.
 *
 * Clients are responsible for animating sprites by periodically updating their
 * properties.
 */
class SpriteController : public MessageHandler {
 ...
 public:
  ...
  /* Opens or closes a transaction to perform a batch of sprite updates as part
   * of a single operation such as setPosition and setAlpha.  It is not
   * necessary to open a transaction when updating a single property. Calls to
   * openTransaction() nest and must be matched by an equal number of calls to
   * closeTransaction(). */
 ...
 private:
   ...
   * surfaces can be resized and redrawn without blocking the client by holding
   * a lock on the sprites for a long time. Note that the SkBitmap holds a
   * reference to a shared (and immutable) pixel ref. */
  struct SpriteState {
    inline SpriteState()
        : dirty(0),
          visible(false),
   ...
   * Requests acquire a lock on the controller, update local state and request
   * the controller to invalidate the sprite. The real heavy lifting of
   * creating, resizing and redrawing surfaces happens asynchronously with no
   * locks held except in short critical section to copy the sprite state before
   * the work and update the sprite surface control afterwards.
   */
  class SpriteImpl : public Sprite {
   ...
   public:
    ...
    inline void setSurfaceLocked(const sp<SurfaceControl>& surfaceControl,
                                 int32_t width, int32_t height, bool drawn,
                                 bool visible) {
