...
status_t Tokenizer::open(const String8& filename, Tokenizer** outTokenizer) {
  *outTokenizer = NULL;
  ...
  if (fd < 0) {
    ...
    if (fstat(fd, &stat)) {
      result = -errno;
      ALOGE("Error getting size of file '%s', %s.", filename.string(),
            strerror(errno));
    } else {
      ...
      if (fileMap->create(NULL, fd, 0, length, true)) {
        ...
        if (nrd < 0) {
          result = -errno;
          ALOGE("Error reading file '%s', %s.", filename.string(),
                strerror(errno));
          delete[] buffer;
          buffer = NULL;
        } else {
...
status_t Tokenizer::fromContents(const String8& filename, const char* contents,
  ...
  *outTokenizer = new Tokenizer(filename, NULL, const_cast<char*>(contents),
