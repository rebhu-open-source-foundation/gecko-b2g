...
static void appendInputDeviceConfigurationFileRelativePath(
    String8& path, const String8& name, InputDeviceConfigurationFileType type) {
...
String8 getInputDeviceConfigurationFilePathByDeviceIdentifier(
  ...
  if (deviceIdentifier.vendor != 0 && deviceIdentifier.product != 0) {
    ...
    String8 productPath(getInputDeviceConfigurationFilePathByName(
        String8::format("Vendor_%04x_Product_%04x", deviceIdentifier.vendor,
...
String8 getInputDeviceConfigurationFilePathByName(
  ...
  appendInputDeviceConfigurationFileRelativePath(path, name, type);
#if DEBUG_PROBE
  ALOGD("Probing for system user input device configuration file: path='%s'",
        path.string());
#endif
  if (!access(path.string(), R_OK)) {
#if DEBUG_PROBE
...
#if DEBUG_PROBE
  ALOGD(
      "Probe failed to find input device configuration file: name='%s', "
      "type=%d",
      name.string(), type);
#endif
  return String8();
...
void InputDeviceInfo::addMotionRange(int32_t axis, uint32_t source, float min,
