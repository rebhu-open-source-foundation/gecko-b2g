...
static const nsecs_t INACTIVITY_TIMEOUT_DELAY_TIME_NORMAL =
    15 * 1000 * 1000000LL;  // 15 seconds
static const nsecs_t INACTIVITY_TIMEOUT_DELAY_TIME_SHORT =
    3 * 1000 * 1000000LL;  // 3 seconds
...
void PointerController::setDisplayViewport(int32_t width, int32_t height,
  ...
  if (mLocked.displayOrientation != orientation) {
    // Apply offsets to convert from the pixel top-left corner position to the
    // pixel center. This creates an invariant frame of reference that we can
    // easily rotate when taking into account that the pointer may be located at
    ...
    // Apply offsets to convert from the pixel center to the pixel top-left
    // corner position and save the results.
...
void PointerController::doAnimate() {
  ...
  mLocked.animationPending = false;
  nsecs_t frameDelay =
      systemTime(SYSTEM_TIME_MONOTONIC) - mLocked.animationTime;
...
void PointerController::doInactivityTimeout() { fade(TRANSITION_GRADUAL); }

void PointerController::startAnimationLocked() {
  if (!mLocked.animationPending) {
    mLocked.animationPending = true;
    mLocked.animationTime = systemTime(SYSTEM_TIME_MONOTONIC);
    mLooper->sendMessageDelayed(ANIMATION_FRAME_INTERVAL, mHandler,
                                Message(MSG_ANIMATE));
  }
}
...
PointerController::Spot* PointerController::createAndAddSpotLocked(
    uint32_t id) {
...
void PointerController::Spot::updateSprite(const SpriteIcon* icon, float x,
                                           float y) {
  sprite->setLayer(Sprite::BASE_LAYER_SPOT + id);
  sprite->setAlpha(alpha);
  sprite->setTransformationMatrix(
      SpriteTransformationMatrix(scale, 0.0f, 0.0f, scale));
