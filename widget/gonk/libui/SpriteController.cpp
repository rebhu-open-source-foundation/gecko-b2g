...
sp<Sprite> SpriteController::createSprite() { return new SpriteImpl(this); }
...
void SpriteController::closeTransaction() {
  ...
  LOG_ALWAYS_FATAL_IF(mLocked.transactionNestingCount == 0,
                      "Sprite closeTransaction() called but there is no open "
                      "sprite transaction");
...
#ifdef HAVE_ANDROID_OS
void SpriteController::disposeSurfaceLocked(
...
void SpriteController::doUpdateSprites() {
  ...
  // Each sprite update record includes a reference to its associated sprite so
  // we can be certain the sprites will not be deleted while this function runs.
  // Sprites may invalidate themselves again during this time but we will handle
  // those changes in the next iteration.
  ...
  for (size_t i = 0; i < numSprites; i++) {
    ...
    if (update.state.surfaceControl != NULL &&
      ...
      if (update.state.surfaceWidth < desiredWidth ||
        ...
        status_t status =
            update.state.surfaceControl->setSize(desiredWidth, desiredHeight);
        if (status) {
          ALOGE("Error %d resizing sprite surface from %dx%d to %dx%d", status,
  ...
  for (size_t i = 0; i < numSprites; i++) {
    ...
    if (update.state.surfaceControl != NULL && !update.state.surfaceDrawn &&
      ...
      if (status) {
        ...
        surfaceBitmap.setConfig(SkBitmap::kARGB_8888_Config, outBuffer.width,
                                outBuffer.height, bpr);
        ...
        if (outBuffer.width > uint32_t(update.state.icon.bitmap.width())) {
          ...
          surfaceCanvas.drawRectCoords(
              update.state.icon.bitmap.width(), 0, outBuffer.width,
              update.state.icon.bitmap.height(), paint);
        }
        ...
        if (status) {
          ALOGE("Error %d unlocking and posting sprite surface after drawing.",
                status);
        } else {
  ...
  for (size_t i = 0; i < numSprites; i++) {
    SpriteUpdate& update = updates.editItemAt(i);
    bool wantSurfaceVisibleAndDrawn =
        update.state.wantSurfaceVisible() && update.state.surfaceDrawn;
    bool becomingVisible =
        wantSurfaceVisibleAndDrawn && !update.state.surfaceVisible;
    bool becomingHidden =
        !wantSurfaceVisibleAndDrawn && update.state.surfaceVisible;
    if (update.state.surfaceControl != NULL &&
        (becomingVisible || becomingHidden ||
         (wantSurfaceVisibleAndDrawn &&
          (update.state.dirty &
           (DIRTY_ALPHA | DIRTY_POSITION | DIRTY_TRANSFORMATION_MATRIX |
            DIRTY_LAYER | DIRTY_VISIBILITY | DIRTY_HOTSPOT))))) {
      status_t status;
      if (!haveTransaction) {
        SurfaceComposerClient::openGlobalTransaction();
      ...
      if (wantSurfaceVisibleAndDrawn &&
          (becomingVisible ||
           (update.state.dirty & (DIRTY_POSITION | DIRTY_HOTSPOT)))) {
      ...
      if (wantSurfaceVisibleAndDrawn &&
        ...
        if (status) {
          ALOGE("Error %d setting sprite surface transformation matrix.",
                status);
        }
      }
...
#ifdef HAVE_ANDROID_OS
  // If any surfaces were changed, write back the new surface properties to the
  // sprites.
  if (surfaceChanged) {  // acquire lock
    ...
    for (size_t i = 0; i < numSprites; i++) {
      ...
      if (update.surfaceChanged) {
        update.sprite->setSurfaceLocked(
            update.state.surfaceControl, update.state.surfaceWidth,
            update.state.surfaceHeight, update.state.surfaceDrawn,
            update.state.surfaceVisible);
      }
    }
  }  // release lock
#endif
  ...
  // we do not clear sprite references inside the lock since we could be
  // releasing the last remaining reference to the sprite here which would
  // result in the sprite being deleted and the lock being reacquired by the
  // sprite destructor while already held.
  updates.clear();
}
...
#ifdef HAVE_ANDROID_OS
sp<SurfaceControl> SpriteController::obtainSurface(int32_t width,
                                                   int32_t height) {
