...
status_t VirtualKeyMap::load(const String8& filename, VirtualKeyMap** outMap) {
  *outMap = NULL;
  ...
  if (status) {
    ALOGE("Error %d opening virtual key map file %s.", status,
          filename.string());
  } else {
...
status_t VirtualKeyMap::Parser::parse() {
  while (!mTokenizer->isEof()) {
    ...
    if (!mTokenizer->isEol() && mTokenizer->peekChar() != '#') {
      // Multiple keys can appear on one line or they can be broken up across
      // multiple lines.
      do {
        ...
        if (!success) {
          ALOGE(
              "%s: Expected 5 colon-delimited integers in virtual key "
              "definition.",
              mTokenizer->getLocation().string());
          return BAD_VALUE;
        }
...
#if DEBUG_PARSER
        ALOGD(
            "Parsed virtual key: scanCode=%d, centerX=%d, centerY=%d, "
            "width=%d, height=%d",
