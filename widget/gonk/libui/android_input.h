...
enum {
  ...
  /* The key is down but is a virtual key press that is being emulated by the
     system. */
...
enum {
  ...
  /* Multiple duplicate key events have occurred in a row, or a complex string
   * is being delivered.  The repeat_count property of the key event contains
   * the number of times the given key code should be executed.
   */
...
enum {
   ...
   * actual pointer index where the data for the pointer going up or down can be
   * found.
   */
   ...
   * The bits in AMOTION_EVENT_ACTION_POINTER_INDEX_MASK indicate which pointer
   * changed.
   */
   ...
   * well as any intermediate points since the last hover move event.
   */
  AMOTION_EVENT_ACTION_HOVER_MOVE = 7,

  /* The motion event contains relative vertical and/or horizontal scroll
   * offsets. Use getAxisValue to retrieve the information from
...
enum {
   ...
   * situations in which a malicious application may have covered up part of its
   * content for the purpose of misleading the user or hijacking touches.  An
   * appropriate response might be to drop the suspect touches or to take
 ...
 * Refer to the documentation on the MotionEvent class for descriptions of each
 * axis.
 */
enum {
  ...
  // NOTE: If you add a new axis here you must also add it to several other
  // files.
  //       Refer to frameworks/base/core/java/android/view/MotionEvent.java for
  //       the full list.
};
 ...
 * Input sources.
 *
 * Refer to the documentation on android.view.InputDevice for more details about
 * input sources and their correct interpretation.
 */
enum {
...
/*
 * Constants used to retrieve information about the range of motion for a
 * particular coordinate of a motion event.
 *
 * Refer to the documentation on android.view.InputDevice for more details about
 * input sources and their correct interpretation.
 *
 * DEPRECATION NOTICE: These constants are deprecated.  Use AMOTION_EVENT_AXIS_*
 * constants instead.
 */
enum {
 ...
 *
 * Note that most functions can only be used on input events that are of a given
 * type. Calling these functions on input events of other types will yield
 * undefined behavior.
 */
...
/* Get a bitfield indicating which edges, if any, were touched by this motion
 * event. For touch events, clients can use this to determine if the user's
 ...
 * screen coordinates to adjust for the absolute position of the containing
 * windows and views. */
 ...
 * pressure), although values higher than 1 may be generated depending on the
 * calibration of the input device. */
float AMotionEvent_getPressure(const AInputEvent* motion_event,
                               size_t pointer_index);

/* Get the current scaled value of the approximate size for the given pointer
 * index. This represents some approximation of the area of the screen being
...
/* Get the current length of the major axis of an ellipse that describes the
 * touch area at the point of contact for the given pointer index. */
float AMotionEvent_getTouchMajor(const AInputEvent* motion_event,
                                 size_t pointer_index);

/* Get the current length of the minor axis of an ellipse that describes the
 * touch area at the point of contact for the given pointer index. */
float AMotionEvent_getTouchMinor(const AInputEvent* motion_event,
                                 size_t pointer_index);

/* Get the current length of the major axis of an ellipse that describes the
 * size of the approaching tool for the given pointer index. The tool area
...
/* Get the current orientation of the touch area and tool area in radians
 * clockwise from vertical for the given pointer index. An angle of 0 degrees
 ...
 * to AMOTION_EVENT_ACTION_MOVE events -- all other actions will have a size of
 * 0. Historical samples are indexed from oldest to newest. */
 ...
 * that occurred between this event and the previous motion event. For touch
 * events on the screen, this is the original location of the event on the
 * screen, before it had been adjusted for the containing window and views.
 * Whole numbers are pixels; the value may have a fraction for input devices
 * that are sub-pixel precise. */
 ...
 * that occurred between this event and the previous motion event. Whole numbers
 * are pixels; the value may have a fraction for input devices that are
 * sub-pixel precise. */
float AMotionEvent_getHistoricalX(AInputEvent* motion_event,
 ...
 * are pixels; the value may have a fraction for input devices that are
 * sub-pixel precise. */
float AMotionEvent_getHistoricalY(AInputEvent* motion_event,
                                  size_t pointer_index, size_t history_index);
 ...
 * pressure), although values higher than 1 may be generated depending on the
 * calibration of the input device. */
float AMotionEvent_getHistoricalPressure(AInputEvent* motion_event,
                                         size_t pointer_index,
                                         size_t history_index);

/* Get the current scaled value of the approximate size for the given pointer
 * index that occurred between this event and the previous motion event. This
 * represents some approximation of the area of the screen being pressed; the
 * actual value in pixels corresponding to the touch is normalized with the
 * device specific range of values and scaled to a value between 0 and 1.  The
 * value of size can be used to determine fat touch events. */
float AMotionEvent_getHistoricalSize(AInputEvent* motion_event,
                                     size_t pointer_index,
                                     size_t history_index);

/* Get the historical length of the major axis of an ellipse that describes the
 * touch area at the point of contact for the given pointer index that
 * occurred between this event and the previous motion event. */
...
/* Get the historical length of the major axis of an ellipse that describes the
 * size of the approaching tool for the given pointer index that occurred
 * between this event and the previous motion event. The tool area represents
 * the estimated size of the finger or pen that is touching the device
 * independent of its actual touch area at the point of contact. */
...
/* Get the historical orientation of the touch area and tool area in radians
 * clockwise from vertical for the given pointer index that occurred between
 * this event and the previous motion event. An angle of 0 degrees indicates
