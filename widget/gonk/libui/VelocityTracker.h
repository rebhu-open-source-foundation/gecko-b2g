...
class VelocityTracker {
 public:
  ...
  // position information for each pointer in order by increasing id.  Its size
  // should be equal to the number of one bits in idBits.
...
class VelocityTrackerStrategy {
 ...
 public:
  ...
  virtual bool getEstimator(uint32_t id,
                            VelocityTracker::Estimator* outEstimator) const = 0;
};
...
class IntegratingVelocityTrackerStrategy : public VelocityTrackerStrategy {
 ...
 private:
  ...
  void updateState(State& state, nsecs_t eventTime, float xpos,
                   float ypos) const;
  void populateEstimator(const State& state,
                         VelocityTracker::Estimator* outEstimator) const;
};
