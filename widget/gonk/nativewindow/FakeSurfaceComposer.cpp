...
void FakeSurfaceComposer::instantiate() {
  defaultServiceManager()->addService(String16("SurfaceFlinger"),
                                      new FakeSurfaceComposer());
}

FakeSurfaceComposer::FakeSurfaceComposer() : BnSurfaceComposer() {}
...
class DestroyDisplayRunnable : public mozilla::Runnable {
 public:
  DestroyDisplayRunnable(FakeSurfaceComposer* aComposer, ssize_t aIndex)
      : mozilla::Runnable("DestroyDisplayRunnable"),
        mComposer(aComposer),
        mIndex(aIndex) {}
    ...
    virtual ~DisplayToken() {
      ...
      if (idx >= 0) {
        nsCOMPtr<nsIRunnable> task(
            new DestroyDisplayRunnable(composer.get(), idx));
...
uint32_t FakeSurfaceComposer::setDisplayStateLocked(const DisplayState& s) {
  ...
  if (what & DisplayState::eSurfaceChanged) {
    if (disp.surface->asBinder(disp.surface) !=
        s.surface->asBinder(s.surface)) {
  ...
  if (what & DisplayState::eSurfaceChanged) {
    nsCOMPtr<nsIRunnable> runnable =
        NS_NewRunnableFunction("GonkDisplay", [&]() {
          MOZ_ASSERT(NS_IsMainThread(), "Must be on main thread.");
          ScreenHelperGonk* screenHelper = ScreenHelperGonk::GetSingleton();
          screenHelper->AddScreen(DisplayType::DISPLAY_VIRTUAL,
                                  disp.surface.get());
        });
...
FakeSurfaceComposer::createDisplayEventConnection() {
  return nullptr;
}
...
status_t FakeSurfaceComposer::captureScreen(
    const sp<IBinder>& display, const sp<IGraphicBufferProducer>& producer,
    Rect sourceCrop, uint32_t reqWidth, uint32_t reqHeight, uint32_t minLayerZ,
    uint32_t maxLayerZ, bool useIdentityTransform, Rotation rotation) {
  ...
  sp<IGraphicBufferProducer> fakeProducer =
      IGraphicBufferProducer::asInterface(wrapper);
...
void FakeSurfaceComposer::captureScreenImp(
    const sp<IGraphicBufferProducer>& producer, uint32_t reqWidth,
    uint32_t reqHeight, const sp<GraphicProducerWrapper>& wrapper) {
  ...
  if (reqWidth > hw_w || reqHeight > hw_h) {
    ALOGE("size mismatch (%d, %d) > (%d, %d)", reqWidth, reqHeight, hw_w, hw_h);
    static_cast<GraphicProducerWrapper*>(producer->asBinder(producer).get())
        ->exit(BAD_VALUE);
    return;
  }
  ...
  nsCOMPtr<nsIRunnable> runnable = NS_NewRunnableFunction(
      "CaptureScreen", [screen, reqWidth, reqHeight, producer, wrapper]() {
        ...
        if (native_window_api_connect(window, NATIVE_WINDOW_API_EGL) !=
            NO_ERROR) {
          static_cast<GraphicProducerWrapper*>(
              producer->asBinder(producer).get())
              ->exit(BAD_VALUE);
...
void FakeSurfaceComposer::setPowerMode(const sp<IBinder>& display, int mode) {}

status_t FakeSurfaceComposer::getDisplayConfigs(const sp<IBinder>& display,
  ...
  nsCOMPtr<nsIRunnable> runnable = NS_NewRunnableFunction("DisplayInfo", [&]() {
    MOZ_ASSERT(NS_IsMainThread());
...
status_t FakeSurfaceComposer::getDisplayStats(const sp<IBinder>& display,
                                              DisplayStatInfo* stats) {
  return INVALID_OPERATION;
}

int FakeSurfaceComposer::getActiveConfig(const sp<IBinder>& display) {
...
status_t FakeSurfaceComposer::setActiveConfig(const sp<IBinder>& display,
                                              int id) {
  return INVALID_OPERATION;
}

status_t FakeSurfaceComposer::clearAnimationFrameStats() {
  return INVALID_OPERATION;
}

status_t FakeSurfaceComposer::getAnimationFrameStats(
...
status_t FakeSurfaceComposer::getDisplayInfo(const sp<IBinder>& display,
                                             DisplayInfo* info) {
  if (info == NULL) {
    return BAD_VALUE;
  }
...
void FakeSurfaceComposer::getPrimaryDisplayInfo(DisplayInfo* info) {
  MOZ_ASSERT(NS_IsMainThread(), "Must be on main thread.");
