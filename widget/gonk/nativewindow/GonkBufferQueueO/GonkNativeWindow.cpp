...
status_t GonkNativeWindow::acquireBuffer(BufferItem* item, nsecs_t presentWhen,
                                         bool waitForFence) {
  status_t err;
...
status_t GonkNativeWindow::releaseBuffer(const BufferItem& item,
  ...
  if (err != OK) {
    ALOGE("Failed to release buffer: %s (%d)", strerror(-err), err);
  }
  return err;
}
...
already_AddRefed<TextureClient> GonkNativeWindow::getCurrentBuffer() {
  Mutex::Autolock _l(mMutex);
  BufferItem item;
...
/* static */ void GonkNativeWindow::RecycleCallback(TextureClient* client,
                                                    void* closure) {
  GonkNativeWindow* nativeWindow = static_cast<GonkNativeWindow*>(closure);
...
void GonkNativeWindow::returnBuffer(TextureClient* client) {
  ...
  FenceHandle handle =
      client->GetInternalData()->GetAndResetReleaseFenceHandle();
...
already_AddRefed<TextureClient> GonkNativeWindow::getTextureClientFromBuffer(
