...
status_t GonkBufferQueueProducer::setMaxDequeuedBufferCount(
    ...
    { // Autolock scope
        ...
        if (mCore->mIsAbandoned) {
            ALOGE("setMaxDequeuedBufferCount: BufferQueue has been "
                    "abandoned");
            return NO_INIT;
        }
...
status_t GonkBufferQueueProducer::dequeueBuffer(int* outSlot, sp<android::Fence>* outFence,
    ...
    ALOGV("dequeueBuffer: w=%u h=%u format=%#x, usage=%#" PRIx64, width, height, format, usage);
    ...
    *outSlot = GonkBufferQueueCore::INVALID_BUFFER_SLOT;
    ...
    { // Autolock scope
        ...
        int found;
        status_t status = waitForFreeSlotThenRelock("dequeueBuffer", false,
                &found, &returnFlags);
        if (status != NO_ERROR) {
            return status;
        }
...
status_t GonkBufferQueueProducer::cancelBuffer(int slot, const sp<Fence>& fence) {
    ATRACE_CALL();
...
void GonkBufferQueueProducer::allocateBuffers(uint32_t width,
...
status_t GonkBufferQueueProducer::getLastQueuedBuffer(sp<GraphicBuffer>* outBuffer,
        sp<Fence>* outFence, float outTransformMatrix[16]) {
    return INVALID_OPERATION;
}

void GonkBufferQueueProducer::getFrameTimestamps(FrameEventHistoryDelta* outDelta) {
}
