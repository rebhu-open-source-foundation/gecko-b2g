...
size_t IGonkGraphicBufferConsumer::BufferItem::getPodSize() const {
  size_t c = sizeof(mCrop) + sizeof(mTransform) + sizeof(mScalingMode) +
             sizeof(mTimestamp) + sizeof(mIsAutoTimestamp) +
             sizeof(mFrameNumber) + sizeof(mBuf) + sizeof(mIsDroppable) +
             sizeof(mAcquireCalled) + sizeof(mTransformToDisplayInverse);
  return c;
}
...
status_t IGonkGraphicBufferConsumer::BufferItem::flatten(void*& buffer,
                                                         size_t& size,
                                                         int*& fds,
                                                         size_t& count) const {
  ...
  // check we have enough space (in case flattening the fence/graphicbuffer lied
  // to us)
  if (size < getPodSize()) {
    return NO_MEMORY;
  }
...
status_t IGonkGraphicBufferConsumer::BufferItem::unflatten(void const*& buffer,
                                                           size_t& size,
                                                           int const*& fds,
                                                           size_t& count) {
  if (size < sizeof(uint32_t)) return NO_MEMORY;
...
class BpGonkGraphicBufferConsumer
 ...
 public:
  ...
  virtual status_t setDefaultMaxBufferCount(int bufferCount) {
    ...
    status_t result =
        remote()->transact(SET_DEFAULT_MAX_BUFFER_COUNT, data, &reply);
    if (result != NO_ERROR) {
      return result;
    }
    return reply.readInt32();
  ...
  virtual status_t setMaxAcquiredBufferCount(int maxAcquiredBuffers) {
    ...
    status_t result =
        remote()->transact(SET_MAX_ACQUIRED_BUFFER_COUNT, data, &reply);
    if (result != NO_ERROR) {
      return result;
    }
    return reply.readInt32();
  ...
  virtual status_t setDefaultBufferFormat(uint32_t defaultFormat) {
    ...
    status_t result =
        remote()->transact(SET_DEFAULT_BUFFER_FORMAT, data, &reply);
    if (result != NO_ERROR) {
      return result;
    }
    return reply.readInt32();
  ...
  virtual sp<NativeHandle> getSidebandStream() const {
    ...
    data.writeInterfaceToken(
        IGonkGraphicBufferConsumer::getInterfaceDescriptor());
    if ((err = remote()->transact(GET_SIDEBAND_STREAM, data, &reply)) !=
        NO_ERROR) {
      return NULL;
    }
...
status_t BnGonkGraphicBufferConsumer::onTransact(uint32_t code,
                                                 const Parcel& data,
                                                 Parcel* reply,
                                                 uint32_t flags) {
  switch (code) {
    ...
    case DUMP: {
      ...
      static_cast<IGonkGraphicBufferConsumer*>(this)->dumpToString(result,
                                                                   prefix);
