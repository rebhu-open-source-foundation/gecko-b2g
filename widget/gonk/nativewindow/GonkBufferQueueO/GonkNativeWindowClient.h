 ...
 * GonkNativeWindowClient.  GonkNativeWindowClient then forwards the buffers
 * through Binder IPC to the GonkBufferQueue's producer interface, providing the
 * new frame to a consumer such as GLConsumer.
 */
class GonkNativeWindowClient
    : public ANativeObjectBase<ANativeWindow, GonkNativeWindowClient, RefBase> {
 public:
  /*
   * creates a GonkNativeWindowClient from the given IGraphicBufferProducer
   * (which concrete implementation is a GonkBufferQueue).
   *
   * GonkNativeWindowClient is mainly state-less while it's disconnected, it can
   * be viewed as a glorified IGraphicBufferProducer holder. It's therefore safe
   * to create other GonkNativeWindowClients from the same
   * IGraphicBufferProducer.
   *
   * However, once a GonkNativeWindowClient is connected, it'll prevent other
   * GonkNativeWindowClients referring to the same IGraphicBufferProducer to
   * become connected and therefore prevent them to be used as actual producers
   * of buffers.
   *
   * the controlledByApp flag indicates that this GonkNativeWindowClient
   * (producer) is controlled by the application. This flag is used at connect
   * time.
   */
  explicit GonkNativeWindowClient(
  ...
  /* Attaches a sideband buffer stream to the GonkNativeWindowClient's
   * IGraphicBufferProducer.
   *
 ...
 private:
  ...
  // IGraphicBufferProducer::requestBuffer when the client dequeues a buffer
  // from a slot that has not yet been used. The buffer allocated to a slot will
  // also be replaced if the requested buffer usage or geometry differs from
  // that of the buffer allocated to a slot.
