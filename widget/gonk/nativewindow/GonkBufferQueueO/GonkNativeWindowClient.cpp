...
sp<IGraphicBufferProducer> GonkNativeWindowClient::getIGraphicBufferProducer()
    const {
...
void GonkNativeWindowClient::allocateBuffers() {
  ...
  mGraphicBufferProducer->allocateBuffers(reqWidth, reqHeight, mReqFormat,
                                          mReqUsage);
}

int GonkNativeWindowClient::hook_setSwapInterval(ANativeWindow* window,
                                                 int interval) {
...
int GonkNativeWindowClient::hook_dequeueBuffer_DEPRECATED(
...
int GonkNativeWindowClient::hook_cancelBuffer_DEPRECATED(
    ANativeWindow* window, ANativeWindowBuffer* buffer) {
  GonkNativeWindowClient* c = getSelf(window);
  return c->cancelBuffer(buffer, -1);
}

int GonkNativeWindowClient::hook_lockBuffer_DEPRECATED(
    ANativeWindow* window, ANativeWindowBuffer* buffer) {
  GonkNativeWindowClient* c = getSelf(window);
  return c->lockBuffer_DEPRECATED(buffer);
}

int GonkNativeWindowClient::hook_queueBuffer_DEPRECATED(
    ANativeWindow* window, ANativeWindowBuffer* buffer) {
  GonkNativeWindowClient* c = getSelf(window);
  return c->queueBuffer(buffer, -1);
}

int GonkNativeWindowClient::hook_query(const ANativeWindow* window, int what,
                                       int* value) {
...
int GonkNativeWindowClient::dequeueBuffer(android_native_buffer_t** buffer,
...

  {
    ...
    reqUsage = mReqUsage;
  }  // Drop the lock so that we can still touch the GonkNativeWindowClient
     // while blocking in IGBP::dequeueBuffer

  int buf = -1;
  sp<Fence> fence;
  uint64_t bufferAge = 0;
  status_t result = mGraphicBufferProducer->dequeueBuffer(
      &buf, &fence, reqW, reqH, reqFormat, reqUsage, &bufferAge, nullptr);

  if (result < 0) {
    ALOGV(
        "dequeueBuffer: IGraphicBufferProducer::dequeueBuffer(%d, %d, %d, %d)"
        "failed: %d",
        reqW, reqH, reqFormat, reqUsage, result);
    return result;
  }
  ...
  ALOGE_IF(fence == NULL,
           "GonkNativeWindowClient::dequeueBuffer: received null Fence! buf=%d",
           buf);
...
int GonkNativeWindowClient::lockBuffer_DEPRECATED(
    android_native_buffer_t* buffer __attribute__((unused))) {
...
int GonkNativeWindowClient::queueBuffer(android_native_buffer_t* buffer,
  ...
  IGraphicBufferProducer::QueueBufferInput input(
      timestamp, isAutoTimestamp, HAL_DATASPACE_UNKNOWN, crop, mScalingMode,
...
int GonkNativeWindowClient::setCrop(Rect const* rect) {
  ...
  ALOGV("GonkNativeWindowClient::setCrop rect=[%d %d %d %d]", realRect.left,
        realRect.top, realRect.right, realRect.bottom);
...
int GonkNativeWindowClient::setBuffersDimensions(int w, int h) {
  ...
  if (w < 0 || h < 0) return BAD_VALUE;

  if ((w && !h) || (!w && h)) return BAD_VALUE;
...
status_t GonkNativeWindowClient::lock(ANativeWindow_Buffer* outBuffer,
                                      ARect* inOutDirtyBounds) {
  return INVALID_OPERATION;
}

status_t GonkNativeWindowClient::unlockAndPost() { return INVALID_OPERATION; }
