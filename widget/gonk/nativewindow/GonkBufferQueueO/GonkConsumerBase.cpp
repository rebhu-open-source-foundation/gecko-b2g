...
GonkConsumerBase::GonkConsumerBase(
    const sp<IGonkGraphicBufferConsumer>& bufferQueue, bool controlledByApp)
    : mAbandoned(false), mConsumer(bufferQueue) {
  ...
  sp<IConsumerListener> proxy =
      new GonkBufferQueue::ProxyConsumerListener(listener);
...
GonkConsumerBase::~GonkConsumerBase() {
  ...
  LOG_ALWAYS_FATAL_IF(!mAbandoned,
                      "[%s] ~GonkConsumerBase was called, but the "
...
void GonkConsumerBase::dump(String8& result) const { dump(result, ""); }
...
status_t GonkConsumerBase::acquireBufferLocked(
    GonkBufferQueue::BufferItem* item, nsecs_t presentWhen) {
  ...
  ALOGV("acquireBufferLocked: -> slot=%d/%" PRIu64, item->mBuf,
        item->mFrameNumber);

  return OK;
}

status_t GonkConsumerBase::addReleaseFence(
    int slot, const sp<GraphicBuffer> graphicBuffer, const sp<Fence>& fence) {
  Mutex::Autolock lock(mMutex);
  return addReleaseFenceLocked(slot, graphicBuffer, fence);
}

status_t GonkConsumerBase::addReleaseFenceLocked(
    int slot, const sp<GraphicBuffer> graphicBuffer, const sp<Fence>& fence) {
  ...
  if (!mSlots[slot].mFence.get()) {
    ...
    sp<Fence> mergedFence =
        Fence::merge(String8::format("%.28s:%d", mName.string(), slot),
                     mSlots[slot].mFence, fence);
    if (!mergedFence.get()) {
...
status_t GonkConsumerBase::releaseBufferLocked(
  ...
  ALOGV("releaseBufferLocked: slot=%d/%" PRIu64, slot,
