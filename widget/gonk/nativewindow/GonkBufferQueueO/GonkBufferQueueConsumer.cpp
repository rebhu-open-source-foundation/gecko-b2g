...
GonkBufferQueueConsumer::GonkBufferQueueConsumer(
    const sp<GonkBufferQueueCore>& core)
...
status_t GonkBufferQueueConsumer::acquireBuffer(BufferItem* outBuffer,
  ...
  if (expectedPresent != 0) {
    ...
    while (mCore->mQueue.size() > 1 && !mCore->mQueue[0].mIsAutoTimestamp) {
      ...
      if (desiredPresent < expectedPresent - MAX_REASONABLE_NSEC ||
        ...
        ALOGV("acquireBuffer: nodrop desire=%" PRId64 " expect=%" PRId64
              " (%" PRId64 ") now=%" PRId64,
  ...
  ALOGV("acquireBuffer: acquiring { slot=%d/%" PRIu64 " buffer=%p }", slot,
        front->mFrameNumber, front->mGraphicBuffer->handle);
...
status_t GonkBufferQueueConsumer::detachBuffer(int slot) {
  ...
  if (slot < 0 || slot >= GonkBufferQueueDefs::NUM_BUFFER_SLOTS) {
    ALOGE("detachBuffer(C): slot index %d out of range [0, %d)", slot,
          GonkBufferQueueDefs::NUM_BUFFER_SLOTS);
    return BAD_VALUE;
  } else if (mSlots[slot].mBufferState != GonkBufferSlot::ACQUIRED) {
    ALOGE(
        "detachBuffer(C): slot %d is not owned by the consumer "
        "(state = %d)",
...
status_t GonkBufferQueueConsumer::attachBuffer(
  ...
  if (numAcquiredBuffers >= mCore->mMaxAcquiredBufferCount + 1) {
    ALOGE(
        "attachBuffer(P): max acquired buffer count reached: %d "
        "(max %d)",
        numAcquiredBuffers, mCore->mMaxAcquiredBufferCount);
    return INVALID_OPERATION;
  }
  ...
  {  // Autolock scope
    ...
    while (current != mCore->mQueue.end()) {
      if (current->mSlot == slot) {
        ALOGE(
            "releaseBuffer: buffer slot %d pending release is "
            "currently queued",
            slot);
        return BAD_VALUE;
      }
    ...
    if (mSlots[slot].mBufferState == GonkBufferSlot::ACQUIRED) {
      ...
      ALOGV(
          "releaseBuffer: releasing a stale buffer slot %d "
          "(state = %d)",
          slot, mSlots[slot].mBufferState);
      mSlots[slot].mNeedsCleanupOnRelease = false;
      return STALE_BUFFER_SLOT;
      ...
      ALOGV(
          "releaseBuffer: attempted to release buffer slot %d "
          "but its state was %d",
          slot, mSlots[slot].mBufferState);
      return BAD_VALUE;
    }
...
status_t GonkBufferQueueConsumer::connect(
  ...
  ALOGV("connect(C): controlledByApp=%s", controlledByApp ? "true" : "false");
...
status_t GonkBufferQueueConsumer::setDefaultBufferSize(uint32_t width,
  ...
  if (width == 0 || height == 0) {
    ALOGV(
        "setDefaultBufferSize: dimensions cannot be 0 (width=%u "
        "height=%u)",
        width, height);
    return BAD_VALUE;
  }
...
status_t GonkBufferQueueConsumer::setMaxAcquiredBufferCount(
  ...
  if (maxAcquiredBuffers < 1 ||
    ...
    ALOGE("setMaxAcquiredBufferCount: invalid count %d", maxAcquiredBuffers);
    return BAD_VALUE;
  }
...
status_t GonkBufferQueueConsumer::setDefaultBufferFormat(
    uint32_t defaultFormat) {
  ATRACE_CALL();
...
GonkBufferQueueConsumer::getTextureClientFromBuffer(
  ...
  for (int i = 0; i < GonkBufferQueueDefs::NUM_BUFFER_SLOTS; i++) {
    if (mSlots[i].mGraphicBuffer != NULL &&
        mSlots[i].mGraphicBuffer->handle == buffer->handle) {
...
int GonkBufferQueueConsumer::getSlotFromTextureClientLocked(
