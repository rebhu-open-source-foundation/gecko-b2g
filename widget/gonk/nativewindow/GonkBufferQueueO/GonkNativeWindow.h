 ...
 * access to the whole BufferItem entry from GonkBufferQueue. Multiple buffers
 * may be acquired at once, to be used concurrently by the client. This consumer
 * can operate either in synchronous or asynchronous mode.
 */
class GonkNativeWindow : public GonkConsumerBase {
 ...
 public:
  ...
  //
  // If waitForFence is true, and the acquired BufferItem has a valid fence
  // object, acquireBuffer will wait on the fence with no timeout before
  // returning.
