...
Display::Display(android::Hwc2::Composer& composer,
    ...
    : mComposer(composer),
      ...
      mIsConnected(false),
      mType(type) {
  ALOGD("Created display %" PRIu64, id);
}
...
Error Display::getActiveConfig(
  ...
  ALOGD("[%" PRIu64 "] getActiveConfig", mId);
...
Error Display::getActiveConfigIndex(int* outIndex) const {
  ALOGD("[%" PRIu64 "] getActiveConfigIndex", mId);
  hwc2_config_t configId = 0;
  auto intError = mComposer.getActiveConfig(mId, &configId);
  auto error = static_cast<Error>(intError);
...
void Display::loadConfig(hwc2_config_t configId) {
  ALOGD("[%" PRIu64 "] loadConfig(%u)", mId, configId);
...
Layer::Layer(android::Hwc2::Composer& composer,
  ...
  ALOGD("Created layer %" PRIu64 " on display %" PRIu64, layerId, displayId);
}
