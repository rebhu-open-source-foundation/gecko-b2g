...
Hwc2TestCoverage getCoverage(
    Hwc2TestPropertyName property, Hwc2TestCoverage coverage,
...
int Hwc2TestLayer::getBuffer(int& outSlot,
                             android::sp<android::GraphicBuffer>& outBuffer,
                             android::sp<android::Fence>& outAcquireFence) {
  return mBuffer.get(outSlot, outBuffer, outAcquireFence);
}

void Hwc2TestLayer::setZOrder(uint32_t zOrder) { mZOrder = zOrder; }
...
bool Hwc2TestLayer::advance() {
  for (auto property : mProperties) {
    if (property->isSupported(mComposition.get())) {
      if (property->advance()) {
        // printf("advance %s", property->dump().c_str());
        return true;
      }
    }
  }
...
HWC2::BlendMode Hwc2TestLayer::getBlendMode() const { return mBlendMode.get(); }

Area Hwc2TestLayer::getBufferArea() const { return mBufferArea.get(); }

hwc_color_t Hwc2TestLayer::getColor() const { return mColor.get(); }

HWC2::Composition Hwc2TestLayer::getComposition() const {
  return mComposition.get();
}

/* The cursor position corresponds to {displayFrame.left, displayFrame.top} */
android::Rect Hwc2TestLayer::getCursorPosition() const {
...
android::Rect Hwc2TestLayer::getDisplayFrame() const {
  return mDisplayFrame.get();
}

float Hwc2TestLayer::getPlaneAlpha() const { return mPlaneAlpha.get(); }

android::FloatRect Hwc2TestLayer::getSourceCrop() const {
  return mSourceCrop.get();
}

android::Region Hwc2TestLayer::getSurfaceDamage() const {
  return mSurfaceDamage.get();
}

HWC2::Transform Hwc2TestLayer::getTransform() const { return mTransform.get(); }

android::Region Hwc2TestLayer::getVisibleRegion() const {
  return mVisibleRegion.get();
}

uint32_t Hwc2TestLayer::getZOrder() const { return mZOrder; }

bool Hwc2TestLayer::advanceBlendMode() { return mBlendMode.advance(); }

bool Hwc2TestLayer::advanceBufferArea() { return mBufferArea.advance(); }

bool Hwc2TestLayer::advanceColor() { return mColor.advance(); }

bool Hwc2TestLayer::advanceComposition() { return mComposition.advance(); }

bool Hwc2TestLayer::advanceCursorPosition() { return mDisplayFrame.advance(); }

bool Hwc2TestLayer::advanceDataspace() { return mDataspace.advance(); }

bool Hwc2TestLayer::advanceDisplayFrame() { return mDisplayFrame.advance(); }

bool Hwc2TestLayer::advancePlaneAlpha() { return mPlaneAlpha.advance(); }

bool Hwc2TestLayer::advanceSourceCrop() { return mSourceCrop.advance(); }

bool Hwc2TestLayer::advanceSurfaceDamage() { return mSurfaceDamage.advance(); }

bool Hwc2TestLayer::advanceTransform() { return mTransform.advance(); }

bool Hwc2TestLayer::advanceVisibleRegion() {
  if (mPlaneAlpha.advance()) return true;
