...
class Hwc2TestSurfaceManager {
 public:
  ...
  class BufferListener : public ConsumerBase::FrameAvailableListener {
   public:
    BufferListener(sp<IGraphicBufferConsumer> consumer, FenceCallback callback,
                   void* callbackArgs)
        : mConsumer(consumer),
          mCallback(callback),
...
class Hwc2TestEglManager {
 public:
  ...
  int initialize(sp<Surface> surface) {
    ...
    mEglContext =
        eglCreateContext(mEglDisplay, configs[0], EGL_NO_CONTEXT, attrs);
...
class Hwc2TestFenceGenerator {
 public:
  Hwc2TestFenceGenerator() {
    mSurfaceManager.initialize({1, 1}, HAL_PIXEL_FORMAT_RGBA_8888, setFence,
                               this);
  ...
  int32_t get() {
    ...
    while (mPending != false) {
      if (mCv.wait_for(lock, std::chrono::seconds(2)) ==
...
static void setColor(int32_t x, int32_t y, android_pixel_format_t format,
                     uint32_t stride, uint8_t* img, uint8_t r, uint8_t g,
                     uint8_t b, uint8_t a) {
  switch (format) {
 ...
 * egl. */
int Hwc2TestBuffer::get(int& outSlot,
  ...
  outFence = new android::Fence(mFenceGenerator->get());
  outBuffer = mBuffer;
  outSlot = 0;
...
static void compositeBufferFromLayers(
    ...
    const std::set<HWC2::Layer*>* layersToDraw,
  ...
  for (int32_t y = 0; y < bufferArea.height; y++) {
    for (int32_t x = 0; x < bufferArea.width; x++) {
      ...
      for (auto layer = layersToDraw->rbegin(); layer != layersToDraw->rend();
           ++layer) {
        const android::Rect df = testLayers->GetDisplayFrame(*layer);
        ...
        if (x < dfL || x >= dfR || y < dfT || y >= dfB) continue;
        ...
        int transform = (int)testLayers->GetTransform(*layer);
        ...
        if (transform > 0) {
          ...
          if (transform & (int)HWC2::Transform::FlipH) xPos = -xPos;

          /* Flip vertical by reflecting across the x axis. */
          if (transform & (int)HWC2::Transform::FlipV) yPos = -yPos;
          ...
          if (transform & (int)HWC2::Transform::Rotate90) {
            float tmp = xPos;
            xPos = yPos * dfW / dfH;
            yPos = -tmp * dfH / dfW;
          }
        ...
        android::FloatRect sc = testLayers->GetSourceCrop(*layer);
        ...
        HWC2::BlendMode blendMode = testLayers->GetBlendMode(*layer);

        if (blendMode == HWC2::BlendMode::Premultiplied) {
          rCur *= planeAlpha;
          gCur *= planeAlpha;
          bCur *= planeAlpha;
        }
        ...
        if (blendMode == HWC2::BlendMode::Premultiplied) {
          ...
          b = bCur + b * (1.0 - aCur);
          a = aCur + a * (1.0 - aCur);
        } else if (blendMode == HWC2::BlendMode::Coverage) {
 ...
 * transform, blend mode, source crop, etc. */
int Hwc2TestClientTargetBuffer::get(
    int& outSlot, android::sp<android::GraphicBuffer>& outBuffer,
  ...
  /* Create new graphic buffer with correct dimensions */
  mGraphicBuffer = new GraphicBuffer(
      bufferArea.width, bufferArea.height, mFormat,
      BufferUsage::CPU_READ_OFTEN | BufferUsage::CPU_WRITE_OFTEN |
          BufferUsage::COMPOSER_OVERLAY,
      "hwc2_test_buffer");
  ...
  if (!mGraphicBuffer->handle) return -EINVAL;
...
int Hwc2TestExpectedBuffer::generateExpectedBuffer(
  ...
  const std::set<HWC2::Layer*> allLayerSet(allLayers->begin(),
                                           allLayers->end());
...
int Hwc2TestOutputBuffer::getOutputBuffer(
    int& outSlot, android::sp<android::GraphicBuffer>& outBuffer,
    android::sp<android::Fence>& outFence) {
  if (mBufferArea.width == -1 || mBufferArea.height == -1) return -EINVAL;

  mGraphicBuffer = new GraphicBuffer(
      mBufferArea.width, mBufferArea.height, mFormat,
      BufferUsage::CPU_READ_OFTEN | BufferUsage::GPU_RENDER_TARGET,
      "hwc2_test_buffer");
