...
this.WifiP2pWorkerObserver = function(aDomMsgResponder) {
  ...
  return {
    ...
    onConnecting(aPeer) {
      ...
      fireEvent("onconnecting", { peer });
    },

    onConnected(aGroupOwner, aPeer) {
      ...
      fireEvent("onconnected", { groupOwner: go, peer });
    },

    onDisconnected(aPeer) {
      let peer = _peerList[aPeer.address];
      if (!peer) {
    ...
    onDOMMessage(aMessage) {
      ...
      switch (aMessage.name) {
        case "WifiP2pManager:getState": // A new DOM manager is created.
          addDomManager(msg);
          return {
            // Synchronous call. Simply return it.
            enabled: _enabled,
