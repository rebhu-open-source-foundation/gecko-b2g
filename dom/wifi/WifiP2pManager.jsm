...
const { StateMachine } = ChromeUtils.import(
  "resource://gre/modules/StateMachine.jsm"
);
...
this.WifiP2pManager = function(aP2pCommand, aNetUtil) {
  ...
  function parseEventString(aEventString) {
    ...
    let event = { id, info };
    debug(
      "Event parsing result: " + aEventString + ": " + JSON.stringify(event)
    );
...
function P2pStateMachine(aP2pCommand, aNetUtil) {
  ...
  let _localDevice = {
    ...
    deviceName:
      ...
      "_" +
      libcutils.property_get("ro.build.product"),
    ...
    enter() {
      ...
      connectToSupplicantIfNeeded(function callback() {
        ...
        detail = {
          ...
          deviceType:
            libcutils.property_get("ro.moz.wifi.p2p_device_type") ||
            DEFAULT_P2P_DEVICE_TYPE,
          wpsMethods:
            libcutils.property_get("ro.moz.wifi.p2p_wps_methods") ||
        ...
        aP2pCommand.p2pEnable(detail, function(success) {
          ...
          gNetworkService.enableInterface(P2P_INTERFACE_NAME, function(
            success
          ) {
            onSuccess();
          });
    ...
    handleEvent(aEvent) {
      switch (aEvent.id) {
        ...
        case EVENT_P2P_PROV_DISC_ENTER_PIN:
          debug(
            "Someone is trying to connect to me: " + JSON.stringify(aEvent.info)
          );
  ...
  var stateWaitingForConfirmation = _sm.makeState("WAITING_FOR_CONFIRMATION", {
    ...
    enter() {
      ...
      this.timeoutTimer = initTimeoutTimer(
        30000,
      ...
      switch (aEvent.id) {
        case EVENT_P2P_GO_NEG_REQUEST:
          if (aEvent.info.wpsMethod !== _savedConfig.wpsMethod) {
            debug(
              "Unmatched wps method: " +
                aEvent.info.wpsMethod +
                ", " +
                _savedConfig.wpsMetho
            );
          }
          _sm.gotoState(stateConnecting);
          break;
  ...
  var stateWaitingForInvitationConfirmation = _sm.makeState(
        ...
        switch (aEvent.id) {
          case EVENT_P2P_SET_PAIRING_CONFIRMATION:
            ...
            aP2pCommand.p2pGetGroupCapab(_savedConfig.address, function(gc) {
              ...
              _sm.gotoState(
                isPeeGroupOwner ? stateGroupAdding : stateReinvoking
              );
            });
    ...
    handleEvent(aEvent) {
      switch (aEvent.id) {
        ...
        case EVENT_P2P_GROUP_REMOVED:
          debug(
            "Received P2P-GROUP-REMOVED due to previous failed handleGroupdStarted()"
          );
          _removedGroupInfo = {
  ...
  var stateReinvoking = _sm.makeState("REINVOKING", {
    ...
    enter() {
      ...
      _observer.onConnecting(_savedConfig);
      _sm.pause();
      aP2pCommand.p2pReinvoke(
        _savedConfig.netId,
        _savedConfig.address,
        function(success) {
          if (!success) {
            _sm.gotoState(stateInactive);
            return;
  ...
  var stateConnecting = _sm.makeState("CONNECTING", {
    ...
    enter() {
      ...
      if (
        WPS_METHOD_KEYPAD === _savedConfig.wpsMethod ||
    ...
    handleEvent(aEvent) {
      switch (aEvent.id) {
        ...
        case EVENT_P2P_GROUP_REMOVED:
          debug(
            "Received P2P-GROUP-REMOVED due to previous failed " +
              "handleGroupdStarted()"
          );
          _removedGroupInfo = {
            role: aEvent.info.role,
      ...
      switch (aEvent.id) {
        ...
        case EVENT_AP_STA_DISCONNECTED:
          ...
          aP2pCommand.p2pGroupRemove(_groupInfo.ifname, function(success) {
            debug(
              "Requested to remove p2p group. Wait for EVENT_P2P_GROUP_REMOVED."
            );
            _sm.resume();
          });
          break;
  ...
  var stateWaitingForJoiningConfirmation = _sm.makeState(
  ...
  var stateDisabling = _sm.makeState("DISABLING", {
    enter() {
      ...
      aNetUtil.stopDhcpServer(function(success) {
        // Stopping DHCP server is harmless.
        ...
        aP2pCommand.p2pDisable(function(success) {
          ...
          closeSupplicantConnectionIfNeeded(function() {
            ...
            gNetworkService.disableInterface(P2P_INTERFACE_NAME, function(
              success
            ) {
  ...
  function handleGroupStarted(aInfo, aCallback) {
    ...
    if (_groupInfo.isGroupOwner) {
      ...
      aNetUtil.startDhcpServer(dhcpServerConfig, function(success) {
        ...
        _p2pNetworkInterface.info.state =
        ...
        _p2pNetworkInterface.info.prefixLengths = [
          GO_NETWORK_INTERFACE.maskLength,
        ];
    ...
    debug(
      "Client. Request IP from DHCP server on interface: " + _groupInfo.ifname
    );
    ...
    aNetUtil.runDhcp(aInfo.ifname, 0, function(dhcpData) {
      ...
      let maskLength = netHelpers.getMaskLength(
        netHelpers.stringToIP(dhcpData.info.mask_str)
      );
      if (!maskLength) {
  ...
  function handleGroupRemoved(aInfo, aCallback) {
    ...
    if (_groupInfo.ifname !== aInfo.ifname || _groupInfo.role !== aInfo.role) {
      debug(
        "Unmatched group info: " +
          JSON.stringify(_groupInfo) +
          " v.s. " +
  ...
  function handleEventCommon(aEvent) {
    switch (aEvent.id) {
      ...
      case EVENT_P2P_CMD_DISCONNECT:
        debug(
          "The current state couldn't handle connect/disconnect request. Ignore it."
        );
        break;
