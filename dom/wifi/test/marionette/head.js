...
const HOSTAPD_CONFIG_LIST = [
  ...
  { ssid: "ap2", wpa: 2, rsn_pairwise: "CCMP", wpa_passphrase: "12345678" },
];
...
var gTestSuite = (function() {
  ...
  function getProcessDetail(aProcessName) {
    return runEmulatorShellSafe(["ps"]).then(processes => {
      // Sample 'ps' output:
      //
  ...
  function addRequiredPermissions() {
    ...
    let permissions = [
      { type: "wifi-manage", allow: 1, context: window.document },
  ...
  function wrapDomRequestAsPromise(aRequest) {
    ...
    ok(
      aRequest instanceof DOMRequest,
  ...
  function waitForRilDataConnected(aConnected, aServiceId) {
    aServiceId = aServiceId || 0;
    return waitForMobileConnectionEventOnce("datachange", aServiceId).then(
      function() {
        let mobileconnection = navigator.mozMobileConnections[aServiceId];
  ...
  function requestTetheringEnabled(aEnabled) {
    ...
    return setSettings1(SETTINGS_TETHERING_WIFI_ENABLED, aEnabled).then(
      function waitForRoutingVerified() {
        return verifyTetheringRouting(aEnabled).then(null, function onreject(
          aReason
        ) {
          log(
            "verifyTetheringRouting rejected due to " +
              aReason +
              " (" +
              retryCnt +
              ")"
  ...
  function forgetAllKnownNetworks() {
    ...
    return getKnownNetworks().then(networks =>
      createForgetNetworkChain(networks)
    );
  }
  ...
  function testWifiScanWithRetry(aRetryCnt, aExpectedNetworks) {
    ...
    function isScanResultExpected(aScanResult) {
      ...
      for (let i = 0; i < aScanResult.length; i++) {
        if (
          -1 === getFirstIndexBySsid(aScanResult[i].ssid, aExpectedNetworks)
        ) {
          return false;
        }
      }
  ...
  function waitForConnected(aExpectedNetwork) {
    return waitForWifiManagerEventOnce("statuschange").then(
  ...
  function getSettings(aKey, aAllowError) {
    ...
    return wrapDomRequestAsPromise(request).then(
      function resolve(aEvent) {
  ...
  function startHostapds(aConfigList) {
    ...
    function startOneHostapd(aIndex) {
      ...
      return writeHostapdConfFile(
        configFileName,
        createConfigFromCommon(aIndex)
      )
        ...
        .then(function(reply) {
          ...
          if (
            reply[0].indexOf("bind(PF_UNIX): Address already in use") !== -1
          ) {
    ...
    return Promise.all(
      aConfigList.map(function(aConfig, aIndex) {
        return startOneHostapd(aIndex);
      })
    );
  }
  ...
  function isInitServiceRunning(aServiceName) {
    return runEmulatorShellSafe(["getprop", "init.svc." + aServiceName]).then(
      function(result) {
  ...
  function startStockHostapd() {
    return startStopInitService(STOCK_HOSTAPD_NAME, true).then(
      null,
      function onreject() {
  ...
  function verifyNumOfProcesses(aProcessName, aExpectedNum) {
    return getProcessDetail(aProcessName).then(function(detail) {
      if (detail.length === aExpectedNum) {
        return;
      }
      throw "Unexpected number of running processes:" +
        aProcessName +
        ", expected: " +
        aExpectedNum +
        ", actual: " +
  ...
  function exeAndParseNetcfg() {
    return runEmulatorShellSafe(["netcfg"]).then(function(aLines) {
      // Sample output:
      //
  ...
  function exeAndParseIpRoute() {
    return runEmulatorShellSafe(["ip", "route"]).then(function(aLines) {
      // Sample output:
      //
      ...
      aLines.forEach(function(aLine) {
        ...
        ipRouteResult[ifname] = { src, default: false, gateway: null };
      });
  ...
  function verifyTetheringRouting(aEnabled) {
    ...
    function verifyDefaultRouteAndIp(aExpectedWifiTetheringIp) {
      if (aEnabled) {
        isOrThrow(
          ipRouteResult.rmnet0.src,
          netcfgResult.rmnet0.ip,
          "rmnet0.ip"
        );
    ...
    return verifyIptables()
      ...
      .then(aResult => {
        netcfgResult = aResult;
      })
      .then(exeAndParseIpRoute)
      .then(aResult => {
        ipRouteResult = aResult;
      })
    ...
    return initTestEnvironment()
      ...
      .then(
        ...
        function onreject(aReason) {
          ok(
            false,
            "Promise rejects during test" + (aReason ? "(" + aReason + ")" : "")
          );
          cleanUp();
        }
      );
  ...
  suite.doTestTethering = function(aTestCaseChain) {
    function verifyInitialState() {
      return getSettings(SETTINGS_RIL_DATA_ENABLED)
        ...
        .then(enabled =>
          isOrThrow(enabled, false, SETTINGS_TETHERING_WIFI_ENABLED)
        );
    }
      ...
      return (
        verifyInitialState()
          ...
          .then(restoreToInitialState, function onreject(aReason) {
            return restoreToInitialState().then(() => {
              throw aReason;
            }); // Re-throw the orignal reject reason.
          })
      );
    });
  };
   ...
   * @return A deferred promise.
   */
  suite.doTestWithCertificate = function(
    certBlob,
    password,
    nickname,
    usage,
