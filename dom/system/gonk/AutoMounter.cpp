...
// TODO: temporarily comment out OpenFileFinder
#if 0
...
#undef ERR
#define LOG(args...) \
  __android_log_print(ANDROID_LOG_INFO, "AutoMounter", ##args)
#define LOGW(args...) \
  __android_log_print(ANDROID_LOG_WARN, "AutoMounter", ##args)
#define ERR(args...) \
  __android_log_print(ANDROID_LOG_ERROR, "AutoMounter", ##args)

#undef DBG
#if USE_DEBUG
#  define DBG(args...) \
    __android_log_print(ANDROID_LOG_DEBUG, "AutoMounter", ##args)
#else
#  define DBG(args...)
#endif
...
class AutoMounter {
 public:
  ...
  const char* ModeStr(int32_t aMode) {
    switch (aMode) {
      case AUTOMOUNTER_DISABLE:
        return "Disable";
      case AUTOMOUNTER_ENABLE_UMS:
        return "Enable-UMS";
      case AUTOMOUNTER_DISABLE_WHEN_UNPLUGGED:
        return "DisableWhenUnplugged";
      case AUTOMOUNTER_ENABLE_MTP:
        return "Enable-MTP";
    }
    return "??? Unknown ???";
  }
  ...
  bool IsModeEnabled(int32_t aMode) {
    return aMode == AUTOMOUNTER_ENABLE_MTP || aMode == AUTOMOUNTER_ENABLE_UMS;
  }
  ...
  void SetMode(int32_t aMode) {
    ...
    if (aMode == AUTOMOUNTER_DISABLE && mMode == AUTOMOUNTER_ENABLE_UMS &&
        IsUsbCablePluggedIn()) {
      // On many devices (esp non-Samsung), we can't force the disable, so we
      // need to defer until the USB cable is actually unplugged.
      // See bug 777043.
      //
      // Otherwise our attempt to disable it will fail, and we'll wind up in a
      // bad state where the AutoMounter thinks that Sharing has been turned
      // off, but the files are actually still being Shared because the attempt
      // to unshare failed.
 ...
 private:
  enum STATE {
    ...
    // We've added mass_storage (aka UMS) to sys.usb.config and we're waiting
    // for mass_storage to appear in
  ...
  const char* StateStr(STATE aState) {
    switch (aState) {
      ...
      case STATE_MTP_CONFIGURING:
        return "MTP_CONFIGURING";
      case STATE_MTP_CONNECTED:
        return "MTP_CONNECTED";
      case STATE_MTP_STARTED:
        return "MTP_STARTED";
      case STATE_UMS_CONFIGURING:
        return "UMS_CONFIGURING";
      case STATE_UMS_CONFIGURED:
        return "UMS_CONFIGURED";
      case STATE_RNDIS_CONFIGURED:
        return "RNDIS_CONFIGURED";
    }
    return "STATE_???";
  }
...
void AutoVolumeManagerStateObserver::Notify(
    const VolumeManager::StateChangedEvent&) {
...
void AutoVolumeEventObserver::Notify(Volume* const&) {
  if (!sAutoMounter) {
    return;
  }
...
static bool IsUsbFunctionEnabled(const char* aConfig, const char* aUsbFunc) {
  ...
  while (tokenizer.hasMoreTokens()) {
    ...
    if (token.Equals(aUsbFunc)) {
      DBG("IsUsbFunctionEnabled('%s', '%s'): returning true", aConfig,
          aUsbFunc);
      return true;
    }
  }
...
static void SetUsbFunction(const char* aUsbFunc) {
...
void AutoMounter::UpdateState() {
  ...
  LOG("UpdateState: ums:A%dC%dE%d mtp:A%dC%dE%d mode:%d usb:%d tryToShare:%d "
      "state:%s",
      umsAvail, umsConfigured, umsEnabled, mtpAvail, mtpConfigured, mtpEnabled,
  ...
  for (volIndex = 0; volIndex < numVolumes; volIndex++) {
    ...
    if (vol->State() == nsIVolume::STATE_MOUNTED) {
      LOG("UpdateState: Volume %s is %s and %s @ %s gen %d locked %d sharing "
          "%s",
          vol->NameStr(), vol->StateStr(),
          vol->MediaPresent() ? "inserted" : "missing", vol->MountPoint().get(),
          vol->MountGeneration(), (int)vol->IsMountLocked(),
          vol->CanBeShared()
              ? (vol->IsSharingEnabled() ? (vol->IsSharing() ? "en-y" : "en-n")
                                         : "dis")
              : "x");
      ...
      LOG("UpdateState: Volume %s is %s and %s", vol->NameStr(),
          vol->StateStr(), vol->MediaPresent() ? "inserted" : "missing");
    }
    if (!vol->MediaPresent()) {
    ...
    if (vol->State() == nsIVolume::STATE_CHECKMNT) {
      ...
      if (rc == -1) {
        ...
        ERR("statfs failed for '%s': errno = %d (%s)", vol->NameStr(), errno,
            strerror(errno));
        continue;
      }
      ...
      if (fsbuf.f_blocks == 0) {
        if (delay <= 4000) {
          LOG("UpdateState: Volume '%s' is inaccessible, checking again in %d "
              "msec",
              vol->NameStr(), delay);
          MessageLoopForIO::current()->PostDelayedTask(
              NewRunnableMethod("AutoMounter::UpdateState", this,
                                &AutoMounter::UpdateState),
              delay);
          delay *= 2;
        } else {
          LOG("UpdateState: Volume '%s' is inaccessible, giving up",
              vol->NameStr());
        }
        continue;
      } else {
      ...
      switch (volState) {
        ...
        case nsIVolume::STATE_MOUNTED: {
          if (vol->IsMountLocked()) {
            ...
            // state.
            LOGW(
                "UpdateState: Mounted volume %s is locked, not sharing or "
                "formatting",
                vol->NameStr());
            break;
          }
          ...
          // This will cause apps which watch device storage notifications to
          // see the volume go into the different state, and prompt them to
          // close any open files that they might have.
          ...
          if (fileFinder.First(&fileInfo)) {
            ...
            do {
              LOGW("  PID: %d file: '%s' app: '%s' comm: '%s' exe: '%s'\n",
                   fileInfo.mPid, fileInfo.mFileName.get(),
                   fileInfo.mAppName.get(), fileInfo.mComm.get(),
                   fileInfo.mExe.get());
            } while (fileFinder.Next(&fileInfo));
            LOGW(
                "UpdateState: Mounted volume %s has open files, not sharing or "
                "formatting",
                vol->NameStr());
            ...
            MessageLoopForIO::current()->
              PostDelayedTask(NewRunnableMethod("AutoMounter::UpdateState",
                                                this, &AutoMounter::UpdateState),
                              delay);
            filesOpen = true;
            break;
          }
...
void AutoMounter::GetStatus(bool& umsAvail, bool& umsConfigured,
                            bool& umsEnabled, bool& mtpAvail,
                            bool& mtpConfigured, bool& mtpEnabled,
                            bool& rndisConfigured) {
  umsAvail = false;
  umsConfigured = false;
  umsEnabled = false;
  ...
  if (umsAvail) {
    umsConfigured =
        usbConfigured && strstr(functionsStr, USB_FUNC_UMS) != nullptr;
    umsEnabled =
        (mMode == AUTOMOUNTER_ENABLE_UMS) ||
...
static void InitAutoMounterIOThread() {
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
...
static void ShutdownAutoMounterIOThread() {
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
...
static void SetAutoMounterModeIOThread(const int32_t& aMode) {
...
static void AutoMounterFormatVolumeIOThread(const nsCString& aVolumeName) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  MOZ_ASSERT(sAutoMounter);
...
static void AutoMounterMountVolumeIOThread(const nsCString& aVolumeName) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  MOZ_ASSERT(sAutoMounter);
...
static void AutoMounterUnmountVolumeIOThread(const nsCString& aVolumeName) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  MOZ_ASSERT(sAutoMounter);
...
class UsbCableObserver final : public hal::SwitchObserver {
  ~UsbCableObserver() { hal::UnregisterSwitchObserver(hal::SWITCH_USB, this); }

 public:
  NS_INLINE_DECL_REFCOUNTING(UsbCableObserver)

  UsbCableObserver() { hal::RegisterSwitchObserver(hal::SWITCH_USB, this); }

  virtual void Notify(const hal::SwitchEvent& aEvent) {
    DBG("UsbCable switch device: %d state: %s\n", aEvent.device(),
        SwitchStateStr(aEvent));
    XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
        "UsbCableEventIOThreadRunnable", UsbCableEventIOThread));
  }
};
...
void SetAutoMounterSharingMode(const nsCString& aVolumeName,
                               bool aAllowSharing) {
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "SetAutoMounterSharingModeIOThreadRunnable",
      SetAutoMounterSharingModeIOThread, aVolumeName, aAllowSharing));
}

void AutoMounterFormatVolume(const nsCString& aVolumeName) {
  XRE_GetIOMessageLoop()->PostTask(
      NewRunnableFunction("AutoMounterFormatVolumeIOThreadRunnable",
                          AutoMounterFormatVolumeIOThread, aVolumeName));
}

void AutoMounterMountVolume(const nsCString& aVolumeName) {
  XRE_GetIOMessageLoop()->PostTask(
      NewRunnableFunction("AutoMounterMountVolumeIOThreadRunnable",
                          AutoMounterMountVolumeIOThread, aVolumeName));
}

void AutoMounterUnmountVolume(const nsCString& aVolumeName) {
  XRE_GetIOMessageLoop()->PostTask(
      NewRunnableFunction("AutoMounterUnmountVolumeIOThreadRunnable",
                          AutoMounterUnmountVolumeIOThread, aVolumeName));
}
