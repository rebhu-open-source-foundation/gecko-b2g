...
class Volume final {
 public:
  ...
  static const char* StateStr(STATE aState) {
    return NS_VolumeStateStr(aState);
  }
  ...
  bool IsFormatRequested() const {
    return CanBeFormatted() && mFormatRequested;
  }
  bool IsMountRequested() const { return CanBeMounted() && mMountRequested; }
  bool IsUnmountRequested() const {
    return CanBeMounted() && mUnmountRequested;
  }
  ...
  static void RegisterVolumeObserver(EventObserver* aObserver,
                                     const char* aName);
  static void UnregisterVolumeObserver(EventObserver* aObserver,
                                       const char* aName);
