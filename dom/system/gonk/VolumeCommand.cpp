 ...
 *   605 Volume sdcard /mnt/sdcard state changed from 7 (Shared-Unmounted) to 1
 *(Idle-Unmounted) 200 volume operation succeeded
 *
...
VolumeActionCommand::VolumeActionCommand(Volume* aVolume, const char* aAction,
                                         const char* aExtraArgs,
                                         VolumeResponseCallback* aCallback)
    : VolumeCommand(aCallback), mVolume(aVolume) {
  nsAutoCString cmd;
