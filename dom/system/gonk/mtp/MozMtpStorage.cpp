...
MozMtpStorage::MozMtpStorage(Volume* aVolume, MozMtpServer* aMozMtpServer)
    : mMozMtpServer(aMozMtpServer), mVolume(aVolume) {
...
// virtual
void MozMtpStorage::Notify(Volume* const& aVolume) {
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
...
void MozMtpStorage::StorageAvailable() {
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  ...
  if (db->isStorageIDExisted(mStorageID)) {
    MTP_LOG(
        "Volume %s mStorageID 0x%08x mountPoint %s already existed in "
        "MozMtpDatabase",
        mVolume->NameStr(), mStorageID, mountPoint.get());
    return;
  }
...
void MozMtpStorage::StorageUnavailable() {
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  MOZ_ASSERT(mMtpStorage);
