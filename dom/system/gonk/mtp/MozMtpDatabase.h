...
class MozMtpDatabase final : public IMtpDatabase {
 public:
  ...
  virtual void endSendObject(MtpObjectHandle aHandle, bool aSucceeded);

  virtual void rescanFile(const char* path, MtpObjectHandle handle,
  ...
  virtual int getNumObjects(MtpStorageID aStorageID, MtpObjectFormat aFormat,
                            MtpObjectHandle aParent);
  ...
  virtual MtpObjectPropertyList* getSupportedObjectProperties(
      MtpObjectFormat aFormat);
  ...
  virtual MtpResponseCode beginMoveObject(MtpObjectHandle handle,
                                          MtpObjectHandle newParent,
                                          MtpStorageID newStorage);

  virtual void endMoveObject(MtpObjectHandle oldParent,
                             MtpObjectHandle newParent, MtpStorageID oldStorage,
                             MtpStorageID newStorage, MtpObjectHandle handle,
                             bool succeeded);

  virtual MtpResponseCode beginCopyObject(MtpObjectHandle handle,
                                          MtpObjectHandle newParent,
                                          MtpStorageID newStorage);
  virtual void endCopyObject(MtpObjectHandle handle, bool succeeded);

  void AddStorage(MtpStorageID aStorageID, const char* aPath,
                  const char* aName);
  void RemoveStorage(MtpStorageID aStorageID);

  void MtpWatcherUpdate(RefCountedMtpServer* aMtpServer,
                        DeviceStorageFile* aFile, const nsACString& aEventType);

  bool GetMtpConnectedStatus() { return mMtpConnectedStatus; }

  void SetMtpConnectedStatus(bool status) { mMtpConnectedStatus = status; }

  bool isStorageIDExisted(MtpStorageID aStorageID) {
    return (MozMtpDatabase::StorageArray::NoIndex != FindStorage(aStorageID));
  }
 ...
 private:
  ...
  class ProtectedTArray : private nsTArray<T> {
   public:
    ...
    ProtectedTArray(mozilla::Mutex& aMutex) : mMutex(aMutex) {}
  ...
  MtpObjectHandle GetNextHandle() { return mDb.Length(); }

  void AddDirectory(MtpStorageID aStorageID, const char* aPath,
                    MtpObjectHandle aParent);
