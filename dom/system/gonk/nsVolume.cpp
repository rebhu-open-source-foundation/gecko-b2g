...
const char* NS_VolumeStateStr(int32_t aState) {
  switch (aState) {
    ...
    case nsIVolume::STATE_IDLE:
      return "Idle";
    case nsIVolume::STATE_PENDING:
      return "Pending";
    case nsIVolume::STATE_CHECKING:
      return "Checking";
...
void nsVolume::Dump(const char* aLabel) const {
  LOG("%s: Volume: %s is %s and %s @ %s gen %d locked %d", aLabel,
      NameStr().get(), StateStr(), IsMediaPresent() ? "inserted" : "missing",
      MountPointStr().get(), MountGeneration(), (int)IsMountLocked());
  LOG("%s:   IsSharing %s IsFormating %s IsUnmounting %s", aLabel,
      (IsSharing() ? "y" : "n"), (IsFormatting() ? "y" : "n"),
      (IsUnmounting() ? "y" : "n"));
}
...
NS_IMETHODIMP nsVolume::Format() {
  ...
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "FormatVolumeIOThreadRunnable", FormatVolumeIOThread, NameStr()));
...
NS_IMETHODIMP nsVolume::Mount() {
  ...
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "MountVolumeIOThreadRunnable", MountVolumeIOThread, NameStr()));
...
NS_IMETHODIMP nsVolume::Unmount() {
  ...
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "UnmountVolumeIOThreadRunnable", UnmountVolumeIOThread, NameStr()));
...
void nsVolume::LogState() const {
  if (mState == nsIVolume::STATE_MOUNTED) {
    LOG("nsVolume: %s state %s @ '%s' gen %d locked %d fake %d "
        "media %d sharing %d formatting %d unmounting %d removable %d "
        "hotswappable %d",
        NameStr().get(), StateStr(), MountPointStr().get(), MountGeneration(),
        (int)IsMountLocked(), (int)IsFake(), (int)IsMediaPresent(),
        (int)IsSharing(), (int)IsFormatting(), (int)IsUnmounting(),
...
void nsVolume::UpdateMountLock(const nsAString& aMountLockState) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(NS_IsMainThread());
...
void nsVolume::UpdateMountLock(bool aMountLocked) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(NS_IsMainThread());
  ...
  LogState();
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "Volume::UpdateMountLockRunnable", Volume::UpdateMountLock,
      NS_LossyConvertUTF16toASCII(Name()), MountGeneration(), aMountLocked));
}

void nsVolume::SetIsFake(bool aIsFake) {
  mIsFake = aIsFake;
  if (mIsFake) {
...
void nsVolume::SetIsRemovable(bool aIsRemovable) {
...
void nsVolume::SetIsHotSwappable(bool aIsHotSwappable) {
...
void nsVolume::SetState(int32_t aState) {
