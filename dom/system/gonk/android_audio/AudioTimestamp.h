...
class AudioTimestamp {
 public:
  ...
  struct timespec
      mTime;  // corresponding CLOCK_MONOTONIC when frame is expected to present
};
...
struct alignas(8) /* bug 29096183, bug 29108507 */ ExtendedTimestamp {
  enum Location {
    ...
    LOCATION_CLIENT,  // timestamp of last read frame from client-server track
                      // buffer.
    LOCATION_SERVER,  // timestamp of newest frame from client-server track
                      // buffer.
    LOCATION_KERNEL,  // timestamp of newest frame in the kernel (alsa) buffer.

    // Historical data: info when the kernel timestamp was OK (prior to the
    // newest frame). This may be useful when the newest frame kernel timestamp
    // is unavailable. Available for playback timestamps.
    LOCATION_SERVER_LASTKERNELOK,  // timestamp of server the prior time kernel
                                   // timestamp OK.
    LOCATION_KERNEL_LASTKERNELOK,  // timestamp of kernel the prior time kernel
                                   // timestamp OK.
  ...
  // hence not included in mPosition. This is used for adjusting server
  // positions information for frames "dropped".
  // FIXME: This variable should be eliminated, with the offset added on the
  // server side before sending to client, but differences in legacy position
  // offset handling and new extended timestamps require this to be maintained
  ...
  void clear() {
    memset(mPosition, 0,
           sizeof(mPosition));  // actually not necessary if time is -1
  ...
  // pipeline with a valid timestamp.  If the optional location parameter is
  // non-null, it will be filled with the location where the time was obtained.
  status_t getBestTimestamp(int64_t* position, int64_t* time, int timebase,
                            Location* location = nullptr) const {
    if (position == nullptr || time == nullptr || timebase < 0 ||
        timebase >= TIMEBASE_MAX) {
      return BAD_VALUE;
    }
    // look for the closest-to-hw stage in the pipeline with a valid timestamp.
    // We omit LOCATION_CLIENT as we prefer at least LOCATION_SERVER based
    // accuracy when getting the best timestamp.
  ...
  status_t getBestTimestamp(AudioTimestamp* timestamp,
                            Location* location = nullptr) const {
    if (timestamp == nullptr) {
      return BAD_VALUE;
    }
    int64_t position, time;
    if (getBestTimestamp(&position, &time, TIMEBASE_MONOTONIC, location) ==
        OK) {
  ...
  std::string toString() {
    ...
    for (int i = 0; i < LOCATION_MAX; ++i) {
      ss << "ExtendedTimestamp[" << i << "]  position: " << mPosition[i]
         << "  time: " << mTimeNs[i] << "\n";
    }
    return ss.str();
  }
