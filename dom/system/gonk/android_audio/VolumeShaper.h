...
// TODO: member static const or constexpr float initialization not permitted in
// C++11
...
#define MAX_LOG_VOLUME 0.f     // type T: max volume, unity gain in dBFS
  ...
  class Configuration : public Interpolator<S, T>, public RefBase {
   public:
    ...
    // Bring from base class; must match with VolumeShaper.java in
    // frameworks/base.
    ...
    explicit Configuration(const Configuration& configuration)
        : Interpolator<S, T>(
              *static_cast<const Interpolator<S, T>*>(&configuration)),
          RefBase(),
    ...
    OptionFlag getOptionFlags() const { return mOptionFlags; }
    ...
    double getDurationMs() const { return mDurationMs; }
    ...
    /* Adjust the volume to be in linear range from MIN_LINEAR_VOLUME to
     * MAX_LINEAR_VOLUME and compensate for log dbFS volume as needed.
     */
    T adjustVolume(T volume) const {
      ...
      return clamp(volume, MIN_LINEAR_VOLUME /* lo */,
                   MAX_LINEAR_VOLUME /* hi */);
    }
    ...
    void scaleToStartVolume(T volume) {
      ...
      if (endVolume == startVolume) {
        ...
        static const T scale =
            1.f / (MAX_CURVE_TIME - MIN_CURVE_TIME);  // nominally 1.f
        for (auto it = this->begin(); it != this->end(); ++it) {
          it->second =
              it->second + offset * (MAX_CURVE_TIME - it->first) * scale;
        }
      } else {
  ...
  class Operation : public RefBase {
   public:
    ...
    Operation() : Operation(FLAG_NONE, -1 /* replaceId */) {}

    Operation(Flag flags, int replaceId)
        : Operation(flags, replaceId,
                    std::numeric_limits<S>::quiet_NaN() /* xOffset */) {}
    ...
    Operation(Flag flags, int replaceId, S xOffset)
        : mFlags(flags), mReplaceId(replaceId), mXOffset(xOffset) {}

    int32_t getReplaceId() const { return mReplaceId; }

    void setReplaceId(int32_t replaceId) { mReplaceId = replaceId; }

    S getXOffset() const { return mXOffset; }
    ...
    void setNormalizedTime(S normalizedTime) {
      setXOffset((mFlags & FLAG_REVERSE) != 0 ? MAX_CURVE_TIME - normalizedTime
                                              : normalizedTime);
    }
    ...
    status_t readFromParcel(const Parcel& parcel) {
      ...
      return parcel.readInt32(&flags)
                 ?: parcel.readInt32(&mReplaceId)
                        ?: parcel.readFloat(&mXOffset) ?: setFlags((Flag)flags);
    }
  ...
  class State : public RefBase {
   public:
    State(T volume, S xOffset) : mVolume(volume), mXOffset(xOffset) {}

    State() : State(NAN, NAN) {}

    T getVolume() const { return mVolume; }

    void setVolume(T volume) { mVolume = volume; }

    S getXOffset() const { return mXOffset; }

    void setXOffset(S xOffset) { mXOffset = xOffset; }

    status_t writeToParcel(Parcel* parcel) const {
      if (parcel == nullptr) return BAD_VALUE;
      return parcel->writeFloat(mVolume) ?: parcel->writeFloat(mXOffset);
    }

    status_t readFromParcel(const Parcel& parcel) {
      return parcel.readFloat(&mVolume) ?: parcel.readFloat(&mXOffset);
    }
   ...
   private:
    T mVolume;   // linear volume in the range MIN_LINEAR_VOLUME to
                 // MAX_LINEAR_VOLUME
    S mXOffset;  // position on curve expressed from MIN_CURVE_TIME to
                 // MAX_CURVE_TIME
  };             // State

  // Internal helper class to do an affine transform for time and amplitude
  // scaling.
  template <typename R>
  class Translate {
   public:
    Translate() : mOffset(0), mScale(1) {}

    R getOffset() const { return mOffset; }

    void setOffset(R offset) { mOffset = offset; }

    R getScale() const { return mScale; }

    void setScale(R scale) { mScale = scale; }

    R operator()(R in) const { return mScale * (in - mOffset); }
   ...
   */
  VolumeShaper(const sp<VolumeShaper::Configuration>& configuration,
  ...
  VolumeShaper::Operation::Flag getFlags() const {
    return mOperation == nullptr ? VolumeShaper::Operation::FLAG_NONE
                                 : mOperation->getFlags();
  }
  ...
  S getDelayXOffset() const { return mDelayXOffset; }
  ...
  bool isStarted() const { return mStartFrame >= 0; }
  ...
  std::pair<T /* volume */, bool /* active */> getVolume(
    ...
    const bool clockTime =
        (mConfiguration->getOptionFlags() &
    ...
    VS_LOG("rescaled time:%f  volume:%f  xOffset:%f  active:%s", x, mLastVolume,
           mLastXOffset, active ? "true" : "false");
  ...
  std::string toString() const {
    ...
    ss << ", mConfiguration="
       << (mConfiguration.get() == nullptr
               ? "nullptr"
               : mConfiguration->toString().c_str());
    ss << ", mOperation="
       << (mOperation.get() == nullptr ? "nullptr"
                                       : mOperation->toString().c_str());
    ss << "}";
    return ss.str();
  }

  Translate<S> mXTranslate;  // translation from frames (usec for clock time) to
                             // normalized time.
  sp<VolumeShaper::Configuration> mConfiguration;
  sp<VolumeShaper::Operation> mOperation;

 private:
  int64_t mStartFrame;  // starting frame, non-negative when started (in usec
                        // for clock time)
  ...
  T computeVolumeFromXOffset(S xOffset) const {
    ...
    const T volume =
        mConfiguration->adjustVolume(unscaledVolume);  // handle log scale
    VS_LOG("computeVolumeFromXOffset %f -> %f -> %f", xOffset, unscaledVolume,
           volume);
    return volume;
  }
...
/* VolumeHandler combines the volume factors of multiple VolumeShapers
 * associated with a player.  It is thread safe by synchronizing all public
 * methods.
 *
...
class VolumeHandler : public RefBase {
 public:
  ...
  // A volume handler which just keeps track of active VolumeShapers does not
  // need sampleRate.
  VolumeHandler() : VolumeHandler(0 /* sampleRate */) {}
  ...
  VolumeShaper::Status applyVolumeShaper(
    ...
    sp<VolumeShaper::Operation> operation(
        new VolumeShaper::Operation(operation_in));
    VS_LOG("applyVolumeShaper:configuration: %s",
    ...
    switch (configuration->getType()) {
      case VolumeShaper::Configuration::TYPE_SCALE: {
        ...
        if (replaceId >= 0) {
          ...
          if (replaceIt == mVolumeShapers.end()) {
            ...
            if ((operation->getFlags() & VolumeShaper::Operation::FLAG_JOIN) !=
                0) {
              // For join, we scale the start volume of the current
              // configuration to match the last-used volume of the replacing
              // VolumeShaper.
        ...
        /* Check if too many application VolumeShapers (with id >=
         * kSystemVolumeShapersMax). We check on the server side to ensure
        ...
        if (id >= VolumeShaper::kSystemVolumeShapersMax &&
            numberOfUserVolumeShapers_l() >=
                VolumeShaper::kUserVolumeShapersMax) {
          ALOGW("Too many app VolumeShapers, cannot add to VolumeHandler");
          return VolumeShaper::Status(INVALID_OPERATION);
        }

        // create new VolumeShaper with default behavior.
        mVolumeShapers.emplace_back(configuration,
                                    new VolumeShaper::Operation());
        VS_LOG("after adding, number of volumeShapers:%zu",
               mVolumeShapers.size());
      }
      ...
      case VolumeShaper::Configuration::TYPE_ID: {
        ...
        if ((operation->getFlags() & VolumeShaper::Operation::FLAG_TERMINATE) !=
            0) {
        ...
        if (!std::isnan(xOffset)) {
          if (it->isStarted()) {
            ...
            const S target = (operation->getFlags() &
                              VolumeShaper::Operation::FLAG_REVERSE) != 0
                                 ? MAX_CURVE_TIME - xOffset
                                 : xOffset;
  ...
  std::pair<T /* volume */, bool /* active */> getVolume(
    ...
    VS_LOG("getVolume: <%f, %s>", mLastVolume.first,
           mLastVolume.second ? "true" : "false");
    return mLastVolume;
  }
  ...
  void setStarted() {
    (void)getVolume(
        mLastFrame);  // getVolume() will start the individual VolumeShapers.
  }
  ...
  void forall(
      const std::function<VolumeShaper::Status(const VolumeShaper&)>& lambda) {
    AutoMutex _l(mLock);
 ...
 private:
  ...
  size_t numberOfUserVolumeShapers_l() const {
    size_t count = 0;
    for (const auto& shaper : mVolumeShapers) {
      count += (shaper.mConfiguration->getId() >=
                VolumeShaper::kSystemVolumeShapersMax);
    }
    return count;
  }
  ...
  int32_t
      mVolumeShaperIdCounter;  // a counter to return a unique volume shaper id.
  std::pair<T /* volume */, bool /* active */> mLastVolume;
  std::list<VolumeShaper>
      mVolumeShapers;  // list provides stable iterators on erase
};                     // VolumeHandler
