...
/* represents an invalid uid for tracks; the calling or client uid is often
 * substituted. */
...
/* AudioFlinger and AudioPolicy services use I/O handles to identify audio
 * sources and sinks */
...
/* a unique ID allocated by AudioFlinger for use as an audio_io_handle_t,
 * audio_session_t, effect ID (int), audio_module_handle_t, and
 * audio_patch_handle_t. Audio port IDs (audio_port_handle_t) are allocated by
 * AudioPolicy in a different namespace than AudioFlinger unique IDs.
 */
...
typedef enum {
  ...
  AUDIO_UNIQUE_ID_USE_SESSION =
      1,  // for allocated sessions, not special AUDIO_SESSION_*
...
static inline audio_unique_id_use_t audio_unique_id_get_use(
 ...
 * the order. But see AUDIO_INTERLEAVE_* below for the platform convention of
 * order.
 *
 ...
 *     Each low-order bit corresponds to the spatial position of a transducer
 * (output), or interpretation of channel (input). The user of a channel mask
 * needs to know the context of whether it is for output or input. The constants
 * AUDIO_CHANNEL_OUT_* or AUDIO_CHANNEL_IN_* apply to the bits portion. It is
 * not permitted for no bits to be set.
 *
...
static inline uint32_t audio_channel_mask_get_bits(
...
/* The return value is undefined if the channel mask is invalid. */
static inline audio_channel_representation_t
 ...
 * or returns false for AUDIO_CHANNEL_NONE, AUDIO_CHANNEL_INVALID, and other
 * invalid values. This function is unable to determine whether a channel mask
 * for position assignment is invalid because an output mask has an invalid
 * output bit set, or because an input mask has an invalid input bit set. All
 * other APIs that take a channel mask assume that it is valid.
 */
static inline bool audio_channel_mask_is_valid(audio_channel_mask_t channel) {
  ...
  audio_channel_representation_t representation =
      audio_channel_mask_get_representation(channel);
...
audio_channel_mask_from_representation_and_bits(
  ...
  return (audio_channel_mask_t)((representation << AUDIO_CHANNEL_COUNT_MAX) |
                                bits);
}
...
/**
 * Stub audio output device. Used in policy configuration file on platforms
 * without audio outputs. This alias value to AUDIO_DEVICE_OUT_DEFAULT is only
 * used in the audio policy context.
 */
#define AUDIO_DEVICE_OUT_STUB AUDIO_DEVICE_OUT_DEFAULT
/**
 * Stub audio input device. Used in policy configuration file on platforms
 * without audio inputs. This alias value to AUDIO_DEVICE_IN_DEFAULT is only
 * used in the audio policy context.
 */
...
struct audio_gain {
  ...
  audio_channel_mask_t
      channel_mask;         /* channels which gain an be controlled.
...
struct audio_port_config_mix_ext {
  ...
  union {
    // TODO: change use case for output streams: use strategy and mixer
    // attributes
...
struct audio_port {
  ...
  unsigned int
      num_sample_rates; /* number of sampling rates in following array */
  unsigned int sample_rates[AUDIO_PORT_MAX_SAMPLING_RATES];
  unsigned int
      num_channel_masks; /* number of channel masks in following array */
 ...
 * policy manager or by applications via framework APIs. Each patch is
 * identified by a handle at the interface used to create that patch. For
 * instance, when a patch is created by the audio HAL, the HAL allocates and
 * returns a handle. This handle is unique to a given audio HAL hardware module.
 * But the same patch receives another system wide unique handle allocated by
 * the framework. This unique handle is used for all transactions inside the
 * framework.
 */
...
/**
 * Mmap buffer read/write position returned by
 * audio_stream->get_mmap_position(). note\ Used by streams opened in mmap mode.
 */
struct audio_mmap_position {
  int64_t time_nanoseconds; /**< timestamp in ns, CLOCK_MONOTONIC */
  int32_t position_frames;  /**< increasing 32 bit frame count reset when
                               stream->stop()  is called */
};
...
static inline bool audio_is_bluetooth_sco_device(audio_devices_t device) {
  if ((device & AUDIO_DEVICE_BIT_IN) == 0) {
    ...
    if ((popcount(device) == 1) &&
        ((device & ~AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET) == 0))
      return true;
  }
 ...
 *  there is at least one channel bit set which _could_ correspond to an input
 * channel, and there are no channel bits set which could _not_ correspond to an
 * input channel. Otherwise returns false.
 */
...
/* Returns true if:
 *  representation is valid, and
 *  there is at least one channel bit set which _could_ correspond to an output
 * channel, and there are no channel bits set which could _not_ correspond to an
 * output channel. Otherwise returns false.
 */
 ...
 */
static inline uint32_t audio_channel_count_from_in_mask(
    audio_channel_mask_t channel) {
  uint32_t bits = audio_channel_mask_get_bits(channel);
  switch (audio_channel_mask_get_representation(channel)) {
    case AUDIO_CHANNEL_REPRESENTATION_POSITION:
 ...
 * Returns zero if the representation is invalid.
 */
static inline uint32_t audio_channel_count_from_out_mask(
    audio_channel_mask_t channel) {
  uint32_t bits = audio_channel_mask_get_bits(channel);
  switch (audio_channel_mask_get_representation(channel)) {
    case AUDIO_CHANNEL_REPRESENTATION_POSITION:
 ...
 * or AUDIO_CHANNEL_INVALID if the channel count exceeds
 * AUDIO_CHANNEL_COUNT_MAX.
 */
static inline audio_channel_mask_t
audio_channel_mask_for_index_assignment_from_count(uint32_t channel_count) {
  if (channel_count == 0) {
 ...
 * This is to be used when the content channel mask is unknown. The 1, 2, 4, 5,
 * 6, 7 and 8 channel cases are mapped to the standard game/home-theater
 * layouts, but note that 4 is mapped to quad, and not stereo + FC + mono
 * surround. A channel count of 3 is arbitrarily mapped to stereo + FC for
 * continuity with stereo. Returns the matching channel mask, or
 * AUDIO_CHANNEL_NONE if the channel count is zero, or AUDIO_CHANNEL_INVALID if
 * the channel count exceeds that of the configurations for which a default
 * output channel mask is defined.
 */
static inline audio_channel_mask_t audio_channel_out_mask_from_count(
 ...
 * Assumes a position mask for mono and stereo, or an index mask for channel
 * counts > 2. Returns the matching channel mask, or AUDIO_CHANNEL_NONE if the
 * channel count is zero, or AUDIO_CHANNEL_INVALID if the channel count exceeds
 * that of the configurations for which a default input channel mask is defined.
 */
static inline audio_channel_mask_t audio_channel_in_mask_from_count(
    uint32_t channel_count) {
  uint32_t bits;
  switch (channel_count) {
    case 0:
...
static inline char* audio_device_address_to_parameter(audio_devices_t device,
                                                      const char* address) {
  const size_t kSize =
      AUDIO_DEVICE_MAX_ADDRESS_LEN + sizeof("a2dp_sink_address=");
  char param[kSize];
...
static inline bool audio_device_is_digital(audio_devices_t device) {
  if ((device & AUDIO_DEVICE_BIT_IN) != 0) {
    ...
    return (~AUDIO_DEVICE_BIT_IN & device &
            (AUDIO_DEVICE_IN_ALL_USB | AUDIO_DEVICE_IN_HDMI |
             AUDIO_DEVICE_IN_SPDIF | AUDIO_DEVICE_IN_IP |
             AUDIO_DEVICE_IN_BUS)) != 0;
  } else {
    // output
    return (device & (AUDIO_DEVICE_OUT_ALL_USB | AUDIO_DEVICE_OUT_HDMI |
                      AUDIO_DEVICE_OUT_HDMI_ARC | AUDIO_DEVICE_OUT_SPDIF |
                      AUDIO_DEVICE_OUT_IP | AUDIO_DEVICE_OUT_BUS)) != 0;
  }
}
 ...
 * Return a valid source (positive integer) or AUDIO_HW_SYNC_INVALID if an error
 * occurs or no HW sync is available. */
...
#define AUDIO_PARAMETER_STREAM_INPUT_SOURCE                                    \
 ...
 * strings from audio_channel_mask_t enum e.g:
 * "sup_channels=AUDIO_CHANNEL_OUT_STEREO|AUDIO_CHANNEL_OUT_MONO" */
