...
class Interpolator : public std::map<S, T> {
 public:
  ...
  // Extend only at the end of enum, as this must match order in
  // VolumeShapers.java.
  enum InterpolatorType : int32_t {
    INTERPOLATOR_TYPE_STEP,             // Not continuous
    INTERPOLATOR_TYPE_LINEAR,           // C0
    INTERPOLATOR_TYPE_CUBIC,            // C1
    INTERPOLATOR_TYPE_CUBIC_MONOTONIC,  // C1 (to provide locally monotonic
                                        // curves)
  ...
  explicit Interpolator(
      ...
      bool cache = true)
      : mCache(cache), mFirstSlope(0), mLastSlope(0) {
    setInterpolatorType(interpolatorType);
  }

  std::pair<S, T> first() const { return *this->begin(); }

  std::pair<S, T> last() const { return *this->rbegin(); }
  ...
  T findY(S x) {  // logically const, but modifies cache
    ...
    switch (mInterpolatorType) {
      ...
      case INTERPOLATOR_TYPE_LINEAR:
        return ((high->first - x) * low->second +
                (x - low->first) * high->second) /
      ...
      default: {
        ...
        if (!catmullRom || monotonic) {
          ...
          sec0 = low2 != this->end()
                     ...
                     : mFirstSlope;
          sec1 = high2 != this->end() ? (high2->second - high->second) /
                                            (high2->first - high->first)
                                      : mLastSlope;
        }
        ...
        if (monotonic) {
          ...
          // A sufficient condition for Fritsch–Carlson monotonicity is
          // constraining (1) the normalized slopes to be within the circle of
          // radius 3, or (2) the normalized slopes to be within the square of
          // radius 3. Condition (2) is more generous and easier to compute.
        ...
        if (mCache) {
          ...
          const S h11 = t3 - t2;
          return h00 * low->second + (h10 * m0 + h11 * m1) * interval +
                 h01 * high->second;
        }
      }  // default
    }
  ...
  InterpolatorType getInterpolatorType() const { return mInterpolatorType; }
  ...
  T getFirstSlope() const { return mFirstSlope; }

  void setFirstSlope(T slope) { mFirstSlope = slope; }

  T getLastSlope() const { return mLastSlope; }

  void setLastSlope(T slope) { mLastSlope = slope; }

  void clearCache() { mMemo.clear(); }
  ...
  status_t writeToParcel(Parcel* parcel) const {
    ...
    status_t res =
        parcel->writeInt32(mInterpolatorType)
            ?: parcel->writeFloat(mFirstSlope)
                   ?: parcel->writeFloat(mLastSlope)
                          ?: parcel->writeUint32(
                                 (uint32_t)this->size());  // silent truncation
    if (res != NO_ERROR) {
      return res;
    }
    for (const auto& pt : *this) {
      res = parcel->writeFloat(pt.first) ?: parcel->writeFloat(pt.second);
      if (res != NO_ERROR) {
        return res;
      }
    }
  ...
  status_t readFromParcel(const Parcel& parcel) {
    ...
    for (uint32_t i = 0; i < size; ++i) {
      float x, y;
      res = parcel.readFloat(&x) ?: parcel.readFloat(&y);
      if (res != NO_ERROR) {
        return res;
      }
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "Interpolator{mInterpolatorType="
       << static_cast<int32_t>(mInterpolatorType);
