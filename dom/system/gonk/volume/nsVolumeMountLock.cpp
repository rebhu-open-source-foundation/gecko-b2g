...
NS_IMPL_ISUPPORTS(nsVolumeMountLock, nsIVolumeMountLock, nsIObserver,
                  nsISupportsWeakReference)

// static
already_AddRefed<nsVolumeMountLock> nsVolumeMountLock::Create(
    const nsAString& aVolumeName) {
...
nsVolumeMountLock::nsVolumeMountLock(const nsAString& aVolumeName)
    : mVolumeName(aVolumeName), mVolumeGeneration(-1), mUnlocked(false) {}

// virtual
nsVolumeMountLock::~nsVolumeMountLock() { Unlock(); }
...
NS_IMETHODIMP nsVolumeMountLock::Observe(nsISupports* aSubject,
                                         const char* aTopic,
...
nsresult nsVolumeMountLock::Lock(nsIVolume* aVolume) {
