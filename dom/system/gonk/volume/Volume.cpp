...
void VolumeObserverList::Broadcast(Volume* const& aVolume) {
  uint32_t size = mObservers.Length();
  for (uint32_t i = 0; i < size; ++i) {
    LOG("VolumeObserverList::Broadcast to [%u] %p volume '%s'", i,
        mObservers[i], aVolume->NameStr());
...
Volume::Volume(const nsACString& aName)
    : mMediaPresent(true),
...
void Volume::Dump(const char* aLabel) const {
  LOG("%s: Volume: %s (%d) is %s and %s @ %s gen %d locked %d", aLabel,
      NameStr(), Id(), StateStr(), MediaPresent() ? "inserted" : "missing",
      MountPoint().get(), MountGeneration(), (int)IsMountLocked());
  LOG("%s:   Sharing %s Mounting %s Formating %s Unmounting %s", aLabel,
      CanBeShared()
          ? (IsSharingEnabled() ? (IsSharing() ? "en-y" : "en-n") : "dis")
          : "x",
...
void Volume::ResolveAndSetMountPoint(const nsACString& aMountPoint) {
  ...
  if (!realpath(mountPoint.get(), realPathBuf)) {
...
void Volume::SetIsSharing(bool aIsSharing) {
  if (aIsSharing == mIsSharing) {
    return;
  }
  mIsSharing = aIsSharing;
  LOG("Volume %s: IsSharing set to %d state %s", NameStr(), (int)mIsSharing,
      StateStr(mState));
  sEventObserverList.Broadcast(this);
}

void Volume::SetIsFormatting(bool aIsFormatting) {
  if (aIsFormatting == mIsFormatting) {
    return;
  }
  mIsFormatting = aIsFormatting;
  LOG("Volume %s: IsFormatting set to %d state %s", NameStr(),
      (int)mIsFormatting, StateStr(mState));
  if (mIsFormatting) {
    sEventObserverList.Broadcast(this);
  }
}

void Volume::SetIsUnmounting(bool aIsUnmounting) {
  if (aIsUnmounting == mIsUnmounting) {
    return;
  }
  mIsUnmounting = aIsUnmounting;
  LOG("Volume %s: IsUnmounting set to %d state %s", NameStr(),
      (int)mIsUnmounting, StateStr(mState));
  sEventObserverList.Broadcast(this);
}

void Volume::SetIsRemovable(bool aIsRemovable) {
  ...
  LOG("Volume %s: IsRemovable set to %d state %s", NameStr(), (int)mIsRemovable,
      StateStr(mState));
  sEventObserverList.Broadcast(this);
}

void Volume::SetIsHotSwappable(bool aIsHotSwappable) {
  ...
  LOG("Volume %s: IsHotSwappable set to %d state %s", NameStr(),
      (int)mIsHotSwappable, StateStr(mState));
  sEventObserverList.Broadcast(this);
}

bool Volume::BoolConfigValue(const nsCString& aConfigValue, bool& aBoolValue) {
...
void Volume::SetConfig(const nsCString& aConfigName,
                       const nsCString& aConfigValue) {
  if (aConfigName.LowerCaseEqualsLiteral("removable")) {
    bool value = false;
    if (BoolConfigValue(aConfigValue, value)) {
      SetIsRemovable(value);
    } else {
      ERR("Volume %s: invalid value '%s' for configuration '%s'", NameStr(),
          aConfigValue.get(), aConfigName.get());
    }
    return;
  }
...
void Volume::SetMediaPresent(bool aMediaPresent) {
  ...
  //    605 Volume sdcard /mnt/sdcard state changed from 1 (Idle-Unmounted) to 0
  //    (No-Media)
  //
  // And on media insertion, we get:
  //    630 Volume sdcard /mnt/sdcard disk inserted (179:0)
  //    605 Volume sdcard /mnt/sdcard state changed from 0 (No-Media) to 2
  //    (Pending) 605 Volume sdcard /mnt/sdcard state changed from 2 (Pending)
  //    to 1 (Idle-Unmounted)
  //
  // On media removal while the media is mounted:
  //    632 Volume sdcard /mnt/sdcard bad removal (179:1)
  //    605 Volume sdcard /mnt/sdcard state changed from 4 (Mounted) to 5
  //    (Unmounting) 605 Volume sdcard /mnt/sdcard state changed from 5
  //    (Unmounting) to 1 (Idle-Unmounted) 631 Volume sdcard /mnt/sdcard disk
  //    removed (179:0) 605 Volume sdcard /mnt/sdcard state changed from 1
  //    (Idle-Unmounted) to 0 (No-Media)
  //
...
void Volume::SetSharingEnabled(bool aSharingEnabled) {
  mSharingEnabled = aSharingEnabled;

  LOG("SetSharingMode for volume %s to %d canBeShared = %d", NameStr(),
      (int)mSharingEnabled, (int)mCanBeShared);
  sEventObserverList.Broadcast(this);
}

void Volume::SetFormatRequested(bool aFormatRequested) {
  mFormatRequested = aFormatRequested;

  LOG("SetFormatRequested for volume %s to %d CanBeFormatted = %d", NameStr(),
      (int)mFormatRequested, (int)CanBeFormatted());
}

void Volume::SetMountRequested(bool aMountRequested) {
  mMountRequested = aMountRequested;

  LOG("SetMountRequested for volume %s to %d CanBeMounted = %d", NameStr(),
      (int)mMountRequested, (int)CanBeMounted());
}

void Volume::SetUnmountRequested(bool aUnmountRequested) {
  mUnmountRequested = aUnmountRequested;

  LOG("SetUnmountRequested for volume %s to %d CanBeMounted = %d", NameStr(),
      (int)mUnmountRequested, (int)CanBeMounted());
}

void Volume::SetState(Volume::STATE aNewState) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  if (aNewState == mState) {
    return;
  ...
  if (aNewState == nsIVolume::STATE_MOUNTED) {
    ...
    LOG("Volume %s (%u): changing state from %s to %s @ '%s' (%d observers) "
        ...
        NameStr(), mId, StateStr(mState), StateStr(aNewState),
        mMountPoint.get(), sEventObserverList.Length(), mMountGeneration,
        (int)mMountLocked);
  } else {
    LOG("Volume %s (%u): changing state from %s to %s (%d observers)",
        NameStr(), mId, StateStr(mState), StateStr(aNewState),
        sEventObserverList.Length());
  }
...
void Volume::SetMountPoint(const nsACString& aMountPoint) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
...
void Volume::RegisterVolumeObserver(Volume::EventObserver* aObserver,
  ...
  DBG("Added Volume Observer '%s' @%p, length = %u", aName, aObserver,
      sEventObserverList.Length());
...
// static
void Volume::UnregisterVolumeObserver(Volume::EventObserver* aObserver,
                                      const char* aName) {
  MOZ_ASSERT(XRE_IsParentProcess());
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());
  ...
  DBG("Removed Volume Observer '%s' @%p, length = %u", aName, aObserver,
      sEventObserverList.Length());
}

// static
void Volume::UpdateMountLock(const nsACString& aVolumeName,
