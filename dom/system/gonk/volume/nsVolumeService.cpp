...
NS_IMPL_ISUPPORTS(nsVolumeService, nsIVolumeService, nsIDOMMozWakeLockListener)

StaticRefPtr<nsVolumeService> nsVolumeService::sSingleton;

// static
already_AddRefed<nsVolumeService> nsVolumeService::GetSingleton() {
...
void nsVolumeService::Shutdown() {
  ...
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "ShutdownVolumeServiceIOThreadRunnable", ShutdownVolumeServiceIOThread));

  sSingleton = nullptr;
}

nsVolumeService::nsVolumeService()
    : mArrayMonitor("nsVolumeServiceArray"), mGotVolumesFromParent(false) {
  sSingleton = this;
  ...
  XRE_GetIOMessageLoop()->PostTask(NewRunnableFunction(
      "InitVolumeServiceIOThreadRunnable", InitVolumeServiceIOThread, this));
...
NS_IMETHODIMP nsVolumeService::GetVolumeByName(const nsAString& aVolName,
                                               nsIVolume** aResult) {
  MonitorAutoLock autoLock(mArrayMonitor);
...
nsVolumeService::GetVolumeByPath(const nsAString& aPath, nsIVolume** aResult) {
  ...
  while (!realpath(utf8Path.get(), realPathBuf)) {
    if (errno != ENOENT) {
      ERR("GetVolumeByPath: realpath on '%s' failed: %d", utf8Path.get(),
          errno);
      return NSRESULT_FOR_ERRNO();
    }
    // The pathname we were passed doesn't exist, so we try stripping off
    // trailing components until we get a successful call to realpath, or until
    // we run out of components (if we finally get to /something then we also
    // stop).
...
NS_IMETHODIMP
nsVolumeService::CreateOrGetVolumeByPath(const nsAString& aPath,
                                         nsIVolume** aResult) {
  ...
  nsCOMPtr<nsIVolume> vol = new nsVolume(
      NS_LITERAL_STRING("fake"), aPath, nsIVolume::STATE_MOUNTED,
...
nsVolumeService::GetVolumeNames(nsIArray** aVolNames) {
  ...
  *aVolNames = nullptr;
  ...
  for (volIndex = 0; volIndex < numVolumes; volIndex++) {
    ...
    rv = volNames->AppendElement(isupportsString);
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
void nsVolumeService::GetVolumesForIPC(nsTArray<dom::VolumeInfo>* aResult) {
...
void nsVolumeService::RecvVolumesFromParent(
    const nsTArray<dom::VolumeInfo>& aVolumes) {
  if (XRE_IsParentProcess()) {
  ...
  for (uint32_t i = 0; i < aVolumes.Length(); i++) {
    ...
    RefPtr<nsVolume> vol =
        new nsVolume(volInfo.name(), volInfo.mountPoint(), volInfo.volState(),
                     volInfo.mountGeneration(), volInfo.isMediaPresent(),
                     volInfo.isSharing(), volInfo.isFormatting(),
                     volInfo.isFake(), volInfo.isUnmounting(),
                     volInfo.isRemovable(), volInfo.isHotSwappable());
    UpdateVolume(vol, false);
  }
}

NS_IMETHODIMP
nsVolumeService::CreateMountLock(const nsAString& aVolumeName,
                                 nsIVolumeMountLock** aResult) {
  nsCOMPtr<nsIVolumeMountLock> mountLock =
      nsVolumeMountLock::Create(aVolumeName);
  if (!mountLock) {
    return NS_ERROR_NOT_AVAILABLE;
  }
...
already_AddRefed<nsVolume> nsVolumeService::FindVolumeByMountLockName(
    const nsAString& aMountLockName) {
  MonitorAutoLock autoLock(mArrayMonitor);
...
already_AddRefed<nsVolume> nsVolumeService::FindVolumeByName(
    const nsAString& aName, nsVolume::Array::index_type* aIndex) {
  mArrayMonitor.AssertCurrentThreadOwns();
...
void nsVolumeService::UpdateVolume(nsVolume* aVolume, bool aNotifyObservers) {
  MOZ_ASSERT(NS_IsMainThread());
...
NS_IMETHODIMP
nsVolumeService::CreateFakeVolume(const nsAString& name,
                                  const nsAString& path) {
  if (XRE_IsParentProcess()) {
    RefPtr<nsVolume> vol = new nsVolume(
        name, path, nsIVolume::STATE_INIT, -1 /* mountGeneration */,
        true /* isMediaPresent */, false /* isSharing */,
        false /* isFormatting */, true /* isFake */, false /* isUnmounting */,
        false /* isRemovable */, false /* isHotSwappable */);
  ...
  ContentChild::GetSingleton()->SendCreateFakeVolume(nsString(name),
                                                     nsString(path));
  return NS_OK;
}
...
void nsVolumeService::RemoveVolumeByName(const nsAString& aName) {
  {
    MonitorAutoLock autoLock(mArrayMonitor);
    nsVolume::Array::index_type volIndex;
 ...
 public:
  ...
  NS_IMETHOD Run() override {
    ...
    DBG("UpdateVolumeRunnable::Run '%s' state %s gen %d locked %d "
        "media %d sharing %d formatting %d unmounting %d removable %d "
        "hotswappable %d",
