 ...
 * Notify Success.
 */
nsresult MobileConnectionCallback::NotifySuccess(
 ...
 */
nsresult MobileConnectionCallback::NotifySuccessWithString(
...
NS_IMETHODIMP
MobileConnectionCallback::NotifyGetNetworksSuccess(
  ...
  for (uint32_t i = 0; i < aCount; i++) {
    RefPtr<DOMMobileNetworkInfo> networkInfo =
        new DOMMobileNetworkInfo(mWindow);
...
NS_IMETHODIMP
MobileConnectionCallback::NotifyGetCallForwardingSuccess(
...
MobileConnectionCallback::NotifyGetPreferredNetworkTypeSuccess(int32_t aType) {
  MOZ_ASSERT(aType <
             static_cast<int32_t>(MobilePreferredNetworkType::EndGuard_));
  MobilePreferredNetworkType type =
      static_cast<MobilePreferredNetworkType>(aType);
