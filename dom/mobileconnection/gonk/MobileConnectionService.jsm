...
XPCOMUtils.defineLazyModuleGetter(this, "gImsRegService",
                                  "resource://gre/modules/ImsRegService.jsm",
                                  "nsIImsRegService");
...
GsmCellInfo.prototype = {
  ...
  QueryInterface: ChromeUtils.generateQI([Ci.nsICellInfo,
                                         Ci.nsIGsmCellInfo]),
...
MobileConnectionProvider.prototype = {
  ...
  _debug: function(aMessage) {
    ...
    console.log("MobileConnectionProvider[" + this._clientId + "]: " + aMessage + "\n");
  },
  ...
  setCallForwarding: function(aAction, aReason, aNumber, aTimeSeconds,
    ...
    if (this._imsRegHandler.isImsRegistered) {
      ...
      if (this._imsUt) {
        let condition = this.getConditionFromCFReason(aReason);
  ...
  onCallForwardQueried: function(aId, aCfInfos) {
    ...
    for (let i = 0; i < aCfInfos.length ; i++) {
      ...
      cfInfo.reason = this.getCFReasonFromCondition(aCfInfos[i].condition);
  ...
  // Helper functions
  getConditionFromCFReason: function (aReason) {
    let condition = Ci.nsIImsUt.CDIV_CF_UNCONDITIONAL;
    switch(aReason) {
      case Ci.nsIMobileConnection.CALL_FORWARD_REASON_UNCONDITIONAL:
        condition = Ci.nsIImsUt.CDIV_CF_UNCONDITIONAL;
        break;
      case Ci.nsIMobileConnection.CALL_FORWARD_REASON_MOBILE_BUSY:
        condition = Ci.nsIImsUt.CDIV_CF_BUSY;
        break;
      case Ci.nsIMobileConnection.CALL_FORWARD_REASON_NO_REPLY:
        condition = Ci.nsIImsUt.CDIV_CF_NO_REPLY;
        break;
      case Ci.nsIMobileConnection.CALL_FORWARD_REASON_NOT_REACHABLE:
        condition = Ci.nsIImsUt.CDIV_CF_NOT_REACHABLE;
        break;
      case Ci.nsIMobileConnection.CALL_FORWARD_REASON_ALL_CALL_FORWARDING:
        condition = Ci.nsIImsUt.CDIV_CF_ALL;
        break;
      case Ci.nsIMobileConnection.CALL_FORWARD_REASON_ALL_CONDITIONAL_CALL_FORWARDING:
        condition = Ci.nsIImsUt.CDIV_CF_ALL_CONDITIONAL;
        break;
    };
    return condition;
  },

  getCFReasonFromCondition: function (aCondition) {
    let reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_UNKNOWN;
    switch(aCondition) {
      case Ci.nsIImsUt.CDIV_CF_UNCONDITIONAL:
        reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_UNCONDITIONAL;
        break;
      case Ci.nsIImsUt.CDIV_CF_BUSY:
        reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_MOBILE_BUSY;
        break;
      case Ci.nsIImsUt.CDIV_CF_NO_REPLY:
        reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_NO_REPLY;
        break;
      case Ci.nsIImsUt.CDIV_CF_NOT_REACHABLE:
        reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_NOT_REACHABLE;
        break;
      case Ci.nsIImsUt.CDIV_CF_ALL:
        reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_ALL_CALL_FORWARDING;
        break;
      case Ci.nsIImsUt.CDIV_CF_ALL_CONDITIONAL:
        reason = Ci.nsIMobileConnection.CALL_FORWARD_REASON_ALL_CONDITIONAL_CALL_FORWARDING;
        break;
    };
    return reason;
  },
...
MobileConnectionService.prototype = {
  ...
  QueryInterface: ChromeUtils.generateQI([Ci.nsIGonkMobileConnectionService,
