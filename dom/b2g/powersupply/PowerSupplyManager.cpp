 ...
 */
#define POWERSUPPLY_STATUS_CHANGE_NAME u"powersupplystatuschanged"_ns
...
PowerSupplyManager::PowerSupplyManager(nsPIDOMWindowInner* aWindow)
    : DOMEventTargetHelper(aWindow),
      ...
      mPowerSupplyType(u"Unknown"_ns) {}

void PowerSupplyManager::Init() {
...
void PowerSupplyManager::Shutdown() {
  mozilla::hal::UnregisterPowerSupplyObserver(this);
}

JSObject* PowerSupplyManager::WrapObject(JSContext* aCx,
                                         JS::Handle<JSObject*> aGivenProto) {
  return PowerSupplyManager_Binding::Wrap(aCx, this, aGivenProto);
}

bool PowerSupplyManager::PowerSupplyOnline() const {
  return mPowerSupplyOnline;
}

void PowerSupplyManager::GetPowerSupplyType(nsString& aRetVal) const {
  aRetVal = mPowerSupplyType;
}

void PowerSupplyManager::UpdateFromPowerSupplyStatus(
    const hal::PowerSupplyStatus& aPowerSupplyStatus) {
  mPowerSupplyOnline = aPowerSupplyStatus.powerSupplyOnline();
  mPowerSupplyType =
      NS_ConvertUTF8toUTF16(aPowerSupplyStatus.powerSupplyType());
}

void PowerSupplyManager::Notify(
    const hal::PowerSupplyStatus& aPowerSupplyStatus) {
  ...
  if (previousPowerSupplyOnline != mPowerSupplyOnline) {
    ...
    RefPtr<PowerSupplyEvent> event = PowerSupplyEvent::Constructor(
        this, POWERSUPPLY_STATUS_CHANGE_NAME, init);
