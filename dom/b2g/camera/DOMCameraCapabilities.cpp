...
class CameraClosedMessage : public Runnable {
 public:
  explicit CameraClosedMessage(nsMainThreadPtrHandle<T> aListener)
      : Runnable("EncodingRunnable"), mListener(aListener) {
...
class CameraClosedListenerProxy : public CameraControlListener {
 public:
  explicit CameraClosedListenerProxy(T* aListener)
      : mListener(new nsMainThreadPtrHolder<T>("CameraClosedListenerProxy",
                                               aListener)) {
    DOM_CAMERA_LOGT("%s:%d : this=%p\n", __func__, __LINE__, this);
  }
...
JSObject* CameraRecorderVideoProfile::WrapObject(
...
CameraRecorderVideoProfile::CameraRecorderVideoProfile(
  ...
  DOM_CAMERA_LOGI("  video: '%s' %ux%u bps=%u fps=%u\n",
                  NS_ConvertUTF16toUTF8(mCodec).get(), mSize.mWidth,
                  mSize.mHeight, mBitrate, mFramerate);
}
...
JSObject* CameraRecorderAudioProfile::WrapObject(
    JSContext* aCx, JS::Handle<JSObject*> aGivenProto) {
...
CameraRecorderAudioProfile::CameraRecorderAudioProfile(
  ...
  DOM_CAMERA_LOGI("  audio: '%s' bps=%u samples/s=%u channels=%u\n",
                  NS_ConvertUTF16toUTF8(mCodec).get(), mBitrate, mSamplerate,
                  mChannels);
}
 ...
 */
NS_IMPL_CYCLE_COLLECTION_WRAPPERCACHE(CameraRecorderProfile, mParent, mVideo,
                                      mAudio)
...
JSObject* CameraRecorderProfile::WrapObject(JSContext* aCx,
...
JSObject* CameraRecorderProfiles::WrapObject(
    JSContext* aCx, JS::Handle<JSObject*> aGivenProto) {
...
CameraRecorderProfiles::CameraRecorderProfiles(nsISupports* aParent,
    ...
    : mParent(aParent), mCameraControl(aCameraControl) {
  DOM_CAMERA_LOGT("%s:%d : this=%p\n", __func__, __LINE__, this);
  if (mCameraControl) {
...
void CameraRecorderProfiles::GetSupportedNames(nsTArray<nsString>& aNames) {
  DOM_CAMERA_LOGT("%s:%d : this=%p\n", __func__, __LINE__, this);
  if (!mCameraControl) {
    aNames.Clear();
    return;
...
CameraRecorderProfile* CameraRecorderProfiles::NamedGetter(
  ...
  if (!aFound || !profile) {
    RefPtr<ICameraControl::RecorderProfile> p =
...
void CameraRecorderProfiles::OnHardwareClosed() {
  DOM_CAMERA_LOGT("%s:%d : this=%p\n", __func__, __LINE__, this);
  MOZ_ASSERT(NS_IsMainThread());
...
/* static */
bool CameraCapabilities::HasSupport(JSContext* aCx, JSObject* aGlobal) {
  return B2G::HasCameraSupport(aCx, aGlobal);
}

CameraCapabilities::CameraCapabilities(nsPIDOMWindowInner* aWindow,
                                       ICameraControl* aCameraControl)
    : mWindow(aWindow), mCameraControl(aCameraControl) {
  DOM_CAMERA_LOGT("%s:%d : this=%p\n", __func__, __LINE__, this);
  if (mCameraControl) {
...
void CameraCapabilities::OnHardwareClosed() {
  DOM_CAMERA_LOGT("%s:%d : this=%p\n", __func__, __LINE__, this);
  MOZ_ASSERT(NS_IsMainThread());
...
JSObject* CameraCapabilities::WrapObject(JSContext* aCx,
                                         JS::Handle<JSObject*> aGivenProto) {
...
nsresult CameraCapabilities::TranslateToDictionary(
...
void CameraCapabilities::GetPreviewSizes(nsTArray<dom::CameraSize>& aRetVal) {
  nsresult rv =
      TranslateToDictionary(CAMERA_PARAM_SUPPORTED_PREVIEWSIZES, aRetVal);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_PREVIEWSIZES);
}

void CameraCapabilities::GetPictureSizes(nsTArray<dom::CameraSize>& aRetVal) {
  nsresult rv =
      TranslateToDictionary(CAMERA_PARAM_SUPPORTED_PICTURESIZES, aRetVal);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_PICTURESIZES);
}

void CameraCapabilities::GetThumbnailSizes(nsTArray<dom::CameraSize>& aRetVal) {
  nsresult rv = TranslateToDictionary(
      CAMERA_PARAM_SUPPORTED_JPEG_THUMBNAIL_SIZES, aRetVal);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_JPEG_THUMBNAIL_SIZES);
}

void CameraCapabilities::GetVideoSizes(nsTArray<dom::CameraSize>& aRetVal) {
  nsresult rv =
      TranslateToDictionary(CAMERA_PARAM_SUPPORTED_VIDEOSIZES, aRetVal);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_VIDEOSIZES);
}

void CameraCapabilities::GetFileFormats(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }

  nsresult rv =
      mCameraControl->Get(CAMERA_PARAM_SUPPORTED_PICTUREFORMATS, aRetVal);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_PICTUREFORMATS);
}

void CameraCapabilities::GetWhiteBalanceModes(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }

  nsresult rv =
      mCameraControl->Get(CAMERA_PARAM_SUPPORTED_WHITEBALANCES, aRetVal);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_WHITEBALANCES);
}

void CameraCapabilities::GetSceneModes(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }
...
void CameraCapabilities::GetEffects(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }
...
void CameraCapabilities::GetFlashModes(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }
...
void CameraCapabilities::GetFocusModes(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }
...
void CameraCapabilities::GetZoomRatios(nsTArray<double>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }
...
uint32_t CameraCapabilities::MaxFocusAreas() {
  if (NS_WARN_IF(!mCameraControl)) {
    return 0;
  }
  ...
  int32_t areas = 0;
  nsresult rv =
      mCameraControl->Get(CAMERA_PARAM_SUPPORTED_MAXFOCUSAREAS, areas);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_MAXFOCUSAREAS);
  return areas < 0 ? 0 : areas;
}

uint32_t CameraCapabilities::MaxMeteringAreas() {
  if (NS_WARN_IF(!mCameraControl)) {
    return 0;
  }
  ...
  int32_t areas = 0;
  nsresult rv =
      mCameraControl->Get(CAMERA_PARAM_SUPPORTED_MAXMETERINGAREAS, areas);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_MAXMETERINGAREAS);
  return areas < 0 ? 0 : areas;
}

uint32_t CameraCapabilities::MaxDetectedFaces() {
  if (NS_WARN_IF(!mCameraControl)) {
    return 0;
  }
  ...
  int32_t faces = 0;
  nsresult rv =
      mCameraControl->Get(CAMERA_PARAM_SUPPORTED_MAXDETECTEDFACES, faces);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_MAXDETECTEDFACES);
  return faces < 0 ? 0 : faces;
}

double CameraCapabilities::MinExposureCompensation() {
  if (NS_WARN_IF(!mCameraControl)) {
    return 0.0;
  }

  double minEv = 0.0;
  nsresult rv = mCameraControl->Get(
      CAMERA_PARAM_SUPPORTED_MINEXPOSURECOMPENSATION, minEv);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_MINEXPOSURECOMPENSATION);
  return minEv;
}

double CameraCapabilities::MaxExposureCompensation() {
  if (NS_WARN_IF(!mCameraControl)) {
    return 0.0;
  }
  ...
  double maxEv = 0.0;
  nsresult rv = mCameraControl->Get(
      CAMERA_PARAM_SUPPORTED_MAXEXPOSURECOMPENSATION, maxEv);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_MAXEXPOSURECOMPENSATION);
  return maxEv;
}

double CameraCapabilities::ExposureCompensationStep() {
  if (NS_WARN_IF(!mCameraControl)) {
    return 0.0;
  }
  ...
  double evStep = 0.0;
  nsresult rv = mCameraControl->Get(
      CAMERA_PARAM_SUPPORTED_EXPOSURECOMPENSATIONSTEP, evStep);
  LOG_IF_ERROR(rv, CAMERA_PARAM_SUPPORTED_EXPOSURECOMPENSATIONSTEP);
  return evStep;
}

CameraRecorderProfiles* CameraCapabilities::RecorderProfiles() {
  if (NS_WARN_IF(!mCameraControl)) {
    return nullptr;
  }
...
void CameraCapabilities::GetIsoModes(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }
...
void CameraCapabilities::GetMeteringModes(nsTArray<nsString>& aRetVal) {
  if (NS_WARN_IF(!mCameraControl)) {
    return;
  }

  nsresult rv =
      mCameraControl->Get(CAMERA_PARAM_SUPPORTED_METERINGMODES, aRetVal);
