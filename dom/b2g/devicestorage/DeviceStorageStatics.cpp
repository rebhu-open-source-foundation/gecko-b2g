...
// kPrefOverrideRootDir is added in InitDirs().
static const char* gObservedPrefs[] = {
    kPrefTesting,
    kPrefPromptTesting,
    kPrefWritableName,
    nullptr,
...
DeviceStorageStatics::~DeviceStorageStatics() { DS_LOG_INFO(""); }

void DeviceStorageStatics::Init() {
...
void DeviceStorageStatics::InitDirs() {
  MOZ_ASSERT(NS_IsMainThread());
  sMutex.AssertCurrentThreadOwns();
  DS_LOG_INFO("");
  ...
  if (!XRE_IsParentProcess()) {
    ...
    NS_NewLocalFile(locationInfo.apps(), true,
                    getter_AddRefs(sInstance->mDirs[TYPE_APPS]));
    NS_NewLocalFile(locationInfo.crashes(), true,
                    getter_AddRefs(sInstance->mDirs[TYPE_CRASHES]));
    NS_NewLocalFile(locationInfo.pictures(), true,
                    getter_AddRefs(sInstance->mDirs[TYPE_PICTURES]));
    NS_NewLocalFile(locationInfo.videos(), true,
                    getter_AddRefs(sInstance->mDirs[TYPE_VIDEOS]));
    NS_NewLocalFile(locationInfo.music(), true,
...
#  if defined(MOZ_WIDGET_COCOA)
  dirService->Get(NS_OSX_PICTURE_DOCUMENTS_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_PICTURES]));
  dirService->Get(NS_OSX_MOVIE_DOCUMENTS_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_VIDEOS]));
  dirService->Get(NS_OSX_MUSIC_DOCUMENTS_DIR, NS_GET_IID(nsIFile),
...
#  elif defined(XP_UNIX)
  dirService->Get(NS_UNIX_XDG_PICTURES_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_PICTURES]));
  dirService->Get(NS_UNIX_XDG_VIDEOS_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_VIDEOS]));
  dirService->Get(NS_UNIX_XDG_MUSIC_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_MUSIC]));

#  elif defined(XP_WIN)
  dirService->Get(NS_WIN_PICTURES_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_PICTURES]));
  dirService->Get(NS_WIN_VIDEOS_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_VIDEOS]));
  dirService->Get(NS_WIN_MUSIC_DIR, NS_GET_IID(nsIFile),
                  getter_AddRefs(mDirs[TYPE_MUSIC]));
#  endif
...
void DeviceStorageStatics::DumpDirs() {
#ifdef DS_LOGGING
  sMutex.AssertCurrentThreadOwns();
  ...
  for (uint32_t i = 0; i < TYPE_COUNT; ++i) {
    ...
    DS_LOG_INFO("(%s) %s: '%s'", ptStr, storageTypes[i],
                NS_LossyConvertUTF16toASCII(path).get());
  }
#endif
}

void DeviceStorageStatics::Shutdown() {
  MOZ_ASSERT(NS_IsMainThread());
  sMutex.AssertCurrentThreadOwns();
  DS_LOG_INFO("");
...
/* static */ void DeviceStorageStatics::GetDeviceStorageLocationsForIPC(
...
/* static */ void DeviceStorageStatics::GetDirPath(DeviceStorageType aType,
                                                   nsString& aDirPath) {
  aDirPath.Truncate();
...
/* static */ already_AddRefed<nsIFile> DeviceStorageStatics::GetAppsDir() {
  return GetDir(TYPE_APPS);
}

/* static */ already_AddRefed<nsIFile> DeviceStorageStatics::GetCrashesDir() {
  return GetDir(TYPE_CRASHES);
}

/* static */ already_AddRefed<nsIFile> DeviceStorageStatics::GetPicturesDir() {
  return GetDir(TYPE_PICTURES);
}

/* static */ already_AddRefed<nsIFile> DeviceStorageStatics::GetVideosDir() {
  return GetDir(TYPE_VIDEOS);
}

/* static */ already_AddRefed<nsIFile> DeviceStorageStatics::GetMusicDir() {
...
/* static */ void DeviceStorageStatics::SetWritableName(
  ...
  NS_DispatchToMainThread(NS_NewRunnableFunction(
      "DeviceStorageStatics::SetWritableName",
      [name]() -> void { Preferences::SetString(kPrefWritableName, name); }));
}
...
/* static */ void DeviceStorageStatics::AddListener(
  ...
  if (sInstance->mListeners.IsEmpty()) {
    NS_DispatchToMainThread(
        NewRunnableMethod("DeviceStorageStatics::AddListener", sInstance.get(),
                          &DeviceStorageStatics::Register));
  }

  RefPtr<ListenerWrapper> wrapper = new ListenerWrapper(aListener);
...
/* static */ void DeviceStorageStatics::RemoveListener(
  ...
  if (removed && sInstance->mListeners.IsEmpty()) {
    NS_DispatchToMainThread(
        NewRunnableMethod("DeviceStorageStatics::RemoveListener",
                          sInstance.get(), &DeviceStorageStatics::Deregister));
  }
}
...
void DeviceStorageStatics::ResetOverrideRootDir() {
  ...
  // on a real device.
  nsAutoString overrideRootDir;
  mozilla::Preferences::GetString(kPrefOverrideRootDir, overrideRootDir);
  if (!overrideRootDir.IsEmpty()) {
...
DeviceStorageStatics::Observe(nsISupports* aSubject, const char* aTopic,
  ...
  if (!XRE_IsParentProcess()) {
    ...
    ContentChild::GetSingleton()->SendFilePathUpdateNotify(
        dsf->mStorageType, dsf->mStorageName, dsf->mPath,
    ...
    for (size_t i = 0; i < MOZ_ARRAY_LENGTH(kMediaTypes); i++) {
      ...
      if (typeChecker->Check(kMediaTypes[i], dsf->mPath)) {
        if (dsf->mStorageType.Equals(kMediaTypes[i])) {
          ...
          dsf2 = new DeviceStorageFile(kMediaTypes[i], dsf->mStorageName,
                                       dsf->mPath);
        }
...
void DeviceStorageStatics::ListenerWrapper::OnFileWatcherUpdate(
  ...
  nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
      "DeviceStorageStatics::ListenerWrapper::OnFileWatcherUpdate",
      [self, data, file]() -> void {
...
void DeviceStorageStatics::ListenerWrapper::OnDiskSpaceWatcher(
    bool aLowDiskSpace) {
  RefPtr<ListenerWrapper> self = this;
  nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
      "DeviceStorageStatics::ListenerWrapper::OnDiskSpaceWatcher",
      [self, aLowDiskSpace]() -> void {
        RefPtr<nsDOMDeviceStorage> listener = do_QueryReferent(self->mListener);
        if (listener) {
...
void DeviceStorageStatics::ListenerWrapper::OnWritableNameChanged() {
  RefPtr<ListenerWrapper> self = this;
  nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
...
void DeviceStorageStatics::ListenerWrapper::OnVolumeStateChanged(
  ...
  nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
      "DeviceStorageStatics::ListenerWrapper::OnVolumeStateChanged",
      [self, volume]() -> void {
        RefPtr<nsDOMDeviceStorage> listener = do_QueryReferent(self->mListener);
        if (listener) {
