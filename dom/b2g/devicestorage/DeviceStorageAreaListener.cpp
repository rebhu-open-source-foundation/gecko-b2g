    ...
    switch (state) {
      case nsIVolume::STATE_MOUNTED:
        mDeviceStorageAreaListener->DispatchStorageAreaChangedEvent(
            volName, DeviceStorageAreaChangedEventOperation::Added);
        break;
      default:
        mDeviceStorageAreaListener->DispatchStorageAreaChangedEvent(
            volName, DeviceStorageAreaChangedEventOperation::Removed);
        break;
    }
  }
#endif
...
JSObject* DeviceStorageAreaListener::WrapObject(
    JSContext* aCx, JS::Handle<JSObject*> aGivenProto) {
  return DeviceStorageAreaListener_Binding::Wrap(aCx, this, aGivenProto);
}

void DeviceStorageAreaListener::DispatchStorageAreaChangedEvent(
  ...
  if (iter != mStorageAreaStateMap.end() && iter->second == aOperation) {
