...
void DeviceStorageUsedSpaceCache::SetUsedSizes(const nsAString& aStorageName,
...
void DeviceStorageTypeChecker::InitFromBundle(nsIStringBundle* aBundle) {
  aBundle->GetStringFromName(DEVICESTORAGE_PICTURES, mPicturesExtensions);
  aBundle->GetStringFromName(DEVICESTORAGE_MUSIC, mMusicExtensions);
  aBundle->GetStringFromName(DEVICESTORAGE_VIDEOS, mVideosExtensions);
}

bool DeviceStorageTypeChecker::Check(const nsAString& aType, BlobImpl* aBlob) {
  MOZ_ASSERT(aBlob);
...
nsresult DeviceStorageTypeChecker::GetPermissionForType(
    const nsAString& aType, nsACString& aPermissionResult) {
  if (!aType.EqualsLiteral(DEVICESTORAGE_PICTURES) &&
...
class IOEventComplete : public Runnable {
 public:
  IOEventComplete(DeviceStorageFile* aFile, const char* aType)
      : Runnable("devicestorage:IOEventComplete"), mFile(aFile), mType(aType) {}
...
void DeviceStorageFile::Init() {
  DeviceStorageFile::GetRootDirectoryForType(mStorageType, mStorageName,
...
void DeviceStorageFile::GetRootDirectoryForType(const nsAString& aStorageType,
                                                const nsAString& aStorageName,
                                                nsIFile** aFile) {
  nsCOMPtr<nsIFile> f;
  *aFile = nullptr;
...
// static
already_AddRefed<DeviceStorageFile> DeviceStorageFile::CreateUnique(
    nsAString& aFileName, uint32_t aFileType, uint32_t aFileAttributes) {
  ...
  return CreateUnique(storageType, storageName, storagePath, aFileType,
                      aFileAttributes);
}

// static
already_AddRefed<DeviceStorageFile> DeviceStorageFile::CreateUnique(
    const nsAString& aStorageType, const nsAString& aStorageName,
    nsAString& aFileName, uint32_t aFileType, uint32_t aFileAttributes) {
...
void DeviceStorageFile::SetEditable(bool aEditable) { mEditable = aEditable; }

// we want to make sure that the names of file can't reach
// outside of the type of storage the user asked for.
bool DeviceStorageFile::IsSafePath() const {
  return ValidateAndSplitPath(mRootDir) && ValidateAndSplitPath(mPath);
}

bool DeviceStorageFile::ValidateAndSplitPath(const nsAString& aPath,
...
void DeviceStorageFile::AppendRelativePath(const nsAString& aPath) {
  if (!mFile) {
    return;
  }
...
nsresult DeviceStorageFile::CreateFileDescriptor(
  ...
  ScopedPRFileDesc fd;
  nsresult rv =
      mFile->OpenNSPRFileDesc(PR_RDWR | PR_CREATE_FILE, 0660, &fd.rwget());
...
nsresult DeviceStorageFile::CalculateSizeAndModifiedDate() {
  MOZ_ASSERT(!NS_IsMainThread());
...
void DeviceStorageFile::CollectFiles(
...
void DeviceStorageFile::collectFilesInternal(
  ...
  nsCOMPtr<nsIDirectoryEnumerator> files = e;
  nsCOMPtr<nsIFile> f;
...
void DeviceStorageFile::AccumDiskUsage(uint64_t* aPicturesSoFar,
  ...
  if (DeviceStorageTypeChecker::IsVolumeBased(mStorageType)) {
    DeviceStorageUsedSpaceCache* usedSpaceCache =
        DeviceStorageUsedSpaceCache::CreateOrGet();
    MOZ_ASSERT(usedSpaceCache);
    nsresult rv = usedSpaceCache->AccumUsedSizes(
        mStorageName, aPicturesSoFar, aVideosSoFar, aMusicSoFar, aTotalSoFar);
    if (NS_SUCCEEDED(rv)) {
      return;
    }
    AccumDirectoryUsage(mFile, &pictureUsage, &videoUsage, &musicUsage,
                        &totalUsage);
...
void DeviceStorageFile::AccumDirectoryUsage(nsIFile* aFile,
...
bool DeviceStorageFile::IsAvailable() {
  nsString status;
  GetStatus(status);
  return status.EqualsLiteral("available");
}

void DeviceStorageFile::DoFormat(nsAString& aStatus) {
  DeviceStorageTypeChecker* typeChecker =
      DeviceStorageTypeChecker::CreateOrGet();
...
void DeviceStorageFile::DoMount(nsAString& aStatus) {
  DeviceStorageTypeChecker* typeChecker =
      DeviceStorageTypeChecker::CreateOrGet();
  if (!typeChecker || !mFile) {
    return;
...
void DeviceStorageFile::DoUnmount(nsAString& aStatus) {
  DeviceStorageTypeChecker* typeChecker =
      DeviceStorageTypeChecker::CreateOrGet();
  if (!typeChecker || !mFile) {
    return;
...
void nsDOMDeviceStorage::SetRootDirectoryForType(
    const nsAString& aStorageType, const nsAString& aStorageName) {
  MOZ_ASSERT(NS_IsMainThread());

  nsCOMPtr<nsIFile> f;
  DeviceStorageFile::GetRootDirectoryForType(aStorageType, aStorageName,
                                             getter_AddRefs(f));
  mRootDirectory = f;
  mStorageType = aStorageType;
  mStorageName = aStorageName;
...
void DeviceStorageRequest::Initialize(
    DeviceStorageRequestManager* aManager,
    already_AddRefed<DeviceStorageFile>&& aFile, uint32_t aId) {
...
void DeviceStorageRequest::Initialize(
    DeviceStorageRequestManager* aManager,
    already_AddRefed<DeviceStorageFile>&& aFile, uint32_t aRequest,
    BlobImpl* aBlob) {
  Initialize(aManager, std::move(aFile), aRequest);
  mBlob = aBlob;
  mCheckBlob = true;
  mCheckFile = true;
  MOZ_ASSERT(mBlob);
...
DeviceStorageAccessType DeviceStorageRequest::GetAccess() const {
  return mAccess;
}

void DeviceStorageRequest::GetStorageType(nsAString& aType) const {
  aType = mFile->mStorageType;
}

nsresult DeviceStorageRequest::Cancel() {
  return Reject(POST_ERROR_EVENT_PERMISSION_DENIED);
}

nsresult DeviceStorageRequest::Allow() {
  if (mUseMainThread && !NS_IsMainThread()) {
    RefPtr<DeviceStorageRequest> self = this;
    nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
        "DeviceStorageRequest::Allow()", [self]() -> void { self->Allow(); });
...
DeviceStorageFile* DeviceStorageRequest::GetFile() const {
  MOZ_ASSERT(mFile);
  return mFile;
}

DeviceStorageFileDescriptor* DeviceStorageRequest::GetFileDescriptor() const {
  MOZ_ASSERT(mDSFileDescriptor);
  return mDSFileDescriptor;
}

DeviceStorageRequestManager* DeviceStorageRequest::GetManager() const {
  return mManager;
}

nsresult DeviceStorageRequest::Prepare() { return NS_OK; }

nsresult DeviceStorageRequest::CreateSendParams(DeviceStorageParams& aParams) {
  MOZ_ASSERT_UNREACHABLE("Cannot send to parent, missing param creator");
  return NS_ERROR_UNEXPECTED;
}

nsresult DeviceStorageRequest::AllowInternal() {
  ...
  if (mCheckBlob &&
      (!mBlob || !typeChecker->Check(mFile->mStorageType, mBlob))) {
...
nsresult DeviceStorageRequest::SendToParentProcess() {
  // PContent can only be used on the main thread
  if (!NS_IsMainThread()) {
    RefPtr<DeviceStorageRequest> self = this;
    nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
        "DeviceStorageRequest::SendToParentProcess()", [self]() -> void {
...
void DeviceStorageCursorRequest::Initialize(
    DeviceStorageRequestManager* aManager,
    already_AddRefed<DeviceStorageFile>&& aFile, uint32_t aRequest,
    PRTime aSince) {
  Initialize(aManager, std::move(aFile), aRequest);
...
nsresult DeviceStorageCursorRequest::SendContinueToParentProcess() {
  ...
  DeviceStorageRequestChild* child = new DeviceStorageRequestChild(this);
  DeviceStorageGetParams params(mStorageType, mFile->mStorageName,
                                mFile->mRootDir, mFile->mPath);
  ContentChild::GetSingleton()->SendPDeviceStorageRequestConstructor(child,
                                                                     params);
  return NS_OK;
}
...
nsresult DeviceStorageCursorRequest::Continue() {
  if (!NS_IsMainThread()) {
    ...
    nsCOMPtr<nsIRunnable> r =
        NS_NewRunnableFunction("DeviceStorageCursorRequest::Continue()",
                               [self]() -> void { self->Continue(); });
...
nsresult DeviceStorageCursorRequest::CreateSendParams(
    DeviceStorageParams& aParams) {
  DeviceStorageEnumerationParams params(
      mFile->mStorageType, mFile->mStorageName, mFile->mRootDir, mSince);
  aParams = params;
  return NS_OK;
}

class DeviceStorageCreateFdRequest final : public DeviceStorageRequest {
 public:
  ...
  NS_IMETHOD Run() override {
    ...
    nsresult rv =
        mFile->CreateFileDescriptor(mDSFileDescriptor->mFileDescriptor);
...
class DeviceStorageCreateRequest final : public DeviceStorageRequest {
 public:
  ...
  NS_IMETHOD Run() override {
    ...
    mBlob->CreateInputStream(getter_AddRefs(stream), rv);
  ...
  void Initialize(DeviceStorageRequestManager* aManager,
    ...
    DeviceStorageRequest::Initialize(aManager, std::move(aFile), aRequest);
 ...
 protected:
  nsresult Prepare() override {
    ...
    RefPtr<DeviceStorageFile> file = DeviceStorageFile::CreateUnique(
        mFile->mStorageType, mFile->mStorageName, path,
...
class DeviceStorageAppendRequest final : public DeviceStorageRequest {
 public:
...
class DeviceStorageOpenRequest final : public DeviceStorageRequest {
 public:
  using DeviceStorageRequest::Initialize;
 ...
 protected:
  ...
  nsresult CreateSendParams(DeviceStorageParams& aParams) override {
    DeviceStorageGetParams params(mFile->mStorageType, mFile->mStorageName,
                                  mFile->mRootDir, mFile->mPath);
    aParams = params;
    return NS_OK;
  }
...
class DeviceStorageDeleteRequest final : public DeviceStorageRequest {
 public:
...
class DeviceStorageFreeSpaceRequest final : public DeviceStorageRequest {
 public:
...
class DeviceStorageUsedSpaceRequest final : public DeviceStorageRequest {
 public:
  ...
  NS_IMETHOD Run() override {
    ...
    mFile->AccumDiskUsage(&picturesUsage, &videosUsage, &musicUsage,
                          &totalUsage);
...
class DeviceStorageAvailableRequest final : public DeviceStorageRequest {
 public:
...
class DeviceStorageStatusRequest final : public DeviceStorageRequest {
 public:
...
class DeviceStorageWatchRequest final : public DeviceStorageRequest {
 public:
  ...
  NS_IMETHOD Run() override { return Resolve(); }
};

class DeviceStorageFormatRequest final : public DeviceStorageRequest {
 public:
 ...
 protected:
  nsresult CreateSendParams(DeviceStorageParams& aParams) override {
    DeviceStorageFormatParams params(mFile->mStorageType, mFile->mStorageName);
    aParams = params;
    return NS_OK;
  }
};

class DeviceStorageMountRequest final : public DeviceStorageRequest {
 public:
 ...
 protected:
  nsresult CreateSendParams(DeviceStorageParams& aParams) override {
    DeviceStorageMountParams params(mFile->mStorageType, mFile->mStorageName);
    aParams = params;
    return NS_OK;
  }
};

class DeviceStorageUnmountRequest final : public DeviceStorageRequest {
 public:
 ...
 protected:
  nsresult CreateSendParams(DeviceStorageParams& aParams) override {
    DeviceStorageUnmountParams params(mFile->mStorageType, mFile->mStorageName);
    aParams = params;
    return NS_OK;
  }
};

class DeviceStoragePermissionCheck final : public ContentPermissionRequestBase,
                                           public nsIRunnable {
 public:
  ...
  NS_IMETHOD Cancel() override { return Resolve(false); }
...
NS_IMPL_ADDREF_INHERITED(DeviceStoragePermissionCheck,
                         ContentPermissionRequestBase)
NS_IMPL_RELEASE_INHERITED(DeviceStoragePermissionCheck,
                          ContentPermissionRequestBase)
NS_IMPL_QUERY_INTERFACE_INHERITED(DeviceStoragePermissionCheck,
                                  ContentPermissionRequestBase, nsIRunnable)

NS_INTERFACE_MAP_BEGIN_CYCLE_COLLECTION(nsDOMDeviceStorage)
...
nsresult nsDOMDeviceStorage::CheckPermission(
    already_AddRefed<DeviceStorageRequest>&& aRequest) {
  MOZ_ASSERT(mManager);
...
bool nsDOMDeviceStorage::IsOwningThread() {
  bool owner = false;
  mOwningThread->IsOnCurrentThread(&owner);
  return owner;
}

nsresult nsDOMDeviceStorage::DispatchToOwningThread(
    already_AddRefed<nsIRunnable>&& aRunnable) {
  return mOwningThread->Dispatch(std::move(aRunnable), NS_DISPATCH_NORMAL);
}

/* virtual */ JSObject* nsDOMDeviceStorage::WrapObject(
    JSContext* aCx, JS::Handle<JSObject*> aGivenProto) {
  return DeviceStorage_Binding::Wrap(aCx, this, aGivenProto);
}

nsresult nsDOMDeviceStorage::Init(nsPIDOMWindowInner* aWindow,
  ...
  rv = CheckPrincipal(aWindow, aType.EqualsLiteral(DEVICESTORAGE_APPS),
...
// static
nsresult nsDOMDeviceStorage::CheckPrincipal(nsPIDOMWindowInner* aWindow,
                                            bool aIsAppsStorage,
...
void nsDOMDeviceStorage::Shutdown() {
  MOZ_ASSERT(IsOwningThread());
...
// static
void nsDOMDeviceStorage::GetOrderedVolumeNames(
    const nsAString& aType, nsDOMDeviceStorage::VolumeNameArray& aVolumeNames) {
...
// static
void nsDOMDeviceStorage::CreateDeviceStorageFor(nsPIDOMWindowInner* aWin,
                                                const nsAString& aType,
...
// static
void nsDOMDeviceStorage::CreateDeviceStorageByNameAndType(
    nsPIDOMWindowInner* aWin, const nsAString& aName, const nsAString& aType,
    nsDOMDeviceStorage** aStore) {
  if (!DeviceStorageTypeChecker::IsVolumeBased(aType)) {
  ...
  RefPtr<nsDOMDeviceStorage> storage =
      GetStorageByNameAndType(aWin, aName, aType);
  if (!storage) {
    *aStore = nullptr;
    return;
  }
...
bool nsDOMDeviceStorage::Equals(nsPIDOMWindowInner* aWin,
                                const nsAString& aName,
                                const nsAString& aType) {
  MOZ_ASSERT(aWin);

  return aWin && aWin->WindowID() == mInnerWindowID &&
         mStorageName.Equals(aName) && mStorageType.Equals(aType);
}

// static
bool nsDOMDeviceStorage::ParseFullPath(const nsAString& aFullPath,
...
already_AddRefed<nsDOMDeviceStorage> nsDOMDeviceStorage::GetStorage(
    const nsAString& aFullPath, nsAString& aOutStoragePath) {
  nsString storageName;
...
already_AddRefed<nsDOMDeviceStorage> nsDOMDeviceStorage::GetStorageByName(
    const nsAString& aStorageName) {
  MOZ_ASSERT(IsOwningThread());
...
void nsDOMDeviceStorage::GetDefaultStorageName(const nsAString& aStorageType,
  ...
  if (!prefStorageName.IsEmpty()) {
    nsString status;
    RefPtr<DeviceStorageFile> dsf =
        new DeviceStorageFile(aStorageType, prefStorageName);
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::AppendNamed(
    Blob* aBlob, const nsAString& aPath, ErrorResult& aRv) {
  if (aPath.IsEmpty()) {
    aRv.Throw(NS_ERROR_ILLEGAL_VALUE);
    return nullptr;
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::AddOrAppendNamed(
    Blob* aBlob, const nsAString& aPath, bool aCreate, ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::GetInternal(
  ...
  RefPtr<DeviceStorageFile> dsf =
      new DeviceStorageFile(mStorageType, mStorageName, aPath);
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::Delete(const nsAString& aPath,
                                                        ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::UsedSpace(ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::Available(ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::StorageStatus(
    ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::Format(ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::Mount(ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::Unmount(ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
already_AddRefed<DOMRequest> nsDOMDeviceStorage::CreateFileDescriptor(
    const nsAString& aPath, DeviceStorageFileDescriptor* aDSFileDescriptor,
    ErrorResult& aRv) {
  MOZ_ASSERT(IsOwningThread());
...
bool nsDOMDeviceStorage::Default() {
...
bool nsDOMDeviceStorage::CanBeFormatted() {
  // Currently, any volume which can be shared can also be formatted.
  return mIsShareable;
}

bool nsDOMDeviceStorage::CanBeMounted() {
  // Currently, any volume which can be shared can also be mounted/unmounted.
  return mIsShareable;
}

bool nsDOMDeviceStorage::CanBeShared() { return mIsShareable; }

bool nsDOMDeviceStorage::IsRemovable() { return mIsRemovable; }

bool nsDOMDeviceStorage::LowDiskSpace() {
  return DeviceStorageStatics::LowDiskSpace();
}

already_AddRefed<Promise> nsDOMDeviceStorage::GetRoot(ErrorResult& aRv) {
  nsString storagePath;
...
#ifdef MOZ_WIDGET_GONK
void nsDOMDeviceStorage::DispatchStatusChangeEvent(nsAString& aStatus) {
...
void nsDOMDeviceStorage::DispatchStorageStatusChangeEvent(
    nsAString& aStorageStatus) {
...
void nsDOMDeviceStorage::OnFileWatcherUpdate(const nsCString& aData,
                                             DeviceStorageFile* aFile) {
  MOZ_ASSERT(IsOwningThread());
  Notify(aData.get(), aFile);
}

void nsDOMDeviceStorage::OnDiskSpaceWatcher(bool aLowDiskSpace) {
  MOZ_ASSERT(IsOwningThread());
  RefPtr<DeviceStorageFile> file =
      new DeviceStorageFile(mStorageType, mStorageName);
  if (aLowDiskSpace) {
...
#ifdef MOZ_WIDGET_GONK
void nsDOMDeviceStorage::OnVolumeStateChanged(nsIVolume* aVolume) {
  MOZ_ASSERT(IsOwningThread());
...
nsresult nsDOMDeviceStorage::Notify(const char* aReason,
                                    DeviceStorageFile* aFile) {
  if (!mManager) {
    return NS_OK;
  }
  ...
  CopyASCIItoUTF16(mozilla::MakeStringSpan(aReason), init.mReason);
...
void nsDOMDeviceStorage::EventListenerAdded(nsAtom* aType) {
  MOZ_ASSERT(IsOwningThread());
  ...
  if (aType != nsGkAtoms::onchange) {
    return;
  }

  ErrorResult rv;
  RefPtr<DOMRequest> domRequest;
  uint32_t id = CreateDOMRequest(getter_AddRefs(domRequest), rv);
  if (rv.Failed()) {
    return;
  }
  ...
  request->Initialize(mManager, dsf.forget(), id);
  rv = CheckPermission(request.forget());
}
...
void DeviceStorageRequestManager::StorePermission(size_t aAccess, bool aAllow) {
  MOZ_ASSERT(NS_IsMainThread());
...
uint32_t DeviceStorageRequestManager::CheckPermission(size_t aAccess) {
...
bool DeviceStorageRequestManager::IsOwningThread() {
  bool owner = false;
  mOwningThread->IsOnCurrentThread(&owner);
  return owner;
}

nsresult DeviceStorageRequestManager::DispatchToOwningThread(
    already_AddRefed<nsIRunnable>&& aRunnable) {
  return mOwningThread->Dispatch(std::move(aRunnable), NS_DISPATCH_NORMAL);
}

nsresult DeviceStorageRequestManager::DispatchOrAbandon(
  ...
  nsresult rv = DispatchToOwningThread(std::move(aRunnable));
  if (NS_WARN_IF(NS_FAILED(rv))) {
...
nsresult DeviceStorageRequestManager::Resolve(uint32_t aId,
  ...
  if (aForceDispatch || !IsOwningThread()) {
    ...
    nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
        "DeviceStorageRequestManager::Resolve",
        [self, aId]() -> void { self->Resolve(aId, false); });
...
nsresult DeviceStorageRequestManager::Resolve(uint32_t aId,
  ...
  if (aForceDispatch || !IsOwningThread()) {
    ...
    nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
        "DeviceStorageRequestManager::Resolve(nsString)",
        [self, aId, result]() -> void { self->Resolve(aId, result, false); });
    return DispatchOrAbandon(aId, r.forget());
  }
...
nsresult DeviceStorageRequestManager::Resolve(uint32_t aId, uint64_t aValue,
  ...
  if (aForceDispatch || !IsOwningThread()) {
    ...
    RefPtr<DeviceStorageRequestManager> self = this;
    nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
        "DeviceStorageRequestManager::Resolve(uint64_t)",
...
nsresult DeviceStorageRequestManager::Resolve(uint32_t aId, BlobImpl* aBlobImpl,
  ...
  if (aForceDispatch || !IsOwningThread()) {
    ...
    nsCOMPtr<nsIRunnable> r =
        NS_NewRunnableFunction("DeviceStorageRequestManager::Resolve(BlobImpl)",
                               [self, aId, blobImpl]() -> void {
...
nsresult DeviceStorageRequestManager::ResolveInternal(ListIndex aIndex,
...
nsresult DeviceStorageRequestManager::RejectInternal(
    DeviceStorageRequestManager::ListIndex aIndex, const nsString& aReason) {
  MOZ_ASSERT(IsOwningThread());
  MOZ_ASSERT(!mShutdown);
...
nsresult DeviceStorageRequestManager::Reject(uint32_t aId,
  ...
  nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
      "DeviceStorageRequestManager::Reject", [self, aId, reason]() -> void {
        DS_LOG_INFO("posted %u w/ %s", aId,
...
DeviceStorageRequestManager::ListIndex DeviceStorageRequestManager::Find(
    uint32_t aId) {
  MOZ_ASSERT(IsOwningThread());
  ListIndex i = mPending.Length();
  while (i > 0) {
