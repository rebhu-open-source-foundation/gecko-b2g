...
var gTestSuite = (function() {
  ...
  function waitForMobileConnectionEventOnce(aEventName, aServiceId) {
    ...
    mobileconnection.addEventListener(
      aEventName,
      function onevent(aEvent) {
    ...
    return waitForWifiManagerEventOnce("statuschange").then(
      function onstatuschange(event) {
        ...
        if (
          ...
          return null; // Got expected 'connected' event from aNetwork.ssid.
        }
    ...
    function startOneHostapd(aIndex) {
      ...
      return writeHostapdConfFile(
        ...
        .then(function(reply) {
          ...
          if (reply[0].includes("bind(PF_UNIX): Address already in use")) {
    ...
    return runEmulatorShellSafe(["getprop", "init.svc." + aServiceName]).then(
      function(result) {
        if ("running" !== result[0] && "stopped" !== result[0]) {
          throw new Error(
            'Init service running state should be "running" or "stopped".'
          );
        }
  ...
  function startStopInitService(aServiceName, aStart) {
    ...
    return runEmulatorShellSafe([aStart ? "start" : "stop", aServiceName])
      ...
      .then(function onIsServiceRunningResolved(aIsRunning) {
        ...
        throw new Error(
          "Failed to " + (aStart ? "start" : "stop") + " " + aServiceName
        );
      });
  }
  ...
  function verifyNumOfProcesses(aProcessName, aExpectedNum) {
    return getProcessDetail(aProcessName).then(function(detail) {
      ...
      throw new Error(
        "Unexpected number of running processes:" +
          aProcessName +
          ", expected: " +
          aExpectedNum +
          ", actual: " +
  ...
  function initTestEnvironment() {
    return addRequiredPermissions()
      .then(function() {
        ...
        if (!wifiManager) {
          throw new Error("window.navigator.mozWifiManager is NULL");
        }
