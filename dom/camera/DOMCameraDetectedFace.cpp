...
NS_IMPL_CYCLE_COLLECTION_WRAPPERCACHE(DOMCameraDetectedFace, mParent, mBounds,
                                      mLeftEye, mRightEye, mMouth)
...
/* static */
bool DOMCameraDetectedFace::HasSupport(JSContext* aCx, JSObject* aGlobal) {
  return B2G::HasCameraSupport(aCx, aGlobal);
}

JSObject* DOMCameraDetectedFace::WrapObject(JSContext* aCx,
                                            JS::Handle<JSObject*> aGivenProto) {
  return CameraDetectedFace_Binding::Wrap(aCx, this, aGivenProto);
}

/* static */
already_AddRefed<DOMCameraDetectedFace> DOMCameraDetectedFace::Constructor(
...
DOMCameraDetectedFace::DOMCameraDetectedFace(
    nsISupports* aParent, const dom::CameraDetectedFaceInit& aFace)
    : mParent(aParent),
      mId(aFace.mId),
      mScore(aFace.mScore),
...
DOMCameraDetectedFace::DOMCameraDetectedFace(nsISupports* aParent,
    ...
    : mParent(aParent),
      ...
      mScore(aFace.score),
      mBounds(new DOMRect(this)) {
  mBounds->SetRect(aFace.bound.left, aFace.bound.top,
