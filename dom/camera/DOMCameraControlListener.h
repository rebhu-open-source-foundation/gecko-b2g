...
class DOMCameraControlListener : public CameraControlListener {
 public:
  ...
  virtual void OnFacesDetected(
      const nsTArray<ICameraControl::Face>& aFaces) override;
  ...
  virtual void OnRecorderStateChange(RecorderState aState, int32_t aStatus,
                                     int32_t aTrackNum) override;
  virtual void OnConfigurationChange(
      const CameraListenerConfiguration& aConfiguration) override;
  virtual void OnShutter() override;
  virtual void OnRateLimitPreview(bool aLimit) override;
  virtual bool OnNewPreviewFrame(layers::Image* aImage, uint32_t aWidth,
