...
enum output_format { OUTPUT_FORMAT_THREE_GPP, OUTPUT_FORMAT_MPEG_4 };
...
enum error_t { OK = 0, UNKNOWN_ERROR, INVALID_OPERATION };

enum camera_msg_t { CAMERA_MSG_SHUTTER, CAMERA_MSG_COMPRESSED_IMAGE };
...
enum camera_facing_t { CAMERA_FACING_BACK, CAMERA_FACING_FRONT };
...
class MediaProfiles final {
 public:
  ...
  int getCamcorderProfileParamByName(const char* aParameter, int aCameraId,
                                     camcorder_quality aQuality) const {
    switch (aQuality) {
      case CAMCORDER_QUALITY_LOW:
      case CAMCORDER_QUALITY_QVGA:
