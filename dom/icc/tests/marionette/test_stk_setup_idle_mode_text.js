...
const TEST_DATA = [
  {
    ...
    expect: { commandQualifier: 0x00, text: "Idle Mode Text" },
  },
  {
    command:
      "D081FD" + // Length
    ...
    expect: {
      ...
      text:
        "The SIM shall supply a text string, which shall be " +
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x00, text: "Idle Mode Text 1" },
  },
  {
    command:
      "D010" + // Length
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testSetupIdleModeText(aEvent.command, data.expect)
        )
      );
