...
/* global MozIccManager, is, log, startTestCommon, getMozIcc, waitForTargetEvent
   ...
   COLOR_ICON, COLOR_TRANSPARENCY_ICON */
...
const TEST_DATA = [
  // Open channel.
  {
    ...
    expect: {
      typeOfCommand: MozIccManager.STK_CMD_OPEN_CHANNEL,
  ...
  // Close channel.
  {
    command:
      ...
      "9E020007", // Icon identifier
    expect: {
      typeOfCommand: MozIccManager.STK_CMD_CLOSE_CHANNEL,
  ...
  // Receive data.
  {
    ...
    expect: {
      typeOfCommand: MozIccManager.STK_CMD_RECEIVE_DATA,
      commandQualifier: 0x00,
    },
  },
  {
  ...
  // Send data.
  {
    ...
    expect: {
      typeOfCommand: MozIccManager.STK_CMD_SEND_DATA,
      commandQualifier: 0x01,
      iconSelfExplanatory: true,
      icons: [COLOR_ICON, COLOR_TRANSPARENCY_ICON],
    },
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testBipCommand(aEvent.command, data.expect)
        )
      );
