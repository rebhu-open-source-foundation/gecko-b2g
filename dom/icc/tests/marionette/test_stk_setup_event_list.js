...
const TEST_DATA = [
  {
    command:
      ...
      "990104", // Event list
    expect: { commandQualifier: 0x00, eventList: [4] },
  },
  {
    command:
      "D00D" + // Length
      ...
      "82028182" + // Device identities
      "99020507", // Event list
    expect: { commandQualifier: 0x00, eventList: [5, 7] },
  },
  {
    command:
      "D00C" + // Length
      ...
      "82028182" + // Device identities
      "990107", // Event list
    expect: { commandQualifier: 0x00, eventList: [7] },
  },
  {
    command:
      "D00C" + // Length
  ...
  },
  {
    command:
      ...
      "82028182" + // Device identities
      "9900", // Event list
    expect: { commandQualifier: 0x00, eventList: null },
  },
  {
    command:
      "D00C" + // Length
...
function testSetupEventList(aCommand, aExpect) {
  ...
  for (let index in aExpect.eventList) {
    is(
      aCommand.options.eventList[index],
      aExpect.eventList[index],
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testSetupEventList(aEvent.command, data.expect)
        )
      );
