...
/* global MozIccManager, is, log, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu, isIcons,
   BASIC_ICON, COLOR_TRANSPARENCY_ICON, COLOR_ICON */
...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: {
      ...
      title: "LargeMenu2",
      items: [
        { identifier: 255, text: "1 Call Forward Unconditional" },
  ...
  },
  {
    ...
    expect: {
      commandQualifier: 0x00,
      title:
        "The SIM shall supply a set of menu items, which shall " +
  ...
  },
  {
    ...
    expect: {
      ...
      title: "ЗДРАВСТВУЙТЕ",
      items: [
        { identifier: 1, text: "ЗДРАВСТВУЙТЕ1" },
  ...
  // Test remove setup menu.
  {
    command:
      ...
      "8F00", // Item
    expect: { commandQualifier: 0x00, title: "", items: [null] },
  },
];
...
function testSetupMenu(aCommand, aExpect) {
  ...
  is(
    aCommand.options.isHelpAvailable,
    !!(aCommand.commandQualifier & 0x80),
    "isHelpAvailable"
  );
  ...
  for (let index in aExpect.items) {
    ...
    if (!itemExpect) {
      ...
      is(
        item.identifier,
        itemExpect.identifier,
        "options.items[" + index + "].identifier"
      );
      is(item.text, itemExpect.text, "options.items[" + index + "].text");

      if (itemExpect.icons) {
        isIcons(item.icons, itemExpect.icons);
        is(
          item.iconSelfExplanatory,
          itemExpect.iconSelfExplanatory,
  ...
  if ("icons" in aExpect) {
    ...
    is(
      aCommand.options.iconSelfExplanatory,
      aExpect.iconSelfExplanatory,
  ...
  if ("nextActionList" in aExpect) {
    for (let index in aExpect.nextActionList) {
      is(
        aCommand.options.nextActionList[index],
        aExpect.nextActionList[index],
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testSetupMenu(aEvent.command, data.expect)
        )
      );
