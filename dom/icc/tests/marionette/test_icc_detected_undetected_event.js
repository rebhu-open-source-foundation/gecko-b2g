...
/* global MozIccManager, is, log, ok, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu, iccManager, setRadioEnabled,
   getMozMobileConnectionByServiceId, MozIcc */
  ...
  return (
    Promise.resolve()
      ...
      .then(() => {
        ...
        promises.push(
          waitForTargetEvent(iccManager, "iccundetected").then(aEvt => {
            is(aEvt.iccId, iccId, "icc " + aEvt.iccId + " becomes undetected");
            is(
              iccManager.iccIds.length,
              origNumIccs - 1,
              "iccIds.length becomes to " + iccManager.iccIds.length
            );
            is(
              iccManager.getIccById(aEvt.iccId),
              null,
      ...
      .then(() => {
        ...
        promises.push(
          waitForTargetEvent(iccManager, "iccdetected").then(aEvt => {
            ...
            ok(
              iccManager.getIccById(aEvt.iccId) instanceof MozIcc,
              "should get a valid icc object here"
            );

            // The mozMobileConnection.iccId should be in sync.
            is(
              mobileConnection.iccId,
              iccId,
