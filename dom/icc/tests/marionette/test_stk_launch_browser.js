...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: {
      ...
      url: "",
      mode: MozIccManager.STK_BROWSER_MODE_LAUNCH_IF_NOT_ALREADY_LAUNCHED,
      confirmMessage: { iconSelfExplanatory: true, icons: [COLOR_ICON] },
    },
  },
];
...
function testLaunchBrowser(aCommand, aExpect) {
  ...
  if ("confirmMessage" in aExpect) {
    isStkText(
      aCommand.options.confirmMessage,
      aExpect.confirmMessage,
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testLaunchBrowser(aEvent.command, data.expect)
        )
      );
