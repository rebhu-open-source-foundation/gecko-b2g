...
const TEST_DATA = [
  {
    ...
    expect: { commandQualifier: 0x00, text: "7-bit USSD" },
  },
  {
    command:
      "D044" + // Length
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x00, text: "8-bit USSD" },
  },
  {
    command:
      "D02F" + // Length
    ...
    expect: { commandQualifier: 0x00, text: "UCS2 USSD" },
  },
  {
    command:
      "D081FD" + // Length
    ...
    expect: {
      ...
      text:
        "once a RELEASE COMPLETE message containing the USSD " +
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          verifySendUSSD(aEvent.command, data.expect)
        )
      );
