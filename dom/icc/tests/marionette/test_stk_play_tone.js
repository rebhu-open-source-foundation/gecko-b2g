...
/* global MozIccManager, is, log, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu, isIcons, BASIC_ICON, COLOR_ICON */
...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x01, text: "Play Tone" },
  },
  {
    command:
      "D00B" + // Length
    ...
    expect: { commandQualifier: 0x01, text: "" },
  },
  {
    command:
      "D00C" + // Length
      ...
      "82028182" + // Device identities
      "8E0101", // Tone
    expect: { commandQualifier: 0x00, tone: 0x01 },
  },
  {
    command:
      "D00D" + // Length
...
function testPlayTone(aCommand, aExpect) {
  ...
  if ("duration" in aExpect) {
    ...
    is(
      duration.timeUnit,
      aExpect.duration.timeUnit,
      "options.duration.timeUnit"
    );
    is(
      duration.timeInterval,
      aExpect.duration.timeInterval,
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testPlayTone(aEvent.command, data.expect)
        )
      );
