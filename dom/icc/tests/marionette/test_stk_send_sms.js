...
/* global MozIccManager, is, ok, log, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu, isIcons, workingFrame, BASIC_ICON */
...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x00, text: "Short Message" },
  },
  {
    command:
      "D081FD" + // Length
  ...
  },
  {
    ...
    expect: {
      ...
      text:
        "Two types are defined: - A short message to be sent to " +
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x00, text: "Text Attribute 1" },
  },
  {
    command:
      "D035" + // Length
    ...
    expect: {
      commandQualifier: 0x00, // 3GPP-SMS TPDU
      text: "80ル0",
    },
  },
  {
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          verifySendSMS(aEvent.command, data.expect)
        )
      );
