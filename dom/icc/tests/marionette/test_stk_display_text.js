...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: {
      ...
      text:
        "This command instructs the ME to display a text message. " +
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x80, text: "你好", responseNeeded: false },
  },
  {
    command:
      "D028" + // Length
  ...
  {
    command:
      "D081FC" + // Length (2-byte length encoded: 252)
      "8103012100" + // Command details
      "82028102" + // Device identities
      "8D" + // Text string
    ...
    expect: {
      commandQualifier: 0x00,
      text:
        "Ce message se compose de 273 caracteres en mode " +
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testDisplayText(aEvent.command, data.expect)
        )
      );
