...
/* global ok, log, is, waitForTargetEvent, startTestCommon, restartRadioAndWait,
   getMozIcc, setPinLockEnabled, DEFAULT_PIN */
...
function testUnlockPin(aIcc, aPin, aErrorName, aRetryCount) {
  ...
  return aIcc.unlockCardLock({ lockType: "pin", pin: aPin }).then(
    aResult => {
      if (aErrorName) {
...
function testUnlockPinAndWait(aIcc, aPin, aCardState) {
  log(
    "testUnlockPin with pin=" +
      aPin +
      ", and wait cardState changes to '" +
      aCardState +
      "'"
  );
...
startTestCommon(function() {
  ...
  return (
    setPinLockEnabled(icc, true)
      ...
      .then(() =>
        testUnlockPin(icc, "1111", "IncorrectPassword", retryCount - 1)
      )
