...
/* global MozIccManager, is, log, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu , isIcons, BASIC_ICON */
...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: {
      commandQualifier: 0x00,
      text:
        "***1111111111###***2222222222###***3333333333###***444444" +
  ...
  },
  {
    ...
    expect: {
      commandQualifier: 0x01,
      text:
        "ЗДРАВСТВУЙТЕЗДРАВСТВУЙТЕЗДРАВСТВУЙТЕЗДРАВСТВУЙТЕЗДРАВСТВУ" +
        "ЙТЕЗДРАВСТВУЙ",
      minLength: 5,
      maxLength: 5,
    },
...
function testGetInput(aCommand, aExpect) {
  ...
  is(
    aCommand.options.isAlphabet,
    !!(aExpect.commandQualifier & 0x01),
    "options.isAlphabet"
  );
  is(
    aCommand.options.isUCS2,
    !!(aExpect.commandQualifier & 0x02),
    "options.isUCS2"
  );
  is(
    aCommand.options.hideInput,
    !!(aExpect.commandQualifier & 0x04),
    "options.hideInput"
  );
  is(
    aCommand.options.isPacked,
    !!(aExpect.commandQualifier & 0x08),
    "options.isPacked"
  );
  is(
    aCommand.options.isHelpAvailable,
    !!(aExpect.commandQualifier & 0x80),
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testGetInput(aEvent.command, data.expect)
        )
      );
