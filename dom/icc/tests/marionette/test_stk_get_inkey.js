...
const TEST_DATA = [
  {
    ...
    expect: { commandQualifier: 0x00, text: 'Enter "0"' },
  },
  {
    command:
      "D00B" + // Length
      ...
      "82028182" + // Device identities
      "8D00", // Text string
    expect: { commandQualifier: 0x00, text: null },
  },
  {
    command:
      "D00C" + // Length
      ...
      "82028182" + // Device identities
      "8D0100", // Text string
    expect: { commandQualifier: 0x00, text: "" },
  },
  {
    command:
      "D081AD" + // Length
    ...
    expect: {
      ...
      text:
        'Enter "x". This command instructs the ME to display ' +
  ...
  },
  {
    ...
    expect: { commandQualifier: 0x00, text: "<TIME-OUT>" },
  },
  {
    command:
      "D08199" + // Length
    ...
    expect: {
      ...
      text:
        "ЗДРАВСТВУЙТЕЗДРАВСТВУЙТЕЗДРАВСТВУЙТЕЗДРАВСТВУЙТЕЗДР" +
        "АВСТВУЙТЕЗДРАВСТВУЙ",
    },
  },
  {
    ...
    expect: { commandQualifier: 0x03, text: "Enter" },
  },
  {
    command:
      "D015" + // Length
    ...
    expect: { commandQualifier: 0x04, text: "Enter YES" },
  },
  {
    command:
      "D019" + // Length
...
function testGetInKey(aCommand, aExpect) {
  ...
  is(
    aCommand.options.isYesNoRequested,
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testGetInKey(aEvent.command, data.expect)
        )
      );
