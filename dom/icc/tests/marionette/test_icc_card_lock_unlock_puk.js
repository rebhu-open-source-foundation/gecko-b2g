...
/* global ok, log, is, waitForTargetEvent, startTestCommon, restartRadioAndWait,
   getMozIcc, setPinLockEnabled, DEFAULT_PIN, DEFAULT_PUK */
...
function testUnlockPuk(aIcc, aPuk, aNewPin, aErrorName, aRetryCount) {
  ...
  return aIcc
    .unlockCardLock({ lockType: "puk", puk: aPuk, newPin: aNewPin })
    .then(
      aResult => {
        if (aErrorName) {
...
function testUnlockPukAndWait(aIcc, aPuk, aNewPin, aCardState) {
  log(
    "testUnlockPuk with puk=" +
      aPuk +
      "/newPin=" +
      aNewPin +
      ", and wait card state changes to '" +
      aCardState +
      "'"
  );
