...
/* global MozIccManager, is, ok, log, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu, isIcons, workingFrame,
   BASIC_ICON, COLOR_TRANSPARENCY_ICON  */
...
const TEST_DATA = [
  {
    ...
    expect: { commandQualifier: 0x00, text: "Send DTMF" },
  },
  {
    command:
      "D010" + // Length
  ...
  },
  {
    command:
      ...
      "AC02C1F2", // DTMF string
    expect: { commandQualifier: 0x00, text: "ЗДРАВСТВУЙТЕ" },
  },
  {
    command:
      "D023" + // Length
    ...
    expect: { commandQualifier: 0x00, text: "Send DTMF 1" },
  },
  {
    command:
      "D014" + // Length
      ...
      "AC02C1F2", // DTMF string
    expect: { commandQualifier: 0x00, text: "你好" },
  },
];
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          verifySendDTMF(aEvent.command, data.expect)
        )
      );
...
function testSendDTMFNullAlphaId() {
  ...
  icc.addEventListener("stkcommand", event =>
    ok(false, event + " should not occur.")
  );

  // No "icc-stkcommand" system message should be sent.
  workingFrame.contentWindow.navigator.mozSetMessageHandler(
    "icc-stkcommand",
  ...
  return sendEmulatorStkPdu(TEST_CMD_NULL_ALPHA_ID).then(
    () =>
