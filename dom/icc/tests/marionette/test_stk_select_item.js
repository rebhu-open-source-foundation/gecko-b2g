...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: {
      ...
      title: "LargeMenu2",
      items: [
        { identifier: 255, text: "Call Forwarding Unconditional" },
  ...
  },
  {
    ...
    expect: {
      commandQualifier: 0x00,
      title:
        "The SIM shall supply a set of items from which the user " +
...
function testSelectItem(aCommand, aExpect) {
  ...
  is(
    aCommand.options.presentationType,
    aCommand.commandQualifier & 0x03,
    "presentationType"
  );
  is(
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testSelectItem(aEvent.command, data.expect)
        )
      );
