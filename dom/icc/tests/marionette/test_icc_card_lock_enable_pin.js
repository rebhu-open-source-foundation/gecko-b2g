...
/* global ok, log, is, waitForTargetEvent, startTestCommon,
   getMozIcc, setPinLockEnabled, DEFAULT_PIN */

const MARIONETTE_TIMEOUT = 60000;
const MARIONETTE_HEAD_JS = "head.js";

function setCardLockAndCheck(
  aIcc,
  aLockType,
  aPin,
  aEnabled,
  ...
  return aIcc.setCardLock(options).then(
    aResult => {
      if (aErrorName) {
      ...
      // Check lock state.
      return aIcc.getCardLock(aLockType).then(aResult => {
...
startTestCommon(function() {
  ...
  return (
    icc
      .getCardLockRetryCount(lockType)
