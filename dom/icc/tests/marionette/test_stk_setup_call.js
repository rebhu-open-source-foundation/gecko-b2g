...
/* global MozIccManager, is, log, startTestCommon, getMozIcc, waitForTargetEvent
   waitForSystemMessage, sendEmulatorStkPdu, isStkText,
   BASIC_ICON, COLOR_ICON */
...
const TEST_DATA = [
  ...
  },
  {
    ...
    expect: {
      ...
      confirmMessage: {
        text:
          "Three types are defined: - set up a call, but only if " +
  ...
  },
  {
    ...
    expect: {
      commandQualifier: 0x00,
      confirmMessage: { iconSelfExplanatory: true, icons: [BASIC_ICON] },
      callMessage: { iconSelfExplanatory: false, icons: [COLOR_ICON] },
      address: "+012340123456,1,2",
    },
  },
];
    ...
    promise = promise.then(() => {
      ...
      promises.push(
        waitForTargetEvent(icc, "stkcommand").then(aEvent =>
          testSetupCall(aEvent.command, data.expect)
        )
      );
