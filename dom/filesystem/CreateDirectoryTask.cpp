...
CreateDirectoryTaskChild::Create(FileSystemBase* aFileSystem,
  ...
  RefPtr<CreateDirectoryTaskChild> task =
      new CreateDirectoryTaskChild(globalObject, aFileSystem, aTargetPath);
...
already_AddRefed<Promise> CreateDirectoryTaskChild::GetPromise() {
  MOZ_ASSERT(NS_IsMainThread(), "Only call on main thread!");
  return RefPtr<Promise>(mPromise).forget();
}

FileSystemParams CreateDirectoryTaskChild::GetRequestParams(
...
void CreateDirectoryTaskChild::SetSuccessRequestResult(
...
void CreateDirectoryTaskChild::HandlerCallback() {
  MOZ_ASSERT(NS_IsMainThread(), "Only call on main thread!");
...
void CreateDirectoryTaskChild::GetPermissionAccessType(
...
CreateDirectoryTaskParent::CreateDirectoryTaskParent(
    FileSystemBase* aFileSystem, const FileSystemCreateDirectoryParams& aParam,
    FileSystemRequestParent* aParent)
...
FileSystemResponseValue CreateDirectoryTaskParent::GetSuccessRequestResult(
...
nsresult CreateDirectoryTaskParent::IOWork() {
  MOZ_ASSERT(XRE_IsParentProcess(), "Only call from parent process!");
...
nsresult CreateDirectoryTaskParent::GetTargetPath(nsAString& aPath) const {
  return mTargetPath->GetPath(aPath);
}

void CreateDirectoryTaskParent::GetPermissionAccessType(
    nsCString& aAccess) const {
  aAccess.AssignLiteral(DIRECTORY_CREATE_PERMISSION);
}
