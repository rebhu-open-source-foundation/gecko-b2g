...
/* static */ already_AddRefed<RemoveTaskChild> RemoveTaskChild::Create(
    FileSystemBase* aFileSystem, nsIFile* aDirPath, nsIFile* aTargetPath,
  ...
  RefPtr<RemoveTaskChild> task = new RemoveTaskChild(
      globalObject, aFileSystem, aDirPath, aTargetPath, aRecursive);
...
RemoveTaskChild::RemoveTaskChild(nsIGlobalObject* aGlobalObject,
                                 FileSystemBase* aFileSystem, nsIFile* aDirPath,
                                 nsIFile* aTargetPath, bool aRecursive)
...
RemoveTaskChild::~RemoveTaskChild() { MOZ_ASSERT(NS_IsMainThread()); }

already_AddRefed<Promise> RemoveTaskChild::GetPromise() {
  MOZ_ASSERT(NS_IsMainThread(), "Only call on main thread!");
  return RefPtr<Promise>(mPromise).forget();
}

FileSystemParams RemoveTaskChild::GetRequestParams(
...
void RemoveTaskChild::SetSuccessRequestResult(
...
void RemoveTaskChild::HandlerCallback() {
  MOZ_ASSERT(NS_IsMainThread(), "Only call on main thread!");
...
void RemoveTaskChild::GetPermissionAccessType(nsCString& aAccess) const {
...
/* static */ already_AddRefed<RemoveTaskParent> RemoveTaskParent::Create(
...
FileSystemResponseValue RemoveTaskParent::GetSuccessRequestResult(
...
nsresult RemoveTaskParent::IOWork() {
  MOZ_ASSERT(XRE_IsParentProcess(), "Only call from parent process!");
...
nsresult RemoveTaskParent::GetTargetPath(nsAString& aPath) const {
  return mTargetPath->GetPath(aPath);
}

void RemoveTaskParent::GetPermissionAccessType(nsCString& aAccess) const {
  aAccess.AssignLiteral(DIRECTORY_WRITE_PERMISSION);
}
