...
DeviceStorageFileSystem::DeviceStorageFileSystem(const nsAString& aStorageType,
    ...
    : mStorageType(aStorageType), mStorageName(aStorageName), mWindowId(0) {
  ...
  Unused << NS_WARN_IF(
      !rootFile || NS_FAILED(rootFile->GetPath(mLocalOrDeviceStorageRootPath)));
...
already_AddRefed<FileSystemBase> DeviceStorageFileSystem::Clone() {
  AssertIsOnOwningThread();
...
void DeviceStorageFileSystem::Init(nsDOMDeviceStorage* aDeviceStorage) {
...
void DeviceStorageFileSystem::Shutdown() {
  AssertIsOnOwningThread();
  mShutdown = true;
}
...
bool DeviceStorageFileSystem::IsSafeFile(nsIFile* aFile) const {
...
bool DeviceStorageFileSystem::IsSafeDirectory(Directory* aDir) const {
  AssertIsOnOwningThread();
  MOZ_ASSERT(aDir);
...
void DeviceStorageFileSystem::SerializeDOMPath(nsAString& aString) const {
  AssertIsOnOwningThread();
...
nsresult DeviceStorageFileSystem::MainThreadWork() {
  MOZ_ASSERT(NS_IsMainThread());
