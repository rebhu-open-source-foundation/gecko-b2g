...
class CreateFileTaskChild final : public FileSystemTaskChildBase {
 public:
  static already_AddRefed<CreateFileTaskChild> Create(
      FileSystemBase* aFileSystem, nsIFile* aFile, Blob* aBlobData,
      nsTArray<uint8_t>& aArrayData, bool replace, ErrorResult& aRv);

  virtual ~CreateFileTaskChild();
 ...
 private:
  CreateFileTaskChild(nsIGlobalObject* aGlobalObject,
                      FileSystemBase* aFileSystem, nsIFile* aFile,
                      bool aReplace);
