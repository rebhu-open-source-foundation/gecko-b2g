...
MmsMessage::MmsMessage(nsPIDOMWindowInner* aWindow,
                       MmsMessageInternal* aMessage)
...
int32_t MmsMessage::Id() const {
  int32_t id;
  mMessage->GetId(&id);
  return id;
}

uint64_t MmsMessage::ThreadId() const {
  uint64_t id;
  mMessage->GetThreadId(&id);
  return id;
}

void MmsMessage::GetIccId(nsString& aRetVal) const {
  mMessage->GetIccId(aRetVal);
}

void MmsMessage::GetDelivery(nsString& aRetVal) const {
...
uint64_t MmsMessage::SentTimestamp() const {
  uint64_t timestamp;
  mMessage->GetSentTimestamp(&timestamp);
  return timestamp;
}

bool MmsMessage::Read() const {
  bool read;
  mMessage->GetRead(&read);
  return read;
}

void MmsMessage::GetSubject(nsString& aRetVal) const {
  mMessage->GetSubject(aRetVal);
}

void MmsMessage::GetSmil(nsString& aRetVal) const {
  mMessage->GetSmil(aRetVal);
}

void MmsMessage::GetAttachments(nsTArray<MmsAttachment>& aRetVal) const {
