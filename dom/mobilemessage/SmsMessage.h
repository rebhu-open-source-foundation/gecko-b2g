...
class SmsMessage final : public nsISupports, public nsWrapperCache {
 ...
 public:
  ...
  void GetDeliveryStatus(nsString& aRetVal) const;

  void GetSender(nsString& aRetVal) const;

  void GetReceiver(nsString& aRetVal) const;

  void GetBody(nsString& aRetVal) const;

  void GetMessageClass(nsString& aRetVal) const;

  uint64_t Timestamp() const;

  uint64_t SentTimestamp() const;

  uint64_t DeliveryTimestamp() const;
