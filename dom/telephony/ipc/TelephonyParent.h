...
protected:
  ...
  AllocPTelephonyRequestParent(const IPCTelephonyRequest& aRequest);

  bool
  DeallocPTelephonyRequestParent(PTelephonyRequestParent* aActor);
/*
#ifdef MOZ_WIDGET_GONK
  PVideoCallProviderParent*
  AllocPVideoCallProviderParent(const uint32_t& clientId, const uint32_t& callIndex);
...
*/
  ...
  mozilla::ipc::IPCResult
  RecvStartTone(const uint32_t& aClientId, const nsString& aTone);

  mozilla::ipc::IPCResult
  RecvStopTone(const uint32_t& aClientId);

  mozilla::ipc::IPCResult
  RecvGetHacMode(bool* aEnabled);

  mozilla::ipc::IPCResult
  RecvSetHacMode(const bool& aEnabled);

  mozilla::ipc::IPCResult
  RecvGetMicrophoneMuted(bool* aMuted);

  mozilla::ipc::IPCResult
  RecvSetMicrophoneMuted(const bool& aMuted);

  mozilla::ipc::IPCResult
  RecvGetSpeakerEnabled(bool* aEnabled);

  mozilla::ipc::IPCResult
  RecvSetSpeakerEnabled(const bool& aEnabled);

  mozilla::ipc::IPCResult
  RecvGetTtyMode(uint16_t* aEnabled);

  mozilla::ipc::IPCResult
  RecvSetTtyMode(const uint16_t& aEnabled);
