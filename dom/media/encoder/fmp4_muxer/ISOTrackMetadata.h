...
class AACTrackMetadata : public AudioTrackMetadata {
 public:
  ...
  AACTrackMetadata()
      : mSampleRate(0), mFrameDuration(0), mFrameSize(0), mChannels(0) {
...
class AMRTrackMetadata : public AudioTrackMetadata {
 public:
  ...
  // TrackMetadataBase member
  MetadataKind GetKind() const override {
    return mWB ? METADATA_AMR_WB : METADATA_AMR;
  }
...
class EVRCTrackMetadata : public AudioTrackMetadata {
 public:
  ...
  EVRCTrackMetadata() : mChannels(0) { MOZ_COUNT_CTOR(EVRCTrackMetadata); }
