...
class OMXCodecReservation : public RefBase {
 public:
  ...
  virtual ~OMXCodecReservation() { ReleaseOMXCodec(); }
...
class OMXCodecWrapper {
 ...
 protected:
   ...
   */
  virtual void AppendFrame(nsTArray<uint8_t>* aOutputBuf, const uint8_t* aData,
                           size_t aSize) {
 ...
 private:
  ...
  Vector<sp<MediaCodecBuffer> > mInputBufs;
  Vector<sp<MediaCodecBuffer> > mOutputBufs;
...
class OMXAudioEncoder final : public OMXCodecWrapper {
 public:
   ...
   */
  nsresult Configure(int aChannelCount, int aInputSampleRate,
 ...
 protected:
  virtual status_t AppendDecoderConfig(nsTArray<uint8_t>* aOutputBuf,
                                       MediaCodecBuffer* aData) override;
...
class OMXVideoEncoder final : public OMXCodecWrapper {
 public:
  ...
  enum BlobFormat {
    AVC_MP4,  // MP4 file config descripter (defined in ISO/IEC
              // 14496-15 5.2.4.1.1)
