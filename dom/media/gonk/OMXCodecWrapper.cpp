...
bool OMXCodecReservation::ReserveOMXCodec() {
  ...
  mOwned =
      mClient->AcquireSyncNoWait();  // don't wait if resource is not available
  // CODEC_ERROR("OMX Reservation: (%d) Acquire was %s", (int) mType, mOwned ?
  // "Successful" : "Failed");
  return mOwned;
}

void OMXCodecReservation::ReleaseOMXCodec() {
  if (!mClient) {
    return;
  }
  // CODEC_ERROR("OMX Reservation: Releasing resource: (%d) %s", (int) mType,
  // mOwned ? "Owned" : "Not owned");
  if (mOwned) {
...
OMXCodecWrapper::OMXCodecWrapper(CodecType aCodecType)
  ...
  if (aCodecType == CodecType::AVC_ENC) {
    ...
    mCodec =
        MediaCodec::CreateByType(mLooper, MEDIA_MIMETYPE_AUDIO_AMR_NB, true);
  } else if (aCodecType == CodecType::AMR_WB_ENC) {
    mCodec =
        MediaCodec::CreateByType(mLooper, MEDIA_MIMETYPE_AUDIO_AMR_WB, true);
...
status_t OMXCodecWrapper::Start() {
  // Already started.
...
nsresult OMXVideoEncoder::Configure(int aWidth, int aHeight, int aFrameRate,
...
nsresult OMXVideoEncoder::ConfigureDirect(sp<AMessage>& aFormat,
                                          BlobFormat aBlobFormat) {
...
// - QCOM proprietary/HAL_PIXEL_FORMAT_YCbCr_420_SP_VENUS (from Flame HW video
// decoder)
static void ConvertGrallocImageToNV12(GrallocImage* aSource,
                                      uint8_t* aDestination) {
  // Get graphic buffer.
...
static nsresult ConvertSourceSurfaceToNV12(
    const RefPtr<SourceSurface>& aSurface, uint8_t* aDestination) {
...
nsresult OMXVideoEncoder::Encode(const Image* aImage, int aWidth, int aHeight,
  ...
  if (img) {
    ...
    if (format == ImageFormat::PLANAR_YCBCR) {
      ...
      // (as used from WebrtcOMXH264VideoCodec, and a few other places) - bug
      // 1067442
      const PlanarYCbCrData* yuv =
          static_cast<PlanarYCbCrImage*>(img)->GetData();
      ...
      int halFormat =
          static_cast<GrallocImage*>(img)->GetGraphicBuffer()->getPixelFormat();
      NS_ENSURE_TRUE(
...
nsresult OMXVideoEncoder::SetBitrate(int32_t aKbps) {
...
nsresult OMXVideoEncoder::RequestIDRFrame() {
  MOZ_ASSERT(mStarted,
             "Configure() should be called before RequestIDRFrame().");
  return mCodec->requestIDRFrame() == OK ? NS_OK : NS_ERROR_FAILURE;
}

nsresult OMXAudioEncoder::Configure(int aChannels, int aInputSampleRate,
                                    int aEncodedSampleRate) {
  MOZ_ASSERT(!mStarted);

  NS_ENSURE_TRUE(
      aChannels > 0 && aInputSampleRate > 0 && aEncodedSampleRate >= 0,
      NS_ERROR_INVALID_ARG);

  if (aInputSampleRate != aEncodedSampleRate) {
    int error;
    mResampler =
        speex_resampler_init(aChannels, aInputSampleRate, aEncodedSampleRate,
  ...
  mResamplingRatio = aEncodedSampleRate > 0
                         ? 1.0 * aEncodedSampleRate / aInputSampleRate
                         : 1.0;
  result = Start();
...
class InputBufferHelper final {
 public:
  ...
  status_t Enqueue(int64_t aTimestamp, int aFlags) {
    ...
    // Queue this buffer.
    status_t result =
        mCodec->queueInputBuffer(mIndex, 0, mOffset, aTimestamp, aFlags);
    NS_ENSURE_TRUE(result == OK, result);
    mData = nullptr;
  ...
  MediaBufferState ReadChunk(AudioChunk& aChunk, size_t* aSamplesRead) {
    ...
    if (bytesToCopy <= AvailableSize()) {
      ...
      AudioTrackEncoder::InterleaveTrackData(
          aChunk, chunkSamples, mOMXAEncoder.mChannels, interleavedSource);
      ...
      while (GetNextSubChunk(bytesToCopy, subChunkSamples)) {
        ...
        if (aChunk.IsNull()) {
          ...
          bytesCopied = SendInterleavedSubChunkToBuffer(interleavedSource,
                                                        subChunkSamples);
        }
 ...
 private:
  ...
  bool IsEmpty() const { return (mOffset == 0); }

  size_t GetCapacity() const { return mCapicity; }
  ...
  // Determine the size of sub-chunk (aSamplesToCopy) according to buffer
  // capacity. For subsequent call, the number of bytes remain to be copied will
  // also be updated in this function.
  bool GetNextSubChunk(size_t& aBytesToCopy, size_t& aSamplesToCopy) {
    size_t bufferCapabity = GetCapacity();
    double sampleBytes = mOMXAEncoder.mChannels *
                         mOMXAEncoder.mResamplingRatio * sizeof(AudioDataValue);
    if (aBytesToCopy) {
...
status_t OMXAudioEncoder::AppendDecoderConfig(nsTArray<uint8_t>* aOutputBuf,
  ...
  const uint8_t decConfig[] = {
      ...
      0x00,
      0x03,
      0x00,  // Buffer size: 768 (kAACFrameSize).
      0x00,
      0x01,
      0x77,
      0x00,  // Max bitrate: 96000 (kAACBitrate).
      0x00,
      0x01,
      0x77,
      0x00,              // Avg bitrate: 96000 (kAACBitrate).
...
nsresult OMXCodecWrapper::GetNextEncodedFrame(nsTArray<uint8_t>* aOutputBuf,
  ...
  bool retry = false;
  do {
    status_t result = mCodec->dequeueOutputBuffer(
        &index, &outOffset, &outSize, &outTimeUs, &outFlags, aTimeOut);
    switch (result) {
      case OK:
        break;
