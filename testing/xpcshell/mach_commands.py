...
def get_parser():
    ...
    if conditions.is_android(build_obj) or (
        build_obj.substs.get("MOZ_BUILD_APP") == "b2g"
        and build_obj.substs.get("MOZ_WIDGET_TOOLKIT") == "gonk"
    ):
...
class MachCommands(MachCommandBase):
    ...
    def run_xpcshell_test(self, test_objects=None, **params):
        ...
        if conditions.is_android(self) or (
            self.substs.get("MOZ_BUILD_APP") == "b2g"
            and self.substs.get("MOZ_WIDGET_TOOLKIT") == "gonk"
        ):
