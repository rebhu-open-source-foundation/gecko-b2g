...
static gfx::SurfaceFormat HalFormatToSurfaceFormat(int aHalFormat,
                                                   TextureFlags aFlags) {
...
static bool NeedsConvertFromYUVtoRGB565(int aHalFormat) {
  switch (aHalFormat) {
    case android::PIXEL_FORMAT_BGRA_8888:
    case android::PIXEL_FORMAT_RGBA_8888:
    case android::PIXEL_FORMAT_RGBX_8888:
...
GrallocTextureHostBasic::GrallocTextureHostBasic(
  ...
  android::GraphicBuffer* grallocBuffer =
      GetGraphicBufferFromDesc(mGrallocHandle).get();
  MOZ_ASSERT(grallocBuffer);

  if (grallocBuffer) {
    mFormat = HalFormatToSurfaceFormat(grallocBuffer->getPixelFormat(),
...
bool GrallocTextureHostBasic::Lock() {
  ...
  if (NeedsConvertFromYUVtoRGB565(graphicBuffer->getPixelFormat())) {
    ...
    surf = GetDataSourceSurfaceFrom(graphicBuffer, mCropSize, ycbcrData);
  } else {
    uint32_t usage = GRALLOC_USAGE_SW_READ_OFTEN;
    int32_t rv =
        graphicBuffer->lock(usage, reinterpret_cast<void**>(&mMappedBuffer));
    if (rv) {
    ...
    surf = gfx::Factory::CreateWrappingDataSourceSurface(
        mMappedBuffer, graphicBuffer->getStride() * gfx::BytesPerPixel(mFormat),
        mCropSize, mFormat);
  }
...
bool GrallocTextureHostBasic::IsValid() const {
  android::GraphicBuffer* graphicBuffer =
      GetGraphicBufferFromDesc(mGrallocHandle).get();
  return graphicBuffer != nullptr;
}

bool GrallocTextureHostBasic::BindTextureSource(
    CompositableTextureSourceRef& aTexture) {
  aTexture = mTextureSource;
  return !!aTexture;
}

void GrallocTextureHostBasic::UnbindTextureSource() {
  TextureHost::UnbindTextureSource();
  ClearTextureSource();
}

void GrallocTextureHostBasic::ClearTextureSource() {
...
void GrallocTextureHostBasic::SetTextureSourceProvider(
  ...
  if (!aProvider || !aProvider->AsBasicCompositor()) {
    mTextureSource = nullptr;
    return;
  }
...
gfx::SurfaceFormat GrallocTextureHostBasic::GetFormat() const {
  return mFormat;
}

void GrallocTextureHostBasic::WaitAcquireFenceHandleSyncComplete() {
...
void GrallocTextureHostBasic::SetCropRect(nsIntRect aCropRect) {
...
void GrallocTextureHostBasic::DeallocateSharedData() {
  ClearTextureSource();
...
void GrallocTextureHostBasic::ForgetSharedData() { ClearTextureSource(); }

void GrallocTextureHostBasic::DeallocateDeviceData() { ClearTextureSource(); }

LayerRenderState GrallocTextureHostBasic::GetRenderState() {
  android::GraphicBuffer* graphicBuffer =
      GetGraphicBufferFromDesc(mGrallocHandle).get();
