...
bool GrallocImage::SetData(const Data& aData) {
  ...
  void* vaddr;
  if (graphicBuffer->lock(GraphicBuffer::USAGE_SW_WRITE_OFTEN, &vaddr) != OK) {
    return false;
  }
  ...
  gfx::IntSize uvSize = gfx::IntSize(ySize.width / 2, ySize.height / 2);
  ...
  if (yStride == mData.mYStride && yStride == ySize.width) {
    memcpy(yChannel, mData.mYChannel, yStride * ySize.height);
  } else {
    for (int i = 0; i < ySize.height; i++) {
      memcpy(yChannel + i * yStride, mData.mYChannel + i * mData.mYStride,
             ySize.width);
    }
  }
  if (uvStride == mData.mCbCrStride && uvStride == uvSize.width) {
    ...
    for (int i = 0; i < uvSize.height; i++) {
      memcpy(uChannel + i * uvStride, mData.mCbChannel + i * mData.mCbCrStride,
             uvSize.width);
      memcpy(vChannel + i * uvStride, mData.mCrChannel + i * mData.mCbCrStride,
             uvSize.width);
    }
  }
 ...
 */
static void ConvertYVU420SPToRGB565(void* aYData, uint32_t aYStride,
                                    void* aUData, void* aVData,
                                    uint32_t aUVStride, void* aOut,
 ...
 * of vendor formats.
 */
static status_t ConvertVendorYUVFormatToRGB565(
  ...
  // Check if the vendor provides explicit addresses of Y/Cb/Cr buffer from
  // lockYCbCr
  ...
  if (ycbcr.chroma_step == 2) {
    ...
    ConvertYVU420SPToRGB565(ycbcr.y, ycbcr.ystride, ycbcr.cb, ycbcr.cr,
                            ycbcr.cstride, aMappedSurface->mData, width,
                            height);
  } else {
...
static status_t ConvertOmxYUVFormatToRGB565(
    android::sp<GraphicBuffer>& aBuffer, gfx::DataSourceSurface* aSurface,
    gfx::DataSourceSurface::MappedSurface* aMappedSurface,
  ...
  if (format == GrallocImage::HAL_PIXEL_FORMAT_YCrCb_420_SP_ADRENO) {
    ...
    ConvertYVU420SPToRGB565(buffer, alignedWidth, buffer + uvOffset + 1,
                            buffer + uvOffset, uvStride, aMappedSurface->mData,
                            width, height);
    return OK;
  }
  ...
  if (format == HAL_PIXEL_FORMAT_YCrCb_420_SP) {
    ...
    ConvertYVU420SPToRGB565(buffer, stride, buffer + uvOffset + 1,
                            buffer + uvOffset, stride, aMappedSurface->mData,
                            width, height);
    return OK;
  }
  ...
  if (format == HAL_PIXEL_FORMAT_YV12) {
    ...
    // yuv data has already stored in GraphicBuffer. Here we try to confgiure
    // the mData if it doesn't contain valid configuration.
