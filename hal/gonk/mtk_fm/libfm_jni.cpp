...
jfloat seek(JNIEnv* env, jobject thiz, jfloat freq,
            jboolean isUp)  // jboolean isUp;
{
  int ret = 0;
  int tmp_freq;
  int ret_freq;
...
#define FM_SCAN_CH_SIZE_MAX 200
  ...
  if (chl_cnt > 0) {
    ...
    env->SetShortArrayRegion(scanChlarray, 0, chl_cnt,
                             (const jshort*)&ScanTBL[0]);
  } else {
...
jshortArray emcmd(JNIEnv* env, jobject thiz, jshortArray val) {
  ...
  LOGI("EM cmd:=%x %x %x %x %x", cmdtbl[0], cmdtbl[1], cmdtbl[2], cmdtbl[3],
       cmdtbl[4]);
...
jshortArray getAFList(JNIEnv* env, jobject thiz) {
  ...
  // ret = FMR_get_af(g_idx, &af, &af_len); // If need, we should implemate this
  // API
  if (ret) {
    LOGE("%s, error, [ret=%d]\n", __func__, ret);
    return NULL;
