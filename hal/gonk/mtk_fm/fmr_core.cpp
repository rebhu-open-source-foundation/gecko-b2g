...
int FMR_get_cfgs(int idx) {
  ...
  LOGI(
      "%s successfully. chip: 0x%x, lband: %d, hband: %d, seek_space: %d, "
      "max_scan_num: %d\n",
...
int FMR_pwr_up(int idx, int freq) {
  ...
  LOGI("%s,[freq=%d],[low_band=%d] [high_band=%d]\n", __func__, freq,
       fmr_data.cfg_data.low_band, fmr_data.cfg_data.high_band);
 ...
 */
static int FMR_Seek_More(int idx, fm_softmute_tune_t* validfreq, fm_u8 seek_dir,
                         fm_u8 step, fm_u16 min_freq, fm_u16 max_freq) {
  fm_s32 i;
...
int FMR_seek_Channel(int idx, int start_freq, int min_freq, int max_freq,
                     int band_channel_no, int seek_space, int dir,
                     int* ret_freq, int* rssi_tmp) {
  fm_s32 i, ret = 0;
  fm_softmute_tune_t cur_freq;

  if (dir == 1) /* forward */ {
    for (i = ((start_freq - min_freq) / seek_space + 1); i < band_channel_no;
         i++) {
      ...
      if (ret == fm_false) {
        ...
        if (cur_freq.valid == fm_false) {
          ...
          if (FMR_Seek_More(idx, &cur_freq, dir, seek_space, min_freq,
                            max_freq) == 0) {
    ...
    for (i = (band_channel_no - 1); i > ((start_freq - min_freq) / seek_space);
         i--) {
      cur_freq.freq = min_freq + seek_space * i;
...
int FMR_seek(int idx, int start_freq, int dir, int* ret_freq) {
  ...
  if (fmr_data.cfg_data.band ==
      FM_BAND_JAPAN) /* Japan band	   76MHz ~ 90MHz */ {
    band_channel_no = (9600 - 7600) / seek_space + 1;
    min_freq = 7600;
    max_freq = 9600;
  } else if (fmr_data.cfg_data.band ==
             FM_BAND_JAPANW) /* Japan wideband  76MHz ~ 108MHz */ {
  ...
  LOGD(
      "seek start freq %d band_channel_no=[%d], seek_space=%d band[%d - %d] "
      "dir=%d\n",
      start_freq, band_channel_no, seek_space, min_freq, max_freq, dir);

  ret = FMR_seek_Channel(idx, start_freq, min_freq, max_freq, band_channel_no,
                         seek_space, dir, ret_freq, &rssi);
...
int FMR_scan_Channels(int idx, uint16_t* scan_tbl, int* max_cnt,
                      fm_s32 band_channel_no, fm_u16 Start_Freq,
                      fm_u8 seek_space, fm_u8 NF_Space) {
  ...
  for (i = 0; i < band_channel_no; i++) {
    ...
    if (cur_freq.valid == fm_true) /* get valid channel */ {
      ...
      if ((LastExist == fm_true) && (Num > 0)) /* neighbor channel */ {
        if (cur_freq.rssi >
            SortData[Num - 1]
                .rssi) /* save current freq and cover last channel */ {
          if (FMR_SevereDensense(cur_freq.freq, cur_freq.rssi) == fm_true) {
            LastExist = fm_false;
            continue;
          }
  ...
  switch (fmr_data.cfg_data.scan_sort) {
    ...
    case FM_SCAN_SORT_DOWN: {
      ...
      for (i = 1; i < Num; i++) {
        for (j = i;
             (j > 0) && ((FM_SCAN_SORT_DOWN == fmr_data.cfg_data.scan_sort)
                             ? (SortData[j - 1].rssi < SortData[j].rssi)
                             : (SortData[j - 1].rssi > SortData[j].rssi));
             j--) {
...
int FMR_scan(int idx, uint16_t* scan_tbl, int* max_cnt) {
  ...
  LOGD("%s, [seek_space=%d] [band=%d]\n", __func__,
       fmr_data.cfg_data.seek_space, fmr_data.cfg_data.band);
  ...
  LOGD("%s, [Start_Freq=%d] [NF_Space=%d],band_channel_no %d\n", __func__,
       Start_Freq, NF_Space, band_channel_no);

  ret = FMR_scan_Channels(idx, scan_tbl, max_cnt, band_channel_no, Start_Freq,
                          seek_space, NF_Space);
...
int FMR_active_af(int idx, uint16_t* ret_freq) {
  ...
  FMR_ASSERT(ret_freq);
  ret = FMR_cbk_tbl(idx).active_af(FMR_fd(idx), &fmr_data.rds,
                                   fmr_data.cfg_data.band, fmr_data.cur_freq,
                                   ret_freq);
...
int FMR_active_ta(int idx, uint16_t* ret_freq) {
  ...
  FMR_ASSERT(ret_freq);
  ret = FMR_cbk_tbl(idx).active_ta(FMR_fd(idx), &fmr_data.rds,
                                   fmr_data.cfg_data.band, fmr_data.cur_freq,
...
int FMR_deactive_ta(int idx, uint16_t* ret_freq) {
  ...
  FMR_ASSERT(ret_freq);
  ret = FMR_cbk_tbl(idx).deactive_ta(FMR_fd(idx), &fmr_data.rds,
                                     fmr_data.cfg_data.band, fmr_data.cur_freq,
                                     &fmr_data.backup_freq, ret_freq);
  if ((ret == 0) && (*ret_freq != fmr_data.cur_freq)) {
    fmr_data.cur_freq = *ret_freq;
...
int FMR_get_hw_info(int idx, int** info, int* info_len) {
  ...
  *info_len = sizeof(struct fm_hw_info) / sizeof(int);
  ...
  LOGD("chip:0x%08x, eco:0x%08x, rom:0x%08x, patch: 0x%08x\n", info_array[0],
       info_array[1], info_array[2], info_array[3]);
  LOGD("%s, [ret=%d]\n", __func__, ret);
  return ret;
}
/*
 * th_idx: threshold type: 0, RSSI. 1,desense RSSI. 2,SMG.
