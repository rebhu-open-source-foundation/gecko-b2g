...
void nsRefreshDriver::CreateVsyncChildVsyncRefreshTimer() {
...
{
  uint32_t screenId;
  GetPresContext()->DeviceContext()->FindScreenId(&screenId);
...
RefreshDriverTimer* nsRefreshDriver::ChooseTimer() {
  ...
  if (!mOwnTimer) {
    mOwnTimer = GetRefreshDriverTimerFromCurrentDisplay();
  }
