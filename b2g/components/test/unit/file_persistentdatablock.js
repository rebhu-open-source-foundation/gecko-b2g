...
function _prepareConfig(_args) {
  ...
  args.oemUnlockAllowed =
    args.oemUnlockAllowed === undefined ? true : args.oemUnlockAllowed;
  ...
  PersistentDataBlock._libcutils.property_set(
    "sys.oem_unlock_allowed",
    args.oemUnlockAllowed === true ? "true" : "false"
  );
  ...
  let tempFile;
  return OS.File.open(PersistentDataBlock._dataBlockFile, {
    write: true,
    append: false,
    truncate: true,
  })
    ...
    .then(bytes => {
      ...
      let data = new Uint8Array(
        PersistentDataBlock._getBlockDeviceSize() -
          (DIGEST_SIZE_BYTES +
  ...
  return OS.File.open(PersistentDataBlock._dataBlockFile, {
    ...
    .then(_file => {
      ...
      return file.setPosition(
        DIGEST_SIZE_BYTES + PARTITION_MAGIC_SIZE_BYTES,
        OS.File.POS_START
      );
    })
    .then(() => {
...
function _installTests() {
  ...
  add_test(function test_getBlockDeviceSize() {
    ...
    ok(
      blockSize !== CACHE_PARTITION_SIZE,
      "test_getBlockDeviceSize: Block device size should be greater than 0"
    );
    run_next_test();
  });
  ...
  add_test(function test_computeDigest() {
    _prepareConfig().then(() => {
      PersistentDataBlock._computeDigest()
        .then(digest => {
  ...
  add_test(function test_getDataFieldSize() {
    PersistentDataBlock.getDataFieldSize()
  ...
  add_test(function test_setOemUnlockedEnabledToTrue() {
    PersistentDataBlock.setOemUnlockEnabled(true)
      .then(() => {
  ...
  add_test(function test_setOemUnlockedEnabledToFalse() {
    PersistentDataBlock.setOemUnlockEnabled(false)
      .then(() => {
        return utils_getByteAt(PersistentDataBlock._getBlockDeviceSize() - 1);
      })
      .then(byte => {
  ...
  add_test(function test_getOemUnlockedEnabledWithTrue() {
    ...
    PersistentDataBlock.setOemUnlockEnabled(true)
      .then(() => {
        return PersistentDataBlock.getOemUnlockEnabled()
          .then(enabled => {
            log("test_getOemUnlockedEnabledWithTrue: enabled is " + enabled);
            ok(
              enabled === true,
              "test_getOemUnlockedEnabledWithTrue: enabled value should be true"
            );
            run_next_test();
          })
          .catch(ex => {
      ...
      .catch(ex => {
        ok(
          false,
          "test_getOemUnlockedEnabledWithTrue failed: An error ocurred while setting the OEM Unlock Enabled byte to true: ex:" +
            ex
        );
      });
  });
  ...
  add_test(function test_getOemUnlockedEnabledWithFalse() {
    ...
    PersistentDataBlock.setOemUnlockEnabled(false)
      .then(() => {
        return PersistentDataBlock.getOemUnlockEnabled()
          .then(enabled => {
            ...
            ok(
              enabled === false,
              "test_getOemUnlockedEnabledWithFalse: enabled value should be false"
            );
            run_next_test();
          })
          .catch(ex => {
      ...
      .catch(ex => {
        ok(
          false,
          "test_getOemUnlockedEnabledWithFalse failed: An error ocurred while setting the OEM Unlock Enabled byte to false: ex:" +
            ex
        );
      });
  });
  ...
  add_test(function test_computeAndWriteDigest() {
    PersistentDataBlock._computeAndWriteDigest()
      .then(() => {
  ...
  add_test(function test_formatIfOemUnlockEnabledWithTrue() {
    _prepareConfig({ oem: true })
      ...
      .then(result => {
        ok(
          result === true,
          "test_formatIfOemUnlockEnabledWithTrue: result should be true"
        );
        return utils_getByteAt(PersistentDataBlock._getBlockDeviceSize() - 1);
      })
      .then(byte => {
  ...
  add_test(function test_formatIfOemUnlockEnabledWithFalse() {
    _prepareConfig({ oem: false })
      ...
      .then(result => {
        ...
        ok(
          result === false,
          "test_formatIfOemUnlockEnabledWithFalse: result should be false"
        );
        return utils_getByteAt(PersistentDataBlock._getBlockDeviceSize() - 1);
      })
      .then(byte => {
  ...
  add_test(function test_enforceChecksumValidityWithValidChecksum() {
    ...
    _prepareConfig().then(() => {
      PersistentDataBlock._enforceChecksumValidity()
        .then(() => {
        ...
        .catch(ex => {
          ok(
            false,
            "test_enforceChecksumValidityWithValidChecksum failed!: ex: " + ex
          );
        });
    });
  });
  ...
  add_test(function test_write() {
    ...
    PersistentDataBlock.write(data)
