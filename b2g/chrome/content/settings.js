...
/* global shell, debug, InitDebuggerSettings, AdbController */
...
try {
  ...
  if (isGonk) {
    ...
    XPCOMUtils.defineLazyGetter(this, "DeviceUtils", function() {
      ChromeUtils.import("resource://gre/modules/DeviceUtils.jsm");
      return DeviceUtils;
    });
  }
  ...
  var SettingsListener = {
    ...
    observe: function sl_observe(name, defaultValue, callback) {
      ...
      req.addEventListener("success", function onsuccess() {
        callback(
          typeof req.result[name] != "undefined"
            ? req.result[name]
            : defaultValue
        );
      });
    ...
    observe_fast(name, defaultValue, callback) {
      var settings = window.navigator.mozSettings;
  ...
  function delayed_start() {
    ...
    (function RILSettingsToPrefs() {
      ...
      ["data", "sms", "telephony", "voicemail"].forEach(function(key) {
        SettingsListener.observe(
          "ril." + key + ".defaultServiceId",
          0,
          function(value) {
            if (value != null) {
              Services.prefs.setIntPref(
                "dom." + key + ".defaultServiceId",
                value
              );
            }
          }
    ...
    (function DeviceInfoToSettings() {
      ...
      if (isGonk) {
        ...
        product_manufacturer = libcutils.property_get(
      ...
      if (value) {
        if (!developerHUD) {
          ...
          Services.scriptloader.loadSubScript(
            "chrome://b2g/content/devtools/hud.js",
            scope
          );
          developerHUD = scope.developerHUD;
        }
        developerHUD.init();
      } else if (developerHUD) {
        developerHUD.uninit();
      }
    });

    // =================== Device Storage ====================
    SettingsListener.observe("device.storage.writable.name", "sdcard", function(
      value
    ) {
      if (
        Services.prefs.getPrefType("device.storage.writable.name") !=
    ...
    // =================== Privacy ====================
    SettingsListener.observe("privacy.donottrackheader.value", 1, function(
      value
    ) {
        ...
        if (!trackingId) {
          ...
          if (navigator.mozSettings) {
            navigator.mozSettings
              .createLock()
              .set({ "app.update.custom": trackingId });
          }
        }
      } catch (e) {
    ...
    (function Composer2DSettingToPref() {
      ...
      var req = navigator.mozSettings
        .createLock()
        .get("layers.composer2d.enabled");
      req.onsuccess = function() {
        if (typeof req.result["layers.composer2d.enabled"] === "undefined") {
          var enabled = false;
          if (
            Services.prefs.getPrefType("layers.composer2d.enabled") ==
          ...
          navigator.mozSettings
            .createLock()
            .set({ "layers.composer2d.enabled": enabled });
        }

        SettingsListener.observe("layers.composer2d.enabled", true, function(
          value
        ) {
    ...
    (function setupAccessibility() {
      ...
      SettingsListener.observe("accessibility.screenreader", false, function(
        value
      ) {
        if (!value) {
          return;
    ...
    (function setupCaptionStyles() {
      ...
      SettingsListener.observe("accessibility.caption.theme", null, function(
        value
      ) {
        if (value) {
      ...
      SettingsListener.observe("layers.low-precision", null, function(value) {
        if (value !== null) {
          ...
          try {
            ...
            console.log(
              "Unable to read pref layers.low-precision-buffer: " + e
            );
          }
        }
      });
      ...
      SettingsListener.observe("layers.low-opacity", null, function(value) {
        if (value !== null) {
          ...
          try {
            ...
            console.log(
              "Unable to read pref layers.low-precision-opacity: " + e
            );
          }
        }
      });
    ...
    SettingsListener.observe(
      ...
      function(value) {
        ...
        try {
          ...
          let uri = Services.io.newURI(value);
        ...
        let currentTheme = Services.prefs.getCharPref(
          "dom.mozApps.selected_theme",
          ""
        );
    ...
    (function setupBrowsingProxySettings() {
      function setPAC() {
        let usePAC = Services.prefs.getBoolPref(
          "network.proxy.pac_generator",
          false
        );
