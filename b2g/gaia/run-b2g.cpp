...
BOOL execute(wchar_t* binary_path, wchar_t* args, int cp_flags) {
  ...
  if (!CreateProcessW(binary_path, args, nullptr, nullptr, FALSE, cp_flags,
...
int wmain(int argc, wchar_t* argv[], wchar_t* envp[]) {
  ...
  wchar_t* args =
      (wchar_t*)malloc(2 * NUM_MAX_PATH_BYTES + wcslen(PROFILE_ARG));
  if (FAILED(StringCchPrintfW(args, NUM_MAX_PATH_BYTES, L"\"%ws\"%ws\"%ws\"",
                              b2g_path, PROFILE_ARG, profile_path))) {
