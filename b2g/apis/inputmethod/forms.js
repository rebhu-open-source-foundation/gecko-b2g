...
function debug_forms(str) {
  dump(`-*- forms.js [${content.location}]: ${str}\n`);
}
...
var FormAssistant = {
  ...
  setFocusedElement: function fa_setFocusedElement(element) {
    ...
    if (element) {
      ...
      if (this._editor) {
        ...
        if (selection) {
          // FIXME(fabrice): do we really need _selectionPrivate ?
          this._selectionPrivate = selection;
    ...
    switch (msg.name) {
      ...
      case "Forms:Input:SendKey":
        ...
        try {
          ...
          debug_forms(`Error: ${err.toString()}`);
  ...
  isFocusableElement: function fa_isFocusableElement(element) {
    debug_forms(`isFocusableElement ${element}`);
...
function getSelectionRange(element) {
  ...
  if (isPlainTextField(element)) {
    ...
    if (sel && sel.rangeCount > 0) {
      ...
      debug_forms("Failed to get window.getSelection()");
    }
   }
   return [start, end];
 }
