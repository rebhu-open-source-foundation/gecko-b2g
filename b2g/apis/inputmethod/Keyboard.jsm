...
var Utils = {
  getMMFromMessage: function u_getMMFromMessage(msg) {
    let mm;
    try {
      mm = msg.target.frameLoader.messageManager;
    } catch(e) {
      mm = msg.target;
    }
  ...
  checkPermissionForMM: function u_checkPermissionForMM(mm, permName) {
    // FIXME(fabrice): implement differently...
...
const Keyboard = {
  ...
  sendToForm: function(name, data) {
    if (!this.formMM) {
      this.debug(`Attempt to send message ${name} to form but no message manager exists.`);
  ...
  sendToSystem: function(name, data) {
    if (!this._systemMMs.length) {
      this.debug(`Attempt to send message ${name} to system but no message manager registered.`);
  ...
  init: function keyboardInit(ppmm) {
    ...
    for (let name of this._messageNames) {
      // this.debug(`registering listener for Keyboard:${name}`);
      Services.ppmm.addMessageListener('Keyboard:' + name, this);
    }

    for (let name of this._systemMessageNames) {
      // this.debug(`registering listener for System:${name}`);
      Services.ppmm.addMessageListener('System:' + name, this);
    }

    this.inputRegistryGlue = new InputRegistryGlue(ppmm);
  ...
  observe: function keyboardObserve(subject, topic, data) {
    ...
    if (topic == 'oop-frameloader-crashed' ||
      ...
      // Ignore notifications that aren't from a Browser frame.
      if (!frameLoader.ownerIsMozBrowseFrame) {
        // FIXME(fabrice): check why this fails with mozbrowser frames...
        this.debug(`Not ownerIsMozBrowseFrame`);
        // return;
      }
  ...
  initFormsFrameScript: function(mm) {
    // this.debug(`Adding forms message listeners`);
    ...
    // this.debug(`Done adding forms message listeners`);
  },

  receiveMessage: function keyboardReceiveMessage(msg) {
    this.debug(`Received message: ${msg.name} ${JSON.stringify(msg.data)}`);
    ...
    switch (msg.name) {
      ...
      case 'System:RegisterSync': {
        if (this._systemMMs.length !== 0) {
          debug('Warning: There are more than one content page ' +
  ...
  handleFocus: function keyboardHandleFocus(msg) {
    ...
    let mm = msg.target.frameLoader.messageManager;
    ...
    this.forwardEvent('Keyboard:Focus', msg).then(() => {
      ...
      debug(`Error: handleFocus is rejected by: ${reason}`);
    });
  },
  ...
  handleBlur: function keyboardHandleBlur(msg) {
    ...
    this.forwardEvent('Keyboard:Blur', msg).then(() => {
      ...
      debug(`Error: handleBlur is rejected by: ${reason}`);
    });
  },
...
function InputRegistryGlue(ppmm) {
  this._messageId = 0;
