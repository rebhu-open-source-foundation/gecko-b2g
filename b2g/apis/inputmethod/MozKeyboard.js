...
MozInputMethodManager.prototype = {
  ...
  handleFocus: function(data) {
    ...
    if (!handled) {
      debug('A frame with input-manage permission did not' +
...
MozInputMethod.prototype = {
  ...
  init: function mozInputMethodInit(win) {
    ...
    if (this._hasInputManagePerm(win)) {
      debug(`Adding manager restricted messages`);
      ...
      debug(`No input-manage permission for ${win.location}`);
    }
  },
  ...
  get inputcontext() {
    if (!WindowMap.isActive(this._window)) {
      debug(`Not sending inputcontext to inactive window (${this.wrappedInputContext}) ${this._window.location}`);
      return null;
    }
...
InputContextDOMRequestIpcHelper.prototype = {
  ...
  receiveMessage: function(msg) {
    if (!this._inputContext) {
      debug('InputContextDOMRequestIpcHelper received message without context attached.');
      return;
    }
...
MozInputContext.prototype = {
  ...
  receiveMessage: function ic_receiveMessage(msg) {
    if (!msg || !msg.json) {
      debug('InputContext received message without data.');
      return;
    }
    ...
    if (!resolver) {
      debug('InputContext received invalid requestId.');
      return;
    }
    ...
    switch (msg.name) {
      ...
      case "Keyboard:DeleteBackward:Result:OK":
        ...
        break;
      default:
        debug("Could not find a handler for " + msg.name);
        resolver.reject();
        break;
    }
  },
