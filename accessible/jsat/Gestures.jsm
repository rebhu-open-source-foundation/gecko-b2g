...
this.GestureTracker = {
  ...
  _create: function GestureTracker__create(
    ...
    this.current = new aGesture(
      aTimeStamp,
      aPoints,
      aLastEvent
    ); /* A constructor name should start with an uppercase letter. */ // jshint ignore:line
...
Gesture.prototype = {
   ...
   */
  _update: function Gesture__update(
    aPoints,
    aType,
    aCanCreate = false,
    aNeedComplete = false
...
function TravelGesture(
  ...
  aLastEvent,
  aTravelTo = Explore,
  aThreshold = GestureSettings.travelThreshold
) {
...
TapGesture.prototype.pointerdown = function TapGesture_pointerdown(
  aPoints,
  aTimeStamp
) {
