...
this.ContentControl.prototype = {
    ...
    if (direction === "Previous" && !aMessage.json.atStart) {
      switch (granularity) {
        case MOVEMENT_GRANULARITY_CHARACTER:
          ...
          break;
        case MOVEMENT_GRANULARITY_WORD:
          accText.getTextBeforeOffset(
            accText.caretOffset,
            Ci.nsIAccessibleText.BOUNDARY_WORD_START,
            start,
            end
          );
          accText.caretOffset =
            end.value === accText.caretOffset ? start.value : end.value;
          break;
      ...
      switch (granularity) {
        case MOVEMENT_GRANULARITY_CHARACTER:
          ...
          break;
        case MOVEMENT_GRANULARITY_WORD:
          accText.getTextAtOffset(
            accText.caretOffset,
            Ci.nsIAccessibleText.BOUNDARY_WORD_END,
            start,
            end
          );
  ...
  autoMove: function cc_autoMove(aAnchor, aOptions = {}) {
    ...
    let moveFunc = () => {
      ...
      let sentToChild = this.sendToChild(
        vc,
        {
