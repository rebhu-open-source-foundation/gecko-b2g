...
let settingsManager = Cc["@mozilla.org/sidl-native/settings;1"].getService(
...
this.AccessFu = {
  ...
  attach: function attach(aWindow) {
    ...
    settingsManager.addObserver(SCREENREADER_SETTING, this, {
      resolve: () =>
        Logger.debug(`addObserver ${SCREENREADER_SETTING} resolve`),
      reject: () => Logger.debug(`addObserver ${SCREENREADER_SETTING} reject`),
    });

    this._activatePref = new PrefCache(
      "accessibility.accessfu.activate",
      this._enableOrDisable.bind(this)
    );
  ...
  detach: function detach() {
    ...
    settingsManager.removeObserver(SCREENREADER_SETTING, this, {
      resolve: () =>
        Logger.debug(`removeObserver ${SCREENREADER_SETTING} resolve`),
      reject: () =>
        Logger.debug(`removeObserver ${SCREENREADER_SETTING} reject`),
    });
  ...
  _enable: function _enable() {
    ...
    this._quicknavModesPref = new PrefCache(
      QUICKNAV_MODES_PREF,
      (aName, aValue, aFirstRun) => {
    ...
    this._quicknavCurrentModePref = new PrefCache(
      QUICKNAV_INDEX_PREF,
      (aName, aValue) => {
    ...
    this._notifyOutputPref = new PrefCache(
    ...
    Services.obs.addObserver(this, "Accessibility:NextObject");
    Services.obs.addObserver(this, "Accessibility:PreviousObject");
    Services.obs.addObserver(this, "Accessibility:Focus");
    Services.obs.addObserver(this, "Accessibility:ActivateObject");
    Services.obs.addObserver(this, "Accessibility:LongPress");
    Services.obs.addObserver(this, "Accessibility:ScrollForward");
    Services.obs.addObserver(this, "Accessibility:ScrollBackward");
    Services.obs.addObserver(this, "Accessibility:MoveByGranularity");
    Services.obs.addObserver(embedderScreenReader => {
      Utils.Embedder = embedderScreenReader.wrappedJSObject;
    ...
    Services.obs.notifyObservers(null, "AccessFu:Enabled");
  ...
  _loadFrameScript: function _loadFrameScript(aMessageManager) {
    if (this._processedMessageManagers.includes(aMessageManager) === false) {
  ...
  observe: function observe(aSubject, aTopic, aData) {
    switch (aTopic) {
      ...
      case "inprocess-browser-shown": {
        ...
        // ownerIsMozBrowserFrame is false, the ownerIsMozBrowserOrAppFrame is setting on web-vew?
  ...
  brailleState: {
    ...
    init: function init(aOutput) {
      if (aOutput && "output" in aOutput) {
        ...
        this.selectionStart =
          typeof aOutput.selectionStart === "number"
            ? aOutput.selectionStart
            : this.selectionStart;
        this.selectionEnd =
          typeof aOutput.selectionEnd === "number"
            ? aOutput.selectionEnd
            : this.selectionEnd;
    ...
    switch (aDetail.eventType) {
      ...
      case "vc-change": {
        ...
        if (!this.highlightBox) {
          ...
          // Add highlight box
          highlightBox = Utils.win.document.createElementNS(
...
var Input = {
  ...
  _handleKeypress: function _handleKeypress(aEvent) {
    ...
    switch (aEvent.keyCode) {
      ...
      case aEvent.DOM_VK_RIGHT:
        ...
        this.moveCursor(
          aEvent.shiftKey ? "moveLast" : "moveNext",
          "Simple",
          "keyboard"
        );
        break;
      case aEvent.DOM_VK_LEFT:
        ...
        this.moveCursor(
          aEvent.shiftKey ? "moveFirst" : "movePrevious",
          "Simple",
          "keyboard"
        );
        break;
  ...
  moveToPoint: function moveToPoint(aRule, aX, aY) {
    ...
    if (Utils.MozBuildApp === "mobile/android") {
      ...
      let win = Utils.win;
      Utils.winUtils.sendMouseEvent(
        "mousemove",
        aX - win.mozInnerScreenX,
        aY - win.mozInnerScreenY,
        0,
        0,
        0
      );
  ...
  moveByGranularity: function moveByGranularity(aDetails) {
    ...
    let mm = Utils.getMessageManager(Utils.CurrentBrowser);
    let type = this.editState.editing
      ? "AccessFu:MoveCaret"
  ...
  activateCurrent: function activateCurrent(aData, aActivateIfKey = false) {
    let mm = Utils.getMessageManager(Utils.CurrentBrowser);
    let offset =
      aData && typeof aData.keyIndex === "number"
        ? aData.keyIndex - Output.brailleState.startOffset
        : -1;

    mm.sendAsyncMessage("AccessFu:Activate", {
      offset,
      activateIfKey: aActivateIfKey,
    });
  },
  ...
  setEditState: function setEditState(aEditState) {
    Logger.debug(() => {
      return ["setEditState", JSON.stringify(aEditState)];
    });
  ...
  doScroll: function doScroll(aDetails) {
    ...
    Utils.winUtils.sendWheelEvent(
      p.x,
      p.y,
      horizontal ? page : 0,
      horizontal ? 0 : page,
      0,
      Utils.win.WheelEvent.DOM_DELTA_PAGE,
      0,
      0,
      0,
      0
  ...
  quickNavMode: {
    ...
    previous: function quickNavMode_previous() {
      Services.prefs.setIntPref(
        QUICKNAV_INDEX_PREF,
        this._currentIndex > 0 ? this._currentIndex - 1 : this.modes.length - 1
      );
    },
    ...
    next: function quickNavMode_next() {
      Services.prefs.setIntPref(
        QUICKNAV_INDEX_PREF,
        this._currentIndex + 1 >= this.modes.length ? 0 : this._currentIndex + 1
      );
    },
